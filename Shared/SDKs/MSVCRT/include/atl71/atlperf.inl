...
inline HRESULT CPerfMon::_OpenAllBlocks() throw()
{
	...
	if (m_aMem.GetCount() == 0)
	{
		...
		m_nHeaderSize  = _AtlAlignUp(m_nHeaderSize,16, ULONG_MAX);
	}
	...
	_ATLTRY
	{
		...
		for (DWORD i=0; i<dwNumItems; i++)
		{
			...
			if (pNameData)
			{
				...
				pNameData += _AtlAlignUp((DWORD)(sizeof(WCHAR) * *pNameData), sizeof(DWORD), DWORD_MAX)/sizeof(DWORD) + 1;
			}
		...
		for (DWORD i=0; i<_GetNumCategories(); i++)
		{
			...
			pCategoryInfo->m_nCounterBlockSize = (ULONG) _AtlAlignUp((ULONG)sizeof(PERF_COUNTER_BLOCK), 8, ULONG_MAX);
			...
			// align at 8 bytes per Q262335
			pCategoryInfo->m_nCounterBlockSize = (ULONG) _AtlAlignUp(pCategoryInfo->m_nCounterBlockSize, 8, ULONG_MAX);
		}
...
inline HRESULT CPerfMon::_SaveMap() throw()
{
	_ATLTRY
	{
		...
		for (UINT i=0; i<_GetNumCategories(); i++)
		{
			...
			{
				nSize += sizeof(DWORD) + _AtlAlignUp((size_t)(sizeof(WCHAR) * pCategoryInfo->m_strName.GetLength()), sizeof(DWORD), SIZE_MAX);
				for (UINT j=0; j<pCategoryInfo->_GetNumCounters(); j++)
				{
					CounterInfo* pCounterInfo = pCategoryInfo->_GetCounterInfo(j);
					nSize += sizeof(DWORD) + _AtlAlignUp((size_t)(sizeof(WCHAR) * pCounterInfo->m_strName.GetLength()), sizeof(DWORD), SIZE_MAX);
				}
			}
		}
		...
		*pCurrent++ = _GetNumCategoriesAndCounters(); // number of items
		...
		for (UINT i=0; i<_GetNumCategories(); i++)
		{
			...
			*pCurrent++ = nLen;
			...
			pCurrent += _AtlAlignUp((INT)(sizeof(WCHAR) * nLen), sizeof(DWORD), INT_MAX)/sizeof(DWORD);
#if _SECURE_ATL
			nSizeLast = nSize;
			...
			for (UINT j=0; j<pCategoryInfo->_GetNumCounters(); j++)
			{
				...
				*pCurrent++ = nCounterLen;
				...
				pCurrent += _AtlAlignUp(sizeof(WCHAR) * nCounterLen, sizeof(DWORD), SIZE_MAX)/sizeof(DWORD);
#if _SECURE_ATL
				nSizeLast = nSize;
...
inline void CPerfMon::_FillCounterDef(CounterInfo* pCounterInfo, ULONG* pnCounterBlockSize) throw()
{
	...
	case PERF_SIZE_VARIABLE_LEN:
		...
		{
			def.CounterSize = (DWORD) _AtlAlignUp((ULONG)(pCounterInfo->m_nMaxCounterSize * sizeof(WCHAR)), sizeof(DWORD), ULONG_MAX);
		}
		else
		{
			def.CounterSize = (DWORD) _AtlAlignUp((ULONG)(pCounterInfo->m_nMaxCounterSize * sizeof(char)), sizeof(DWORD), ULONG_MAX);
		}
		break;
	}
	*pnCounterBlockSize = _AtlAlignUp(*pnCounterBlockSize, dwAlignOfCounter, ULONG_MAX);
	...
	) throw()
{
	...
	else
	{
		...
		// align at 8 bytes per Q262335
		ULONG nNameAlloc = (ULONG) _AtlAlignUp(pInstanceDef->NameLength, 8, DWORD_MAX);
...
#if _SECURE_ATL
#pragma prefast(push)
#pragma prefast(disable: 22013, "Deliberate use of ULONG underflow is tested for explicitly")
