...
public:
	...
	BOOL Append(__in LPCSTR sz, __in int nLen = -1) throw()
	{
		...
		if (newLen > m_dwAlloc)
		{
			size_t nIncr = (nLen+1 > ATL_ISAPI_BUFFER_SIZE) ? nLen+1 : ATL_ISAPI_BUFFER_SIZE;
			if ((DWORD)(m_dwAlloc + nIncr) < m_dwAlloc ||
				!ReAlloc((DWORD)(m_dwAlloc + nIncr)))
			{
				return FALSE;
			}
		}
	...
	while((ch = *szStringIn++) != '\0')
	{
		...
		else if (ch == '%' && (dwFlagsInternal & ATL_URL_DECODE))
		{
			...
			if (*szStringIn != '\0')
			{
				...
				if (*szStringIn != '\0')
				{
					chTemp = (char)AtlHexValue(*szStringIn++);
					if (chTemp < 0)
					{
						bRet = FALSE;
						break;
	...
	while ((ch = *szStringIn) != 0)
	{
		...
		if (bRet)
		{
			if (ch == '%')
			{
				...
				nValueTemp = AtlHexValue(*(++szStringIn));
				if (nValueTemp < 0)
				{
					bRet = FALSE;
					break;
...
public:
	...
	inline BOOL CreateUrl(__out_ecount_part(*pdwMaxLength,*pdwMaxLength) LPTSTR lpszUrl, 
				__inout DWORD* pdwMaxLength, 
				__in DWORD dwFlags = 0) const throw()
	{
		ATLASSERT(lpszUrl != NULL);
...
private:
	...
	{
		ATLENSURE( MAXIMUM_WAIT_OBJECTS >= m_hWaitHandles.GetSize() );
	...
	{
		ATLENSURE( MAXIMUM_WAIT_OBJECTS >= m_ClientEntries.GetSize() );
...
protected:
	...
	DWORD WorkerThreadProc()
	{
		...
		while (TRUE)
		{
			...
			else if (dwRet > (DWORD)WAIT_OBJECT_0 && dwRet < (DWORD)WAIT_OBJECT_0 + (UINT)nSize)
			{
...
public:
	...
	__success(return>=0) __checkReturn HRESULT GetCaps(__in IHttpServerContext * pContext, __deref_out_opt IBrowserCaps ** ppOut)
	{
		if (!pContext)
			return E_POINTER;
	...
	__success(return>=0) __checkReturn HRESULT GetCapsUserAgent(__in BSTR bstrAgent, __deref_out IBrowserCaps ** ppOut)
	{
