...
static inline av_const int av_log2_2(uint16_t low,uint16_t hig)
{
	int n = 0;
	if( hig )
	{
...
{
    if (a&(~65535)) return (uint16_t)((-a)>>31);
...
{
    if ((a+32768) & ~65535) return (int16_t)((a>>31) ^ 32767);
...
typedef struct __AVLock AVLock;
typedef struct __AVMutex AVMutex;

void av_lock_init(AVLock** lk);
void av_lock_close(AVLock** lk);
void av_lock(AVLock* lk);
void av_unlock(AVLock* lk);

void av_mutex_init(AVMutex** mutx, char* mtxname);
void av_mutex_lock(AVMutex* mutx);
void av_mutex_release(AVMutex* mutx);
void av_mutex_close(AVMutex** mutx);

void __cdecl av_register_exit(void (__cdecl*)());
