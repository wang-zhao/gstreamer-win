...
static GHashTable* g_sample_string_hashtable = NULL;

void* init_sample_string_hashtable(GHashTable** pphashtable)
{
	*pphashtable= g_hash_table_new(g_direct_hash, g_direct_equal);

	return pphashtable;
}

const char * get_sample_string(const LangInfo *lang_info)
{
	 static GOnce once_init = G_ONCE_INIT;
	 const  char* ps = NULL;
	 g_once (&once_init, (GThreadFunc)init_sample_string_hashtable, &g_sample_string_hashtable);
 
	ps= g_hash_table_lookup(g_sample_string_hashtable, lang_info);
	if( !ps )
	{ 
		ps= g_locale_to_utf8( lang_pool.str + lang_info->offset, -1, NULL, NULL, NULL);
		g_hash_table_insert(g_sample_string_hashtable, lang_info, ps);
	}
	return ps;
}
/**
...
pango_language_get_sample_string (PangoLanguage *language)
{
  ...
  if (lang_info)
  {
	  return get_sample_string(lang_info);
  }
