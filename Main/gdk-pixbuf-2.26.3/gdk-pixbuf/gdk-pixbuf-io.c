                                ...
                                GError **error)
{
        ...
        if (module->load != NULL) {
                ...
                while (!g_feof (f) && !g_ferror (f)) {
                        length = g_fread (buffer, 1, sizeof (buffer), f);
                        if (length > 0)
                          ...
                          GError    **error)
{
        ...
        size = g_fread (&buffer, 1, sizeof (buffer), f);
        if (size == 0) {
                g_set_error (error,
                             GDK_PIXBUF_ERROR,
                                   ...
                                   GError    **error)
{
        ...
        has_frame = FALSE;
        while (!has_frame && !g_feof (f) && !g_ferror (f)) {
                length = g_fread (buffer, 1, sizeof (buffer), f);
                if (length > 0)
                       ...
                       gpointer     data)
{
        ...
        n = g_fwrite (buf, 1, count, filehandle);
        if (n != count) {
                gint save_errno = errno;
                g_set_error (error,
                             G_FILE_ERROR,
                                ...
                                GError           **error)
{
        ...
        rewind (f);
        for (;;) {
                n = g_fread (buf, 1, TMP_FILE_BUF_SIZE, f);
                if (n > 0) {
