...
setup_msw_rc_style(void) {
    ...
    /* Prelight */
    sys_color_to_gtk_color(get_windows_version() >= VISTA_VERSION ? XP_THEME_CLASS_MENU :
                           XP_THEME_CLASS_TEXT,
...
draw_harrow(GdkWindow* window,
    ...
    cairo_line_to(cr, x_start + (width - 1) * x_increment + 0.5,
                  y + height - (width - 1 - extra) - 1 + 0.5);
    cairo_close_path(cr);
    cairo_stroke_preserve(cr);
    cairo_fill(cr);
...
draw_flat_box(GtkStyle* style, GdkWindow* window,
    ...
    if (detail) {
        if (state_type == GTK_STATE_SELECTED &&
            ...
            GdkColor* gc = gtk_widget_has_focus(widget) ? &style->base[state_type] :
                           &style->base[GTK_STATE_ACTIVE];
            cairo_t* cr = gdk_cairo_create(window);
