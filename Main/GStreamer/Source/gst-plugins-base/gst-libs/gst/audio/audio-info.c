...
gst_audio_info_from_caps(GstAudioInfo* info, const GstCaps* caps) {
    ...
    if (!gst_structure_has_name(str, "audio/x-raw-int") &&
            !gst_structure_has_name(str, "audio/x-raw-float")) {
        goto wrong_name;
    }
    ...
    format = gst_audio_format_build_integer(sign, endianness, width, depth);
