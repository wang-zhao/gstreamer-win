...
GstEncodingProfile * 
gst_discoverer_info_make_profile (GstDiscovererInfo * info)
{
  ...
  if (GST_IS_DISCOVERER_CONTAINER_INFO (sinfo)) {
    ...
    for (tmp = substreams; tmp; tmp = tmp->next) {
      ...
      info = gst_caps_to_string(copy_caps);
      g_free(info);

      if (GST_IS_DISCOVERER_VIDEO_INFO (stream)) {

        gst_structure_remove_field(gst_caps_get_structure(copy_caps, 0), "alignment");
        gst_structure_remove_field(gst_caps_get_structure(copy_caps, 0), "codec_data");

        sprof = (GstEncodingProfile *)
          gst_encoding_video_profile_new (copy_caps, NULL, NULL, 1);
      } else if (GST_IS_DISCOVERER_AUDIO_INFO (stream)) {

        GstDiscovererAudioInfo* ainfo = GST_DISCOVERER_AUDIO_INFO(stream);
        // 修正 Rate Channel Depth等参数
        caps_st = gst_caps_get_structure(copy_caps,0);

        if( gst_structure_has_field(caps_st,"rate"))
        {
          gint rate = 0;
          gst_structure_get_int(caps_st, "rate", &rate);

          if( rate != ainfo->sample_rate )
          {
            gst_structure_set(caps_st, "rate", G_TYPE_INT, ainfo->sample_rate, 0);
          }

        }
        if( gst_structure_has_field(caps_st,"channels"))
        {
          gint chs = 0;
          gst_structure_get_int(caps_st, "channels", &chs);

          if( chs != ainfo->channels )
          {
            gst_structure_set(caps_st, "channels", G_TYPE_INT, ainfo->channels,0);
          }
        }

        if( gst_structure_has_field(caps_st,"depth"))
        {
          gint depth = 0;
          gst_structure_get_int(caps_st, "depth", &depth);

          if( depth != ainfo->depth )
          {
            gst_structure_set(caps_st, "depth", G_TYPE_INT, ainfo->depth,0);
          }
        }

        sprof = (GstEncodingProfile *)
          gst_encoding_audio_profile_new (copy_caps, NULL, NULL, 1);
      } else {
