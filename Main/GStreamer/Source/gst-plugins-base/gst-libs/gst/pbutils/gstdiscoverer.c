    ...
    GstDiscovererStreamInfo * parent)
{
  ...
  if (g_str_has_prefix (name, "audio/")) {
    ...
    //////////////////////////////////////////////////////////////////////////
    // [1/7/2013 wangzhao]
    // 修改 Rate, Channels, Depth 等参数.

    if( info->parent.caps != caps)
    {
      caps_st = gst_caps_get_structure (info->parent.caps, 0);
      ...
      if( gst_structure_has_field(caps_st,"rate"))
      {
        ...
        {
          gst_structure_set(caps_st, "rate", G_TYPE_INT, info->sample_rate, 0);
        }
      ...
      if( gst_structure_has_field(caps_st,"channels"))
      {
        ...
        if( chs != info->channels )
        {
          gst_structure_set(caps_st, "channels", G_TYPE_INT, info->channels, 0);
        }
      }
      ...
      if( gst_structure_has_field(caps_st,"depth"))
      {
        ...
        if( depth != info->depth )
        {
          gst_structure_set(caps_st, "depth", G_TYPE_INT, info->depth, 0);
        }
      }
    }
    ...
    if (nval == NULL) {
      ...
      if (gst_structure_id_get (st, _CAPS_QUARK, GST_TYPE_CAPS, &caps, NULL)) {
        if (gst_caps_can_intersect (parent->caps, caps)) {
          ...
          if( GST_IS_DISCOVERER_AUDIO_INFO(next) && GST_IS_DISCOVERER_AUDIO_INFO(res))
          {
            GstDiscovererAudioInfo* next_info = GST_DISCOVERER_AUDIO_INFO(next);
            GstDiscovererAudioInfo* res_info = GST_DISCOVERER_AUDIO_INFO(res);

            if( res_info->sample_rate == 0 && next_info->sample_rate != 0 )
            {
              res_info->sample_rate = next_info->sample_rate;
            }

            if( res_info->channels == 0 && next_info->channels != 0 )
            {
              res_info->channels = next_info->channels;
            }
          }
        }
        gst_caps_unref (caps);
