...
// *
// * Converts a FOURCC value into the corresponding #gstGstVideoFormat.
...
int gst_video_format_add_top_border(GstVideoFormat format, int borders,
                                    int offsets[3], int strides[3]) {
    g_return_val_if_fail(format != GST_VIDEO_FORMAT_UNKNOWN, 0);
    g_return_val_if_fail((!gst_video_format_is_yuv(format)) || (borders > 0), 0);

    switch (format) {
    case GST_VIDEO_FORMAT_I420:
    case GST_VIDEO_FORMAT_YV12:
        borders = GST_ROUND_DOWN_2(borders);

        if (borders > 0) {
            offsets[0] += borders * strides[0];
            offsets[1] += borders / 2 * strides[1];
            offsets[2] += borders / 2 * strides[2];
        }
    ...
    case GST_VIDEO_FORMAT_YUV9:
    case GST_VIDEO_FORMAT_YVU9:
        borders = GST_ROUND_DOWN_4(borders);

        if (borders > 0) {
            offsets[0] += borders * strides[0];
            offsets[1] += borders / 4 * strides[1];
            offsets[2] += borders / 4 * strides[2];
        }
