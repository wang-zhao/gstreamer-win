...
#ifdef G_OS_WIN32
  if (ioctlsocket (fd, FIONBIO, &noblock) < 0) {
#else
...
#define FLAGS 0
      ...
      if (wrote < 0) {
        ...
        {
          GST_INFO_OBJECT(sink, "send buffer is full, wouldblock recived.");
          gst_poll_fd_ctl_status(sink->fdset, &client->fd, FD_WRITE, FALSE);
          wrote = 0;
          goto except;
...
restart:
  ...
  for (clients = sink->clients; clients; clients = next) {
    ...
    if ((max_buffers > 0 && client->bufpos >= max_buffers) ||
        (sink->timeout > 0 && now > client->last_activity_time  /* be sure now > client->last_activity_time.*/
            && now - client->last_activity_time > sink->timeout)) {
      /* remove client */
              GST_WARNING_OBJECT (sink, "[fd %5d] client %p is too slow, removing."
                "last queue data time:%" G_GINT64_FORMAT ", now: %" G_GINT64_FORMAT ","
                "last active time %" G_GINT64_FORMAT ", bytes send %" G_GINT64_FORMAT "",
          client->fd.fd, client, sink->last_data_queue_time, now,
          client->last_activity_time, client->bytes_sent);
...
gst_multi_fd_sink_handle_clients (GstMultiFdSink * sink)
{
  ...
  do {
    ...
    * - client socket output (ie, client reads)          */
    ...
    result = gst_poll_wait (sink->fdset, GST_MSECOND*100);
    ...
    * and will not disconnect innactive client in the streaming thread. */
    if (G_UNLIKELY (result == 0)) {
      ...
      for (clients = sink->clients; clients; clients = next) {
        ...
        client = (GstTCPClient *) clients->data;
        next = g_list_next (clients);
        if (sink->timeout > 0 && now > client->last_activity_time 
          && now - client->last_activity_time > sink->timeout) {
            client->status = GST_CLIENT_STATUS_SLOW;
            gst_multi_fd_sink_remove_client_link (sink, clients);
        }
...
restart:
        ...
        for (clients = sink->clients; clients; clients = next) {
          ...
          ttv.tv_sec =1;
          ttv.tv_usec=0;
          select(1, NULL, NULL, &fd_e, &ttv);
          if( FD_ISSET(fd, &fd_e)){
		  	        GST_WARNING_OBJECT (sink, "socket error for %d, removing: %s (%d)",
              fd, g_strerror (h_errno), h_errno);
		  {
#else
...
restart2:
  ...
  for (clients = sink->clients; clients; clients = next) {
	...
	if(!gst_poll_fd_is_active (sink->fdset,&client->fd))
        continue;
...
gst_multi_fd_sink_start (GstBaseSink * bsink)
{
  ...
  g_thread_set_priority(this->thread, G_THREAD_PRIORITY_HIGH);
