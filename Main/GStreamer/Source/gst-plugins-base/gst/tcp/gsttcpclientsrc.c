...
#define DEFAULT_TIMEOUT					30000000000
...
gst_tcp_client_src_class_init (GstTCPClientSrcClass * klass)
{
	...
	g_object_class_install_property (gobject_class, PROP_TIMEOUT,
		g_param_spec_int64("timeout", "Timeout", "timeout for rec data, 0 no timeout(ns)",0, LLONG_MAX,
...
gst_tcp_client_src_create (GstPushSrc * psrc, GstBuffer ** outbuf)
{
  ...
  GTimeVal gtime;
  GstClockTime gsttime;

  g_get_current_time(&gtime);
  gsttime = GST_TIMEVAL_TO_TIME (gtime);
    ...
    src->last_recv_data_time = gsttime;
    src->bytes_read += GST_BUFFER_SIZE(*outbuf);
	}
  else
  {
    GST_WARNING_OBJECT(src, "tcp read data error: last recv data time : %" G_GINT64_FORMAT ", now: %" G_GINT64_FORMAT ", bytes read:%" G_GINT64_FORMAT,
      src->last_recv_data_time, gsttime, src->bytes_read);
  }
								 ...
								 const GValue * value, GParamSpec * pspec)
{
	...
	case PROP_PROTOCOL:
		...
		break;
	case PROP_TIMEOUT:
		tcpclientsrc->timeout = g_value_get_int64(value);
		break;
	default:
								 ...
								 GValue * value, GParamSpec * pspec)
{
	...
	case PROP_PROTOCOL:
		...
		break;
	case PROP_TIMEOUT:
		g_value_set_int64(value, tcpclientsrc->timeout);
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
		break;
