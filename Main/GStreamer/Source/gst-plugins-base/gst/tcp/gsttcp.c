...
gst_tcp_socket_write (int socket, const void *buf, size_t count)
{
	...
	while (bytes_written < count) {
		int wrote = send (socket, (const char *) buf + bytes_written,
					 ...
					 GstPoll * fdset)
{
	...
	while (bytes_read < count) {
		...
		n = recv(socket, ((guint8 *) buf) + bytes_read, num_to_read, 0);
...
select_error:
	{
		GST_ELEMENT_ERROR (this, RESOURCE, READ, (NULL),
			("select failed: %s", g_strerror (h_errno)));
...
ioctl_error:
	{
		GST_ELEMENT_ERROR (this, RESOURCE, READ, (NULL),
			("ioctl failed: %s", g_strerror (h_errno)));
		return GST_FLOW_ERROR;
	}
got_eos:
	{
...
read_error:
	{
		GST_ELEMENT_ERROR (this, RESOURCE, READ, (NULL),
			("read failed: %s", g_strerror (h_errno)));
		return GST_FLOW_ERROR;
	}
short_read:
	{
					 ...
					 GstBuffer ** buf, gint64 timeout)
{
	...
	*buf = NULL;
	...
	if(timeout > 0)
		gst_timeout = MAX(3*GST_SECOND, timeout);
	else
		gst_timeout = GST_CLOCK_TIME_NONE;

	for(;;){
		if ((ret = gst_poll_wait (fdset, gst_timeout)) < 0) {
			if (ret == -1 && errno == EBUSY)
				goto cancelled;
			else
				goto select_error;
		...
		/*ret == 0 means timeout, try anyway*/
		if (readsize > 0 || ret == 0)
		{
			if( readsize <= 0 )
				readsize = 0x10;
	...
	*buf = gst_buffer_new_and_alloc (readsize);
	...
	bytes_read = recv(socket, GST_BUFFER_DATA (*buf), readsize,0);
