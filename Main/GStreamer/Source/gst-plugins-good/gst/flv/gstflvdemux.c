...
#define DEFAULT_KEEP_PADS FALSE
...
static void
gst_flv_demux_set_property(GObject* object, guint prop_id,
                           const GValue* value, GParamSpec* pspec);

static void
gst_flv_demux_get_property(GObject* object, guint prop_id,
...
gst_flv_demux_cleanup(GstFlvDemux* demux) {
    ...
    if (demux->audio_pad && !demux->keep_pads) {
        gst_element_remove_pad(GST_ELEMENT(demux), demux->audio_pad);
        gst_object_unref(demux->audio_pad);
        demux->audio_pad = NULL;
    }

    if (demux->video_pad && !demux->keep_pads) {
...
gst_flv_demux_class_init(GstFlvDemuxClass* klass) {
    ...
    g_object_class_install_property(gobject_class, PROP_KEEP_PADS,
                                    g_param_spec_boolean("keep-pads", "keep pads", "Dont remove pads when pipeline stopped",
                                            DEFAULT_KEEP_PADS, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
...
gst_flv_demux_set_property(GObject* object, guint prop_id,
    ...
    dmx = GST_FLV_DEMUX(object);

    switch (prop_id) {
    case PROP_KEEP_PADS:
        dmx->keep_pads = g_value_get_boolean(value);
        break;
...
gst_flv_demux_get_property(GObject* object, guint prop_id,
    ...
    switch (prop_id) {
    case PROP_KEEP_PADS:
        g_value_set_boolean(value, dmx->keep_pads);
        break;
...
gst_flv_demux_init(GstFlvDemux* demux, GstFlvDemuxClass* g_class) {
    ...
    demux->own_index = FALSE;
    demux->keep_pads = DEFAULT_KEEP_PADS;
