...
static GstPadChainFunction collected_chainfun = NULL;
static GstPadEventFunction collected_eventfun = NULL;
...
struct _GstVideoMixer2Collect {
    ...
    __declspec(align(4)) GstBuffer* buffer;            /* buffer that should be blended now */
...
static void
gst_videomixer2_pad_clear(GstVideoMixer2Pad* mpad) {
...
gst_videomixer2_pad_init(GstVideoMixer2Pad* mixerpad) {
    ...
    mixerpad->ismain = FALSE;
    mixerpad->iseos = FALSE;
}
...
gst_videomixer2_blend_buffers(GstVideoMixer2* mix,
    ...
    for (l = mix->sinkpads; l; l = l->next) {
        ...
        if (mixcol->buffer != NULL) {
            buf = gst_buffer_ref(mixcol->buffer);
        }
...
static void
gst_videomixer2_start(GstVideoMixer2* mix) {

    GSList* l = NULL;
    gboolean setted = FALSE;
    ...
    for (l = mix->sinkpads; l; l = l->next) {
        ...
        gst_videomixer2_pad_clear(mpad);
        ...
        if (mpad->fps_d == mix->fps_d && mpad->fps_n == mix->fps_n) {
            mpad->ismain = TRUE;
            setted = TRUE;
        } else {
            mpad->ismain = FALSE;
    ...
    if (!setted) {
        for (l = mix->sinkpads; l; l = l->next) {
            ...
            if (mpad->width == mix->width && mpad->height == mix->height) {
                mpad->ismain = TRUE;
                setted = TRUE;
                break;
            }
...
static void
gst_videomixer2_stop(GstVideoMixer2* mix) {
...
gst_videomixer2_change_state(GstElement* element, GstStateChange transition) {
    ...
    case GST_STATE_CHANGE_READY_TO_PAUSED:
        gst_videomixer2_start(mix);
        break;

    case GST_STATE_CHANGE_PAUSED_TO_READY:
        gst_videomixer2_stop(mix);
        break;
...
static GstFlowReturn
gst_videomixer2_chain_live(GstPad* ipad, GstBuffer* buffer) {
    GstFlowReturn ret = GST_FLOW_OK;
    GstVideoMixer2Pad* pad = GST_VIDEO_MIXER2_PAD(ipad);
    GstVideoMixer2* mix = GST_VIDEO_MIXER2(gst_pad_get_parent(pad));
    ...
    GST_OBJECT_LOCK(pad);
    gst_buffer_replace(&pad->mixcol->buffer, buffer);
    GST_OBJECT_UNLOCK(pad);
    ...
    if (pad->ismain) {
        GstBuffer* outbuf = NULL;
        gst_videomixer2_blend_buffers(mix, GST_BUFFER_TIMESTAMP(buffer),
                                      GST_BUFFER_TIMESTAMP(buffer) + GST_BUFFER_DURATION(buffer), &outbuf);
...
static gboolean
gst_videomixer2_sink_event_live(GstPad* ipad, GstEvent* event) {
    GstVideoMixer2Pad* pad = GST_VIDEO_MIXER2_PAD(ipad);
    GstVideoMixer2* mix = GST_VIDEO_MIXER2(gst_pad_get_parent(pad));
...
gst_videomixer2_request_new_pad(GstElement* element,
    ...
    if (templ == gst_element_class_get_pad_template(klass, "sink_%d")) {
        ...
        if (collected_chainfun == NULL) {
            collected_chainfun = GST_PAD_CHAINFUNC(mixpad);
        }

        if (collected_eventfun == NULL) {
            collected_eventfun = GST_PAD_EVENTFUNC(mixpad);
        }

        if (mix->islive) {
            gst_pad_set_chain_function(GST_PAD(mixpad), GST_DEBUG_FUNCPTR(gst_videomixer2_chain_live));
            gst_pad_set_event_function(GST_PAD(mixpad), GST_DEBUG_FUNCPTR(gst_videomixer2_sink_event_live));
        }
...
gst_videomixer2_get_property(GObject* object,
    ...
    case PROP_LIVE:
        g_value_set_boolean(value, mix->islive);
        break;
...
gst_videomixer2_set_property(GObject* object,
    ...
    case PROP_LIVE: {
        gboolean islive = g_value_get_boolean(value);

        GstPadChainFunction chainfun = NULL;
        GstPadEventFunction eventfun = NULL;

        GST_VIDEO_MIXER2_LOCK(mix);

        if (GST_STATE(mix) != GST_STATE_NULL) {
            if (mix->islive != g_value_get_boolean(value)) {
                GST_ELEMENT_WARNING(mix, CORE, STATE_CHANGE,
                                    ("can not change value of property 'live' when pipeline is running"), (NULL));
            }
        } else  {
            GSList* l = NULL;
            GstPad* pad = NULL;
            ...
            if (mix->islive != g_value_get_boolean(value)) {
                mix->islive = g_value_get_boolean(value);

                if (mix->islive) {
                    chainfun = gst_videomixer2_chain_live;
                    eventfun = gst_videomixer2_sink_event_live;
                } else {
                    chainfun = collected_chainfun;
                    eventfun = collected_eventfun;
                }

                for (l = mix->sinkpads; l; l = l->next) {
                    pad = GST_PAD(l->data);

                    gst_pad_set_chain_function(pad, chainfun);
                    gst_pad_set_event_function(pad, eventfun);
                }
...
gst_videomixer2_class_init(GstVideoMixer2Class* klass) {
    ...
    g_object_class_install_property(gobject_class, PROP_LIVE,
                                    g_param_spec_boolean("live", "Live", "Live or not",
