...
static void 
gst_multipart_mux_init_pad(GstMultipartPadData* pad)
{
  pad->mux_buffer_num = 0;
  pad->last_send_caps_buffer_num = -1;
  pad->buffer = NULL;
    ...
    GstPadTemplate * templ, const gchar * req_name)
{
   ...
   * keep track of its status */
  {
    ...
    gst_multipart_mux_init_pad(multipartpad);
...
gst_multipart_mux_queue_pads (GstMultipartMux * mux)
{
  ...
  while (walk) {
    ...
    buf = gst_collect_pads_peek (mux->collect, data);
    if (buf == NULL) {
      GST_LOG_OBJECT (mux, "Pad %s has no buffers",
    ...
    if (G_LIKELY (GST_CLOCK_TIME_IS_VALID (time))) {
      time = gst_segment_to_running_time (&data->segment, GST_FORMAT_TIME, time);      
    }else
    {
      time = GST_CLOCK_TIME_NONE;
    }

    if (bestpad == NULL || !GST_CLOCK_TIME_IS_VALID (time) ||
  ...
  if (bestpad != NULL) {
    GST_LOG_OBJECT (mux, "selected pad %s with time %" GST_TIME_FORMAT,
      GST_PAD_NAME (bestpad->collect.pad), GST_TIME_ARGS (best_time));
    buf = gst_collect_pads_pop (mux->collect, &bestpad->collect);

    bestpad->timestamp = best_time;
    bestpad->buffer = buf;
  }
  return bestpad;
}
...
gst_multipart_mux_collected (GstCollectPads * pads, GstMultipartMux * mux)
{
  ...
  if(best->last_send_caps_buffer_num == -1 || 
    best->mux_buffer_num - best->last_send_caps_buffer_num  >= SEND_CAPS_BUFFER_NUM)
  {
    header = g_strdup_printf ("\r\n--%s\r\n"
      ...
      GST_BUFFER_DURATION(best->buffer),
      GST_MINI_OBJECT_FLAGS(best->buffer),
      cap_string = gst_structure_to_string(structure));

    g_free(cap_string);

    best->last_send_caps_buffer_num = best->mux_buffer_num;

	GST_INFO_OBJECT (mux, "%s, outgoing %s", best->collect.pad->object.name, header  );
  }
  else
  {
    header = g_strdup_printf ("\r\n--%s\r\n"
...
eos:
  {
    GST_INFO_OBJECT (mux, "Pushing EOS");
...
static void 
gst_multipart_mux_prepare(GstMultipartMux *multipart_mux)
{
  GSList *walk = NULL;

  g_return_if_fail (GST_IS_MULTIPART_MUX (multipart_mux));

  /* try to make sure we have a buffer from each usable pad first */
  walk = multipart_mux->collect->data;
  while (walk) {
    GstCollectData *data = (GstCollectData *) walk->data;
    GstMultipartPadData *pad = (GstMultipartPadData *) data;
    gst_multipart_mux_init_pad(pad);
    walk = g_slist_next (walk);
  }
}
...
gst_multipart_mux_change_state (GstElement * element, GstStateChange transition)
{
  ...
  switch (transition) {
    case GST_STATE_CHANGE_READY_TO_PAUSED:
      ...
      gst_multipart_mux_prepare(multipart_mux);
...
#ifdef _DEBUG
//  gst_debug_set_threshold_for_name("multipartmux", GST_LEVEL_INFO);
#endif 
