    ...
    GstMultipartDemuxClass * g_class)
{
  ...
  multipart->timestamp_offset = GST_CLOCK_TIME_NONE;
  multipart->buffer_timestamp = GST_CLOCK_TIME_NONE;
  multipart->buffer_duration = GST_CLOCK_TIME_NONE;
  multipart->buffer_caps = NULL;
  multipart->buffer_flags = 0;

#ifdef _DEBUG
//  multipart->dump_file= fopen("d:\\multdemux.dat", "wb+");
#endif
}
    ...
    gboolean * created)
{
  ...
  /* dont create pad util find caps*/
  if( demux->buffer_caps )
  {
    GstPad *pad;
...
//    GstCaps *caps;
    ...
    GST_INFO_OBJECT (demux, "creating pad with mime: %s", mime);
    ...
    gst_pad_set_caps (pad, demux->buffer_caps);
    //gst_caps_unref (caps);

    mppad->pad = pad;
    mppad->mime = g_strdup (mime);
    mppad->last_ret = GST_FLOW_OK;
    mppad->is_caps_parsed = FALSE;
    mppad->start_timestamp = GST_CLOCK_TIME_NONE;
    ...
    while (get_line_end (pos, dataend, &end, &next)) {
      ...
      if (len >= 14 && !g_ascii_strncasecmp ("content-type:", (gchar *) pos, 13)) {
        ...
        * properties that we don't handle yet. */
        ...
        !g_ascii_strncasecmp ("timestamp:", (gchar *) pos, 10)) {
          multipart->buffer_timestamp =
            (GstClockTime)g_ascii_strtoull ((gchar *) pos + 10, NULL, 10);
      }
      else if( len >= 10 && 
        !g_ascii_strncasecmp ("duration:", (gchar *) pos, 9)) {
          multipart->buffer_duration =
            (GstClockTime)g_ascii_strtoull ((gchar *) pos + 9, NULL, 10);
      }
      else if( len >= 7 && 
        !g_ascii_strncasecmp ("flags:", (gchar *) pos, 6)) {
          multipart->buffer_flags =
            (guint)g_ascii_strtoull ((gchar *) pos + 6, NULL, 10);
      }
      else if( len >= 6 &&
        !g_ascii_strncasecmp ("caps:", (gchar *) pos, 5)) {
          gchar* caps_string;
          caps_string = g_strndup ((gchar *) (pos + 6), len-6);

          multipart->buffer_caps = gst_caps_from_string ( caps_string );
          g_free(caps_string);
      }
      pos = next;
    }
...
#ifdef _DEBUG
//  fwrite(GST_BUFFER_DATA(buf), GST_BUFFER_SIZE(buf), 1, multipart->dump_file);
#endif
  res = GST_FLOW_OK;
  ...
  while (gst_adapter_available (adapter) > 0) {
    ...
    if (G_UNLIKELY (!multipart->header_completed)) {
      if ((size = multipart_parse_header (multipart)) < 0) {
        ...
        {
          char msg[100];
          sprintf(msg, "MultipartDemux: %s, \tCaps %p\tflags %p\r\n", multipart->mime_type, multipart->buffer_caps, multipart->buffer_flags);
          OutputDebugStringA(msg);
        }
#endif
		GST_INFO_OBJECT(multipart, "%s, \tCaps %p\tflags %p\t timestamp: %" G_GINT64_FORMAT , multipart->mime_type, multipart->buffer_caps, multipart->buffer_flags,
			multipart->buffer_timestamp);
      }
    }
    ...
    if (G_UNLIKELY (datalen <= 0)) {
      ...
      if( !srcpad->is_caps_parsed && !multipart->buffer_caps)
      {
        gst_buffer_unref(outbuf);
      }
      else{
        if( multipart->buffer_caps )
        {
          if ( !srcpad->is_caps_parsed )
          {     
            srcpad->is_caps_parsed = TRUE;
          }
          gst_caps_unref(multipart->buffer_caps);
          multipart->buffer_caps = NULL;
        }
        if( multipart->buffer_duration != GST_CLOCK_TIME_NONE )
        {
          GST_BUFFER_DURATION(outbuf) = multipart->buffer_duration;
          multipart->buffer_duration = GST_CLOCK_TIME_NONE;
        }

        if( multipart->buffer_timestamp != GST_CLOCK_TIME_NONE )
        {
          if( multipart->timestamp_offset== GST_CLOCK_TIME_NONE)
          {
            multipart->timestamp_offset = multipart->buffer_timestamp;
            if( multipart->timestamp_offset > 3*GST_SECOND)
              multipart->timestamp_offset -= 3*GST_SECOND;
            else 
              multipart->timestamp_offset = 0;
          }

          GST_BUFFER_TIMESTAMP(outbuf) =MAX(0, ((gint64)(multipart->buffer_timestamp - multipart->timestamp_offset)));
          multipart->buffer_timestamp = GST_CLOCK_TIME_NONE;
        }

        if( multipart->buffer_flags != 0 )
        {
          GST_MINI_OBJECT_FLAGS(outbuf) = multipart->buffer_flags;
          multipart->buffer_flags = 0;
        }
    ...
    case GST_STATE_CHANGE_PAUSED_TO_READY:
      {
        ...
        walk = multipart->srcpads;
        while (walk) {
          GstMultipartPad *pad = (GstMultipartPad *) walk->data;

          pad->is_caps_parsed = FALSE;
          pad->start_timestamp = GST_CLOCK_TIME_NONE;
...
#ifdef _DEBUG
//  gst_debug_set_threshold_for_name("multipartdemux", GST_LEVEL_INFO);
#endif 
