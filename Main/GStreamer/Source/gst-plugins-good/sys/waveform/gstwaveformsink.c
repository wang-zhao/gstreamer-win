...
gst_waveform_sink_getcaps (GstBaseSink * bsink)
{
  ...
  if (wocaps.dwFormats & WAVE_FORMAT_48S16) {
    caps_temp = gst_waveform_sink_create_caps (48000, 2, 16);
    if (caps_temp) {
      gst_caps_append (caps, caps_temp);
    }
  }
  if (wocaps.dwFormats & WAVE_FORMAT_48S08) {
    caps_temp = gst_waveform_sink_create_caps (48000, 2, 8);
    if (caps_temp) {
      gst_caps_append (caps, caps_temp);
    }
  }
  if (wocaps.dwFormats & WAVE_FORMAT_48M16) {
    caps_temp = gst_waveform_sink_create_caps (48000, 1, 16);
    if (caps_temp) {
      gst_caps_append (caps, caps_temp);
    }
  }
  if (wocaps.dwFormats & WAVE_FORMAT_48M08) {
    caps_temp = gst_waveform_sink_create_caps (48000, 1, 8);
    if (caps_temp) {
      gst_caps_append (caps, caps_temp);
    }
  }
