...
gst_osx_video_sink_set_state (GstElement * element, GstState state);
...
// memcpy (viewdata, GST_BUFFER_DATA (buf), GST_BUFFER_SIZE (buf));
  [osxvideosink->osxwindow->gstview displayTexture: GST_BUFFER_DATA (buf): GST_BUFFER_SIZE (buf)];
...
gst_osx_video_sink_class_init (GstOSXVideoSinkClass * klass)
{
  ...
  gstelement_class->change_state = gst_osx_video_sink_change_state;
    /*
     do each state change on main thread, because of UI opreations
     */
    gstelement_class->set_state = gst_osx_video_sink_set_state;
...
@interface set_state_args : NSObject
{
    GstElement* element;
    GstState state;
...
@property GstState state;
@property GstStateChangeReturn result;

- (void) gst_osx_video_sink_set_state_real;
@end

@ implementation set_state_args

@synthesize element, state, result;

- (void) gst_osx_video_sink_set_state_real {
    result = (GST_ELEMENT_CLASS (parent_class))->set_state (element,  state);
...
gst_osx_video_sink_set_state (GstElement * element, GstState state)
{
    ...
    arg = [set_state_args alloc] ;
    [arg setElement: gst_object_ref(element)];
    [arg setState: state];
    
    [arg gst_osx_video_sink_set_state_real];
