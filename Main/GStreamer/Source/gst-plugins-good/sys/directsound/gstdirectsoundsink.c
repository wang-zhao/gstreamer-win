...
gst_directsound_sink_prepare (GstAudioSink * asink, GstRingBufferSpec * spec)
{
  ...
  GUID SUBFORMAT_SUBTYPE_PCM = {0x00000001, 0x0000, 0x0010, 0x80, 0x00, 0x00, 0xaa, 0x00, 0x38, 0x9b, 0x71};
  GUID SUBFORMAT_SUBTYPE_IEEE_FLOAT = {0x00000003, 0x0000, 0x0010, 0x80, 0x00, 0x00, 0xaa, 0x00, 0x38, 0x9b, 0x71};
  ...
  if (spec->format != GST_IEC958) {
    ...
    wfx.Format.nChannels = spec->channels;
    wfx.Format.nSamplesPerSec = spec->rate;
    wfx.Format.wBitsPerSample = (spec->bytes_per_sample * 8) / wfx.Format.nChannels;
    wfx.Format.nBlockAlign = spec->bytes_per_sample;
    wfx.Format.nAvgBytesPerSec = wfx.Format.nSamplesPerSec * wfx.Format.nBlockAlign;

    //  如果声道数大于2，使用Extension
    if( wfx.Format.nChannels > 2 )
    {
      ...
      wfx.Format.wFormatTag = WAVE_FORMAT_EXTENSIBLE;
       ...
       //KSAUDIO_SPEAKER_QUAD  4
       //KSAUDIO_SPEAKER_SURROUND        4
       //KSAUDIO_SPEAKER_5POINT1        6
       //KSAUDIO_SPEAKER_7POINT1        8
      wfx.Samples.wValidBitsPerSample = wfx.Format.wBitsPerSample;
      wfx.dwChannelMask = (1 << wfx.Format.nChannels)-1;
      ...
      case GST_FLOAT64_LE:
      case GST_FLOAT64_BE:
        wfx.SubFormat =SUBFORMAT_SUBTYPE_IEEE_FLOAT ;
        break;
      default:   
        wfx.SubFormat = SUBFORMAT_SUBTYPE_PCM;
        break;
      }
    }
    ...
    dsoundsink->buffer_size =
        gst_util_uint64_scale_int (wfx.Format.nAvgBytesPerSec, spec->buffer_time,
        GST_MSECOND);
    ...
    spec->segsize =
        gst_util_uint64_scale_int (wfx.Format.nAvgBytesPerSec, spec->latency_time,
        GST_MSECOND);
  ...
  GST_INFO_OBJECT (dsoundsink,
      ...
      spec->bytes_per_sample, wfx.Format.nSamplesPerSec, wfx.Format.wBitsPerSample,
      wfx.Format.nBlockAlign, wfx.Format.nAvgBytesPerSec, dsoundsink->buffer_size);
