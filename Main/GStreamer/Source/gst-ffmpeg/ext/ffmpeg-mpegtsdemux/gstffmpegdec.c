...
gst_ts_info_store (GstFFMpegDec * dec, GstClockTime timestamp,
    GstClockTime duration, gint64 offset, gboolean preroll)
{
  gint idx = dec->ts_idx;
  ...
  dec->ts_info[idx].preroll = preroll;
...
gst_ffmpegdec_init (GstFFMpegDec * ffmpegdec)
{
  ...
  /* some ffmpeg data */
  ffmpegdec->context = g_demuxer.avcodec_alloc_context ();
  ffmpegdec->picture = g_demuxer.avcodec_alloc_frame ();
  ffmpegdec->pctx = NULL;
...
gst_ffmpegdec_finalize (GObject * object)
{
  ...
  if (ffmpegdec->context != NULL) {
    g_demuxer.av_free (ffmpegdec->context);
    ffmpegdec->context = NULL;
  }

  if (ffmpegdec->picture != NULL) {
    g_demuxer.av_free (ffmpegdec->picture);
...
gst_ffmpegdec_close (GstFFMpegDec * ffmpegdec)
{
  ...
  if (ffmpegdec->context->palctrl) {
    g_demuxer.av_free (ffmpegdec->context->palctrl);
    ffmpegdec->context->palctrl = NULL;
  }

  if (ffmpegdec->context->extradata) {
    g_demuxer.av_free (ffmpegdec->context->extradata);
  ...
  if (ffmpegdec->pctx) {
    ...
    g_demuxer.av_parser_close (ffmpegdec->pctx);
    ffmpegdec->pctx = NULL;
  }
  ...
  switch (oclass->in_plugin->id) {
    ...
    case CODEC_ID_H264:
      ...
      if (ffmpegdec->context->extradata_size == 0) {
        ...
        ffmpegdec->pctx = g_demuxer.av_parser_init (oclass->in_plugin->id);
      } else {
...
gst_ffmpegdec_setcaps (GstPad * pad, GstCaps * caps)
{
  ...
  if (ffmpegdec->opened) {
    ...
    g_demuxer.avcodec_get_context_defaults (ffmpegdec->context);
  }
...
gst_ffmpegdec_get_buffer (AVCodecContext * context, AVFrame * picture)
{
  ...
  if (!ffmpegdec->current_dr) {
    ...
    res = g_demuxer.avcodec_default_get_buffer (context, picture);
  ...
  switch (context->codec_type) {
    ...
    case CODEC_TYPE_UNKNOWN:
    {
      ...
      g_demuxer.avcodec_align_dimensions (context, &width, &height);
      ...
      if (width != clip_width || height != clip_height) {
        ...
        return g_demuxer.avcodec_default_get_buffer (context, picture);
      }
...
gst_ffmpegdec_release_buffer (AVCodecContext * context, AVFrame * picture)
{
  ...
  if (picture->opaque == NULL) {
    ...
    g_demuxer.avcodec_default_release_buffer (context, picture);
    return;
  }
  ...
  *outbuf = NULL;
  ...
  if (ffmpegdec->picture->opaque != NULL) {
    ...
    *outbuf = (GstBuffer *) ffmpegdec->picture->opaque;
    ...
    g_demuxer.av_picture_copy (&pic, outpic, ffmpegdec->context->pix_fmt, width, height);
  }
    ...
    const GstTSInfo * dec_info, GstBuffer ** outbuf, GstFlowReturn * ret)
{
  ...
  *outbuf = NULL;
  ...
  len = g_demuxer.avcodec_decode_video (ffmpegdec->context,
  ...
  *ret = get_output_buffer (ffmpegdec, outbuf);
  ...
  /*{
    ...
    if( dump )
    {
      gchar *fn = g_strdup_printf("D:\\%d.yuv", counter++);

      int fd = open("d:\\raw.yuv", O_BINARY | O_WRONLY | O_CREAT | O_APPEND,_S_IREAD | _S_IWRIsnTE);
      if( fd >= 0 )
      {
        write(fd, (*outbuf)->data, (*outbuf)->size);
        close(fd);
      }
      g_free(fn);
    }
    ...
    const GstTSInfo * dec_info, GstBuffer ** outbuf, GstFlowReturn * ret)
{
  ...
  *outbuf =
  ...
  len = g_demuxer.avcodec_decode_audio2 (ffmpegdec->context,
...
gst_ffmpegdec_sink_event (GstPad * pad, GstEvent * event)
{
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    {
      if (ffmpegdec->opened) {
        g_demuxer.avcodec_flush_buffers (ffmpegdec->context);
      }
      gst_ffmpegdec_reset_ts (ffmpegdec);
    ...
    case GST_EVENT_NEWSEGMENT:
    {
      ...
      if (ffmpegdec->context->codec && !update)
        gst_ffmpegdec_drain (ffmpegdec);
...
gst_ffmpegdec_chain (GstPad * pad, GstBuffer * inbuf)
{
  ...
  preroll = GST_BUFFER_FLAG_IS_SET(inbuf, GST_BUFFER_FLAG_PREROLL);

  /* get handle to timestamp info, we can pass this around to ffmpeg */
  in_info = gst_ts_info_store (ffmpegdec, in_timestamp, in_duration, in_offset, preroll);
  ...
  do {
    ...
    if (ffmpegdec->pctx) {
       ...
       * info again later */
      res = g_demuxer.av_parser_parse (ffmpegdec->pctx, ffmpegdec->context,
...
gst_ffmpegdec_register (GstPlugin * plugin)
{
  ...
  in_plugin = g_demuxer.av_codec_next (NULL);
  ...
  while (in_plugin) {
    ...
    if (in_plugin->id != CODEC_ID_H264 || !g_str_has_prefix(in_plugin->name, "mc")){
      goto next;
    }
  ...
  next:
    in_plugin = g_demuxer.av_codec_next (in_plugin);
  }
