...
gst_ffmpeg_get_palette (const GstCaps * caps, AVCodecContext * context)
{
  ...
  if ((palette_v = gst_structure_get_value (str, "palette_data")) && context) {
    ...
    if (GST_BUFFER_SIZE (palette) >= AVPALETTE_SIZE) {
      if (context->palctrl)
        g_demuxer.av_free (context->palctrl);
      context->palctrl = g_demuxer.av_malloc (sizeof (AVPaletteControl));
    ...
    AVCodecContext * context, gboolean encode)
{
  ...
  if (buildcaps) {
    ...
    if ((codec =g_demuxer.avcodec_find_decoder (codec_id)) ||
        (codec = g_demuxer.avcodec_find_encoder (codec_id))) {
      gchar *mime = NULL;
    ...
    enum CodecType codec_type, const GstCaps * caps, AVCodecContext * context)
{
  ...
  if ((value = gst_structure_get_value (str, "codec_data"))) {
    ...
    if (context->extradata)
      g_demuxer.av_free (context->extradata);
    ...
    if (codec_id == CODEC_ID_H264) {
      ...
      context->extradata =
          g_demuxer.av_mallocz (GST_ROUND_UP_16 (size * 2 +
...
#endif
    {
      ...
      context->extradata =
          g_demuxer.av_mallocz (GST_ROUND_UP_16 (size + FF_INPUT_BUFFER_PADDING_SIZE));
    ...
    context->extradata =
        g_demuxer.av_mallocz (GST_ROUND_UP_16 (FF_INPUT_BUFFER_PADDING_SIZE));
      ...
      if (!context->extradata) {
        ...
        if (gst_structure_get_int (str, "halfpel_flag", &halfpel_flag) ||
            ...
            gst_structure_get_int (str, "unknown_svq3_flag",
                &unknown_svq3_flag)) {
          context->extradata = (guint8 *) g_demuxer.av_mallocz (0x64);
...
gst_ffmpeg_caps_to_codecid (const GstCaps * caps, AVCodecContext * context)
{
  ...
  if (!strcmp (mimetype, "video/x-raw-rgb") ||
    ...
    if (strlen (mimetype) <= 30 &&
      ...
      if ((codec = g_demuxer.avcodec_find_decoder_by_name (ext)) ||
          (codec = g_demuxer.avcodec_find_encoder_by_name (ext))) {
        id = codec->id;
        audio = TRUE;
      }
    }
