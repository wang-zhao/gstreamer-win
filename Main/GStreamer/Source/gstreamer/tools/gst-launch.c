...
sigint_setup(void) {
#ifdef HAVE_WIN32
    SetConsoleCtrlHandler((PHANDLER_ROUTINE)sigint_handler_sighandler, TRUE);
#else
    struct sigaction action;
...
#ifdef HAVE_WIN32
    SetConsoleCtrlHandler((PHANDLER_ROUTINE)sigint_handler_sighandler, FALSE);
#else
    struct sigaction action;
...
#define GST_DISABLE_OPTION_PARSING
