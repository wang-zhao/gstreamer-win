 ...
 * Copyright (C) 2008 Ole Andr?Vadla Ravnås <ole.andre.ravnas@tandberg.com>
 *
...
#else
   changed= gst_poll_update_winsock_event_mask (set, idx, FD_WRITE | FD_CONNECT,
        active);
#endif
   if( changed )
    MARK_REBUILD (set);
...
gst_poll_fd_ctl_read_unlocked (GstPoll * set, GstPollFD * fd, gboolean active)
{
  ...
  if (idx >= 0) {
    ...
    if (active)
      ...
      pfd->events &= ~(POLLIN | POLLPRI);
#else
  changed=  gst_poll_update_winsock_event_mask (set, idx, FD_READ | FD_ACCEPT, active);
#endif
  if( changed)
    MARK_REBUILD (set);
  } else {
...
static
void gst_poll_fd_ctl_status_unlocked(GstPoll* set, GstPollFD* fd, guint32 state, gboolean active){
  gint idx;
  ...
  if (idx >= 0) {
    ...
    if( active )
      wfd->events.lNetworkEvents |= state;
    else 
      wfd->events.lNetworkEvents &= ~state;
#else
    struct pollfd *pfd = &g_array_index (set->active_fds, struct pollfd, idx);
...
void gst_poll_fd_ctl_status(GstPoll* set, GstPollFD* fd, guint32 state, gboolean active)
{
  g_return_if_fail (set != NULL);
  g_return_if_fail (fd != NULL);
  g_return_if_fail (fd->fd >= 0);
  ...
  gst_poll_fd_ctl_status_unlocked (set, fd, state, active);
...
gboolean        gst_poll_fd_is_active     (GstPoll *set, GstPollFD* fd)
{
 return find_index (set->active_fds, fd) >=0 ;
}
