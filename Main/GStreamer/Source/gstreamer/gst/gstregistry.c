...
gst_registry_init (GstRegistry * registry)
{
 ...
 //如果是Debug模式，设置GST_PLUGIN_PATH为调试输出目录，方便调试
#ifdef _DEBUG
  {
    gchar* thisfile =  __FILE__;
    gchar** dirs = g_strsplit(thisfile, "\\", 256);
    GString* plug_dir = g_string_new("GST_PLUGIN_PATH=");
    ...
    for( i = 0 ; i < dir_num; i ++ )
    {
      g_string_append(plug_dir, dirs[i]);
      g_string_append(plug_dir, "\\");
    }

    g_string_append(plug_dir, "Build\\Windows\\Win32\\Debug\\bin\\plugins\\");
