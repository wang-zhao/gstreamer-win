...
done_update_cache:
    ...
    if (caps && trans->priv->suggest_pending && trans->priv->sink_suggest) {

        GstCaps* suggest_caps = gst_caps_intersect_full(trans->priv->sink_suggest, caps,
...
gst_base_transform_find_transform(GstBaseTransform* trans, GstPad* pad,
    ...
    if (!is_fixed && otherpeer) {
        ...
        if (gst_caps_is_empty(intersect)) {
            gchar* s1 = gst_caps_to_string(peercaps);
            gchar* s2 = gst_caps_to_string(othercaps);
