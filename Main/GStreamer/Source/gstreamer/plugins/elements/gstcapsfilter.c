...
static int gst_capsfilter_signals[SIGNAL_LAST] = {0};
...
static gboolean gst_capsfilter_set_caps(GstBaseTransform* trans, GstCaps* in_caps, GstCaps* out_caps);
...
gst_capsfilter_class_init (GstCapsFilterClass * klass)
{
  ...
  gst_capsfilter_signals[SIGNAL_CAPS_SETTED] =
      g_signal_new ("caps_setted", G_TYPE_FROM_CLASS (klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET (GstCapsFilterClass, caps_setted), NULL, NULL,
      gst_marshal_VOID__BOXED, G_TYPE_NONE, 1, GST_TYPE_CAPS);

  trans_class->set_caps = GST_DEBUG_FUNCPTR(gst_capsfilter_set_caps);
}

static gboolean gst_capsfilter_set_caps(GstBaseTransform* trans, GstCaps* in_caps, GstCaps* out_caps) {

    g_signal_emit (trans, gst_capsfilter_signals[SIGNAL_CAPS_SETTED], 0, in_caps);
    return TRUE;
}
