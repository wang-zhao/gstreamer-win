...
gst_win32_get_monitor_rect(UINT index) {
    ...
    data = (LPRECT) malloc(sizeof(RECT) * num * 2);

    if (data) {
        LPRECT tmp = data;
        ZeroMemory(data, sizeof(RECT) * num * 2);
...
RECT gst_win32_window_rect(guint hwnd) {
    RECT ret_rect;


    if (GetWindowRect((HWND)hwnd, &ret_rect)) {
        OffsetRect(&ret_rect, -ret_rect.left, -ret_rect.top);
    } else {
        ZeroMemory(&ret_rect, sizeof(RECT));
