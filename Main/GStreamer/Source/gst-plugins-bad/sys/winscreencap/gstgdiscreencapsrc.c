...
gst_gdiscreencapsrc_class_init(GstGDIScreenCapSrcClass* klass) {
    ...
    g_object_class_install_property(go_class, PROP_WINDOW,
                                    g_param_spec_uint("window", "capture specific window",
                                            "capture the given window", 0, G_MAXUINT, 0, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
gst_gdiscreencapsrc_set_property(GObject* object, guint prop_id,
    ...
    case PROP_WINDOW:
        src->window = g_value_get_uint(value);
        break;
...
gst_gdiscreencapsrc_get_property(GObject* object, guint prop_id,
    ...
    case PROP_WINDOW:
        g_value_set_uint(value, src->window);
        break;
...
gst_gdiscreencapsrc_get_caps(GstBaseSrc* bsrc) {
    ...
    if (src->window) {
        src->screen_rect = rect_dst = gst_win32_window_rect(src->window);
    } else {
...
gst_gdiscreencapsrc_create(GstPushSrc* push_src, GstBuffer** buf) {
    ...
    if (src->window) {
        screen_rect = gst_win32_window_rect(src->window);
    } else {
        screen_rect = gst_win32_get_monitor_rect(src->monitor);
    }

    caps = NULL;

    if (memcmp(&screen_rect, &src->screen_rect, sizeof(RECT)) != 0) {
        gst_pad_set_caps(GST_BASE_SRC_PAD(push_src), caps = gst_gdiscreencapsrc_get_caps(GST_BASE_SRC(src)));
    }

    if (G_UNLIKELY(!src->info.bmiHeader.biWidth ||
                   !src->info.bmiHeader.biHeight || !src->hBitmap || !src->dibMem)) {
    ...
    res =
        gst_pad_alloc_buffer_and_set_caps(GST_BASE_SRC_PAD(src),
                                          ...
                                          caps ? caps : GST_PAD_CAPS(GST_BASE_SRC_PAD(push_src)), &new_buf);
...
gst_gdiscreencapsrc_screen_capture(GstGDIScreenCapSrc* src, GstBuffer* buf) {
    ...
    /* Capture screen */
    capture = (src->window) ? (HWND)src->window : GetDesktopWindow();
    ...
    if (width == src->src_rect.right - src->src_rect.left &&
    ...
    if (src->show_cursor) {
        CURSORINFO ci;
        RECT src_rect;
        src_rect = src->src_rect;

        if (src->window) {
            ClientToScreen(capture, (LPPOINT)&src_rect);
            ClientToScreen(capture, (LPPOINT)&src_rect + 1);
        }
        ...
        if (ci.flags & CURSOR_SHOWING) {
            ...
            if (ci.ptScreenPos.x > src_rect.left && ci.ptScreenPos.x < src_rect.right &&
                    ci.ptScreenPos.y > src_rect.top && ci.ptScreenPos.y < src_rect.bottom) {
