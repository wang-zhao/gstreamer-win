...
- (GstCaps *)getCaps
{
  ...
  if (captureScreen) {
    ...
    for (NSNumber *pixel_format in pixel_formats) {
      GstVideoFormat gst_format = [self getGstVideoFormat:pixel_format];
      if (gst_format != GST_VIDEO_FORMAT_UNKNOWN)
        gst_caps_append (result, gst_caps_new_simple ("video/x-raw-yuv",
            "width", G_TYPE_INT, (int)rect.size.width,
            "height", G_TYPE_INT, (int)rect.size.height,
            "format", GST_TYPE_FOURCC, gst_video_format_to_fourcc (gst_format),
            NULL));
    }
#else
...
//  gstbasesrc_class->decide_allocation = gst_avf_video_src_decide_allocation;
  ...
  g_object_class_install_property (gobject_class, PROP_CAPTURE_SCREEN,
      g_param_spec_boolean ("capture-screen", "Enable screen capture",
          "Enable screen capture functionality", TRUE,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property (gobject_class, PROP_CAPTURE_SCREEN_CURSOR,
      g_param_spec_boolean ("capture-screen-cursor", "Capture screen cursor",
          "Enable cursor capture while capturing screen", TRUE,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property (gobject_class, PROP_CURSOR,
      g_param_spec_boolean ("cursor", "Capture screen cursor",
          "Enable cursor capture while capturing screen", TRUE,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property (gobject_class, PROP_MONITOR_ID,
      g_param_spec_int ("monitor", "Monitor Id will be Captured",
          "The Zero-based monitor id", 0, 16, 0,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    ...
    GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_MONITOR_ID:
      g_value_set_int (value, impl.monitorIdx);
      break;
  ...
  switch (prop_id) {
    ...
    case PROP_MONITOR_ID:
      ...
      if (idx >=0 && idx < [ar count]) {
          scr = [ar objectAtIndex:idx];
          screenInfo = [scr deviceDescription];
          screenId = [screenInfo objectForKey:@"NSScreenNumber"];
          impl.displayId = [screenId longValue];
          impl.monitorIdx = idx;
      } else {
          impl.displayId = kCGDirectMainDisplay;
          impl.monitorIdx = 0;
      }
