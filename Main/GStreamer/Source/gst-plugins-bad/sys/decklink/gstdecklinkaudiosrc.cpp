...
static GstStaticPadTemplate sink_template = GST_STATIC_PAD_TEMPLATE ("src",
    ...
    GST_PAD_ALWAYS,
    GST_STATIC_CAPS
    ("audio/x-raw-int, width = (int) { 16, 32 }, depth = (int) { 16, 32 }, endianness = (int) LITTLE_ENDIAN, channels = (int) 2, rate = (int) 48000, signed = (boolean) true")
    );
...
gst_decklink_audio_src_class_init (GstDecklinkAudioSrcClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_AUDIO_INPUT,
    g_param_spec_enum ("audio-input", "audio channel",
    "audio input channel", GST_TYPE_DECKLINK_AUDIO_CONNECTION, GST_DECKLINK_AUDIO_CONNECTION_AUTO,
    (GParamFlags) (G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS |
    G_PARAM_CONSTRUCT)));
  ...
  gst_element_class_set_details_simple (element_class, "Decklink Audio Source",
...
gst_decklink_audio_src_init (GstDecklinkAudioSrc * self)
{
  ...
  self->audio_input = GST_DECKLINK_AUDIO_CONNECTION_AUTO;
...
void
gst_decklink_audio_src_set_provide_clock (GstDecklinkAudioSrc * src,
                                          gboolean provide)
{
    g_return_if_fail (GST_IS_DECKLINK_AUDIO_SRC (src));
...
gboolean
gst_decklink_audio_src_get_provide_clock (GstDecklinkAudioSrc * src)
{
    gboolean result;

    g_return_val_if_fail (GST_IS_DECKLINK_AUDIO_SRC (src), FALSE);
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (property_id) {
    ...
    case PROP_AUDIO_INPUT:
      self->audio_input =(GstDecklinkAudioConnectionEnum) g_value_get_enum(value);
      break;
    ...
    case PROP_PROVIDE_CLOCK:
      gst_decklink_audio_src_set_provide_clock (self, g_value_get_boolean (value));
      break;
    default:
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (property_id) {
    ...
    case PROP_ALIGNMENT_THRESHOLD:
      ...
      break;
    case PROP_AUDIO_INPUT:
      g_value_set_enum(value, self->audio_input);
      break;
    case PROP_DISCONT_WAIT:
    ...
    case PROP_BUFFER_SIZE:
      ...
      break;
    case PROP_PROVIDE_CLOCK:
      g_value_set_boolean (value, gst_decklink_audio_src_get_provide_clock (self));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, property_id, pspec);
      break;
...
gst_decklink_audio_src_set_caps (GstBaseSrc * bsrc, GstCaps * caps)
{
  ...
  if ((current_caps = gst_pad_get_caps (GST_BASE_SRC_PAD (bsrc)))) {
    GST_DEBUG_OBJECT (self, "Pad already has caps %" GST_PTR_FORMAT, caps);

	if(!gst_caps_is_subset (caps, current_caps)){
      GST_ERROR_OBJECT (self, "New caps are not equal to or subset of old caps");
...
gst_decklink_audio_src_get_caps (GstBaseSrc * bsrc/*, GstCaps * filter*/)
{
  ...
  if (!result)
    result = const_cast<GstCaps*>(gst_pad_get_pad_template_caps (GST_BASE_SRC_PAD (bsrc)));
...
gst_decklink_audio_src_create (GstPushSrc * bsrc, GstBuffer ** buffer)
{
  ...
  *buffer = gst_buffer_new();
  GST_BUFFER_MALLOCDATA(*buffer) = (guint8*)(ap);
  GST_BUFFER_FREE_FUNC(*buffer) = audio_packet_free;
...
gst_decklink_audio_src_open (GstDecklinkAudioSrc * self)
{
  ...
  if( self->input->config && self->audio_input != GST_DECKLINK_AUDIO_CONNECTION_AUTO)
    self->input->config->SetInt(bmdDeckLinkConfigAudioInputConnection, gst_decklink_get_audio_connection(self->audio_input));
