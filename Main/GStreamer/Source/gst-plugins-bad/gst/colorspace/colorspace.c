...
getline_RGB(ColorspaceConvert* convert, guint8* dest, const guint8* src,
    ...
    for (i = 0; i < convert->width; i++) {
        *((guint32*)&dest[i * 4]) = 0xff | (*((guint32*)&srcline[i * 3]) << 8);
    }
}
...
getline_BGR(ColorspaceConvert* convert, guint8* dest, const guint8* src,
    ...
    for (i = 0; i < convert->width; i++) {
        *((guint32*)&dest[i * 4]) = 0xff | ((guint32)srcline[i * 3 + 2] << 8) |
                                    ((guint32)srcline[i * 3 + 1] << 16) | ((guint32)srcline[i * 3] << 24);
    }
}
...
static const ColorspaceLine lines[] = {
    ...
    {GST_VIDEO_FORMAT_GRAY8, getline_Y800, putline_Y800},
    {GST_VIDEO_FORMAT_GRAY16_LE, getline_Y16, putline_Y16},
    ...
    , {GST_VIDEO_FORMAT_RGB8P, getline_RGB8P, putline_RGB8P},
