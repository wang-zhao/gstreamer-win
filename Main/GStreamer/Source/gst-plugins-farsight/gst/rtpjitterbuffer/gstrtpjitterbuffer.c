 ...
 * </para>
 * <para>
 * This element also provides the application the ability to query run-time
 * statistics using the custom query, "jitterbuffer-statistics" on it's sinkpad.
 * On receiving the query, the jitterbuffer will fill the sturcture inside the
 ...
 * <tbody>
 * <row>
 * <entry>total-packets</entry>
 * <entry>G_TYPE_UINT64</entry>
 * <entry>Total number of packets received by the jitterbuffer.
 * </entry>
 * </row>
 * <row>
 * <entry>late-packets</entry>
 * <entry>G_TYPE_UINT64</entry>
 * <entry>Total number of late packets received by the jitterbuffer.
 * </entry>
 * </row>
 * <row>
 * <entry>duplicate-packets</entry>
 * <entry>G_TYPE_UINT64</entry>
 * <entry>Total number of duplicate packets received by the jitterbuffer.
 * </entry>
 * </row>
 * <row>
 * <entry>times-overrun</entry>
 * <entry>G_TYPE_UINT64</entry>
 * <entry>The number of times the jitterbuffer queue was overrun: A packet was
 * received when the queue was full.
 * </entry>
 * </row>
 * <row>
 * <entry>times-underrun</entry>
 * <entry>G_TYPE_UINT64</entry>
 * <entry>The number of times the jitterbuffer queue was underrun: The
 * dispatcher thread tried to pop a packet from the queue while it was empty.
 * </entry>
 * </row>
 * <row>
 * <entry>fill-level</entry>
 * <entry>G_TYPE_UINT</entry>
 * <entry>The current fill-level of the jitterbuffer as percentage.
 * </entry>
 * </row>
 * </tbody>
 * </tgroup>
...
#include "gstrtpjitterbuffer-marshal.h"
...
static GstQueryType stats_query_type = GST_QUERY_NONE;
...
#define DEFAULT_LATENCY_MS      500
#define DEFAULT_DROP_ON_LATENCY FALSE

#define GST_QUERY_JB_STATS      "jitterbuffer-statistics"
#define GST_QUERY_JB_STATS_DESC "Query for jitterbuffer statistics"

/* Any seqnum jump larger than this value is invalid */
#define MAX_SEQNUM_JUMP                 100
/* The JB will be reset after this number of consecutive invalid seqnums */
#define MIN_VALID_PACKETS_FOR_RESET     5
...
struct _GstRTPJitterBufferPrivate
{
  ...
  /* for resync after an seqnum jump */
  /* seqnum of last valid packet received */
  guint16 last_valid_seqnum;
  /* if there was a seqnum jump, this is the next seqnum to expect */
  guint16 next_seqnum_after_jump;
  /* this is the count of sequential seqnums received */
  guint32 valid_packets_after_jump;
...
static gboolean gst_rtp_jitter_buffer_src_query (GstPad * pad, GstQuery * query);
static gboolean gst_rtp_jitter_buffer_sink_query (GstPad * pad, GstQuery * query);
...
gst_rtp_jitter_buffer_base_init (gpointer klass)
{
  ...
  if (!stats_query_type)
    stats_query_type = gst_query_type_register (GST_QUERY_JB_STATS,
                    GST_QUERY_JB_STATS_DESC);
}
...
gst_rtp_jitter_buffer_class_init (GstRTPJitterBufferClass * klass)
{
  ...
  /**
   * GstRTPJitterBuffer::hold:
   * 
   * Hold the call by pausing the streaming task
   */
  ...
  g_object_class_install_property (gobject_class, PROP_HIGH_THRESHOLD,
      g_param_spec_float ("high_threshold", "The high threshold",
          "Tells the jitterbuffer when to stop buffering", 0, 1,
          HIGH_THRESHOLD, G_PARAM_READWRITE));

  g_object_class_install_property (gobject_class, PROP_LOW_THRESHOLD,
      g_param_spec_float ("low_threshold", "The low threshold",
          "Tells the jitterbuffer when to start buffering", 0, 1,
          LOW_THRESHOLD, G_PARAM_READWRITE));
  /**
  ...
  gst_rtp_jitter_buffer_signals[SIGNAL_REQUEST_PT_MAP] =
      ...
      G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET (GstRTPJitterBufferClass,
          request_pt_map), NULL, NULL, gstrtpjitterbuffer_marshal_BOXED__UINT,
...
gst_rtp_jitter_buffer_chain (GstPad * pad, GstBuffer * buffer)
{
  ...
  /* Lets see if there is any sequence number re-setting done */
  /* First, we check if there was a valid seqnum
   * Then we compare the last valid seqnum with that of the current packet
   * and we check if the "jump" is larger than the maximum accepted jump
   * but we also check if its not a roll-over
   */
  if (priv->last_valid_seqnum &&
      abs (priv->last_valid_seqnum - seqnum) > MAX_SEQNUM_JUMP &&
      abs (priv->last_valid_seqnum - seqnum) < G_MAXUINT16 - MAX_SEQNUM_JUMP)
  {
    if (seqnum == priv->next_seqnum_after_jump)
    {
      priv->next_seqnum_after_jump++;
      priv->valid_packets_after_jump++;
      if (priv->valid_packets_after_jump > MIN_VALID_PACKETS_FOR_RESET)
      {
        /* We had enough valid seqnums in the new streams, lets flush the
         * JB and start anew
         */
        GST_DEBUG_OBJECT (jitterbuffer,
                          "Flushing queue, resetting sequence numbers");
        priv->last_popped_seqnum = -1;
        priv->next_seqnum = -1;
        priv->next_seqnum_after_jump = 0;
        priv->valid_packets_after_jump = 0;
        async_jitter_queue_flush_unlocked (jitterbuffer->priv->queue,
                                           (GFunc) free_func, jitterbuffer);
      }
      else
      {
    ...
    {
      priv->next_seqnum_after_jump = seqnum + 1;
      priv->valid_packets_after_jump = 0;
      goto invalid_seqnum;
    }
  }
  ...
  if (priv->drop_on_latency) {
    if (async_jitter_queue_length_ts_units_unlocked (priv->queue) >=
      ...
      latency_ts_units = priv->latency_ms * priv->clock_rate / 1000;
      while (async_jitter_queue_length_ts_units_unlocked (priv->queue) >=
          latency_ts_units / 2)
      {
        GstBuffer *old_buf;
        GST_DEBUG_OBJECT (jitterbuffer, "dropping buffer down to 0.5 %u %u", latency_ts_units,
            async_jitter_queue_length_ts_units_unlocked (priv->queue));

        old_buf = async_jitter_queue_pop_unlocked_forced (priv->queue);
        if (old_buf)
...
invalid_seqnum:
  {
    GST_DEBUG_OBJECT (jitterbuffer, "Received packet with invalid seqnum %u,"
                      " last was %u", seqnum, priv->last_valid_seqnum);
    priv->num_late++;
    gst_buffer_unref (buffer);
    goto finished;
  }
...
pause:
  {
    const gchar *reason;
    reason = gst_flow_get_name (result);
...
gst_rtp_jitter_buffer_src_query (GstPad * pad, GstQuery * query)
{
  ...
  if (stats_query_type == GST_QUERY_TYPE (query)) {
    return gst_rtp_jitter_buffer_sink_query (pad, query);
  }
...
/* Returns the fill-level of the queue in percentage */
static guint
gst_rtp_jitter_buffer_get_fill_level (GstRTPJitterBuffer *jitterbuffer)
{
  GstRTPJitterBufferPrivate *priv;
  guint queue_latency;
  guint ret = 0;
  ...
  queue_latency = priv->latency_ms * priv->clock_rate / 1000;
  if (queue_latency != 0) {
    guint queue_length;

    async_jitter_queue_lock (priv->queue);
    queue_length = async_jitter_queue_length_ts_units_unlocked (priv->queue);
    async_jitter_queue_unlock (priv->queue);

    ret = queue_length * 100 / queue_latency;
  }
...
gst_rtp_jitter_buffer_sink_query (GstPad * pad, GstQuery * query)
{
  ...
  if (GST_QUERY_TYPE (query) == stats_query_type) {
    GstStructure *structure;
    guint64 fill_level, underruns;

    fill_level = gst_rtp_jitter_buffer_get_fill_level (jitterbuffer);
    underruns = async_jitter_queue_get_underruns (priv->queue);
    structure = gst_query_get_structure (query);
    gst_structure_set (structure,
                    "total-packets", G_TYPE_UINT64, priv->num_packets,
                    "late-packets", G_TYPE_UINT64, priv->num_late,
                    "duplicate-packets", G_TYPE_UINT64, priv->num_duplicates,
                    "times-overrun", G_TYPE_UINT64, priv->times_overrun,
                    "times-underrun", G_TYPE_UINT64, underruns,
                    "fill-level", G_TYPE_UINT, fill_level,
                    NULL);
    res = TRUE;
  }
    ...
    guint prop_id, const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_HIGH_THRESHOLD:
    {
      gfloat high_threshold = g_value_get_float (value);
      if (high_threshold >= 0 && high_threshold <= 1)
      {
        async_jitter_queue_set_high_threshold (jitterbuffer->priv->queue,
            high_threshold);
      }
      break;
    }
    case PROP_LOW_THRESHOLD:
    {
      gfloat low_threshold = g_value_get_float (value);
      if (low_threshold >= 0 && low_threshold <= 1)
      {
        async_jitter_queue_set_low_threshold (jitterbuffer->priv->queue,
            low_threshold);
      }
      break;
    }
    ...
    case PROP_HOLD:
    {
      gboolean old_value = jitterbuffer->priv->on_hold;
      jitterbuffer->priv->on_hold = g_value_get_boolean (value);
      if (old_value != jitterbuffer->priv->on_hold) {
        if (jitterbuffer->priv->on_hold) {
          async_jitter_queue_lock (jitterbuffer->priv->queue);
          GST_DEBUG_OBJECT (jitterbuffer, "Holding call : Pausing task on srcpad");
          async_jitter_queue_set_flushing_unlocked (jitterbuffer->priv->queue,
              (GFunc) free_func, jitterbuffer);
          async_jitter_queue_unlock (jitterbuffer->priv->queue);

          gst_pad_pause_task (jitterbuffer->priv->srcpad);
        } else {
          async_jitter_queue_lock (jitterbuffer->priv->queue);
          GST_DEBUG_OBJECT (jitterbuffer, "Unholding call : Starting task on srcpad");
          async_jitter_queue_unset_flushing_unlocked (jitterbuffer->priv->queue);
          async_jitter_queue_unlock (jitterbuffer->priv->queue);
    ...
    guint prop_id, GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    case PROP_LATENCY:
      ...
      break;
    case PROP_HIGH_THRESHOLD:
      g_value_set_float (value, async_jitter_queue_get_high_threshold
          (jitterbuffer->priv->queue));
      break;
    case PROP_LOW_THRESHOLD:
      g_value_set_float (value, async_jitter_queue_get_low_threshold
          (jitterbuffer->priv->queue));
      break;
    case PROP_DROP_ON_LATENCY:
      ...
      break;
    case PROP_HOLD:
      g_value_set_boolean (value, jitterbuffer->priv->on_hold);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
gboolean
gst_rtp_jitter_buffer_plugin_init (GstPlugin * plugin)
{
  return gst_element_register (plugin, "rtpjitterbuffer",
      GST_RANK_MARGINAL, GST_TYPE_RTP_JITTER_BUFFER);
}
...
GST_PLUGIN_DEFINE (GST_VERSION_MAJOR,
    ...
    "rtpjitterbuffer",
    "RTP Jitter Buffer",
    gst_rtp_jitter_buffer_plugin_init, VERSION, "LGPL", "Farsight",
