...
guint32
async_jitter_queue_get_underruns (AsyncJitterQueue *queue)
{
...
gfloat
async_jitter_queue_get_high_threshold (AsyncJitterQueue *queue)
{
  g_return_val_if_fail (queue, 0);
  g_return_val_if_fail (g_atomic_int_get (&queue->ref_count) > 0, 0);

  return queue->high_threshold;
}

gfloat
async_jitter_queue_get_low_threshold (AsyncJitterQueue *queue)
{
  g_return_val_if_fail (queue, 0);
  g_return_val_if_fail (g_atomic_int_get (&queue->ref_count) > 0, 0);

  return queue->low_threshold;
}
...
async_jitter_queue_length_ts_units_unlocked (AsyncJitterQueue * queue)
{
  ...
  /* FIXME we should be able to return the tail_buffer_duration if we have only
   * one packet left as long as we can detect discontinuities in audio using the
   * RTP marker bit */
    ...
    gboolean ignore_flushing)
{
  ...
  if (queue->pop_flushing && !ignore_flushing)
    return NULL;

  /* if pops are remaining, do it now without blocking */
  if (queue->pops_remaining == 0)
  {
    /* let's check if we are under our low threshold */
    if (async_jitter_queue_length_ts_units_unlocked (queue) <=
        queue->low_threshold * queue->max_queue_length)
    {
      if (!queue->buffering)
      {
        //g_debug ("Turned on buffering");
        queue->buffering = TRUE;
        /* this will make sure we release all remaining buffers from the queue
         * before starting to buffer */
        queue->pops_remaining = queue->queue->length;
      }
      else
      {
        /* Let's wait until the high threshold is reached */
        while (!g_queue_peek_tail (queue->queue) || queue->pop_blocking) {
          //g_debug ("Sink needs buffer, must wait for buffering to stop");
          queue->underruns++;
    ...
    /* Let's see if there is a buffer, if not, wait for signal */
    else
    {
      while (queue->pop_blocking && !g_queue_peek_tail (queue->queue)) {
        //g_debug ("No packets available for sink! waiting");
        queue->waiting_threads++;
        queue->underruns++;
        g_cond_wait(queue->cond, queue->mutex);
        queue->waiting_threads--;
  ...
  /* Let's calculate the duration of the next buffer to be popped based on the
   * buffer we just popped */
  ...
  if (tail_buffer) {
    if (!GST_IS_BUFFER (tail_buffer) || !GST_IS_BUFFER (retval)) {
      ...
      queue->tail_buffer_duration = calculate_ts_diff (
          gst_rtp_buffer_get_timestamp (tail_buffer),
          gst_rtp_buffer_get_timestamp (retval));
      /* TODO */
      /* double check on the commented code. check the RTP specs to make sure
       * the market bit always means an audio discontinuity, if so, we can't
       * calculate the tail buffer duration even if the seq-num are subsequent
      if (gst_rtp_buffer_get_marker (tail_buffer))
      {
        queue->tail_buffer_duration = 0;
      }
      */
...
async_jitter_queue_pop (AsyncJitterQueue * queue)
{
  ...
  g_mutex_lock (queue->mutex);
  retval = async_jitter_queue_pop_intern_unlocked (queue, FALSE);
  g_mutex_unlock (queue->mutex);
...
async_jitter_queue_pop_unlocked (AsyncJitterQueue * queue)
{
  ...
  return async_jitter_queue_pop_intern_unlocked (queue, FALSE);
}

/**
 * async_jitter_queue_pop_unlocked_forced:
 * @queue: a #AsyncJitterQueue.
 *
 * Pops data from the @queue. This function blocks until data become
 * available. This function must be called while holding the @queue's
 * lock. It will return data even if the queue is flushing.
 *
 * Return value: data from the queue.
 **/
gpointer
async_jitter_queue_pop_unlocked_forced (AsyncJitterQueue * queue)
{
  g_return_val_if_fail (queue, NULL);
  g_return_val_if_fail (g_atomic_int_get (&queue->ref_count) > 0, NULL);

  return async_jitter_queue_pop_intern_unlocked (queue, TRUE);
}
...
/**
 * async_jitter_queue_flush_unlocked:
 * @queue: a #AsyncJitterQueue.
 * @free_func: a function to call to free the elements
 * @user_data: user data passed to @free_func
 *
 * This functions empties the queue, but does not touch the other threads,
 * its mostly used when the sequence numbers and timestamps are reset
 */

void
async_jitter_queue_flush_unlocked (AsyncJitterQueue * queue,
    GFunc free_func, gpointer user_data)
{
  gpointer elem;
