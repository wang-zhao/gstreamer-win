...
// --gst-debug=seek:5 15 "filesrc location=h:/AVIK/video/10.mpg ! typefind ! mpegpsdemux ! multiqueue ! mpegvideoparse ! mpeg2dec ! queue ! ffmpegcolorspace ! directdrawsink"
// 
...
static GtkListStore* video_combo_items,*audio_combo_items,*text_combo_items, *vis_combo_items;
...
make_mpeg_pipeline (const gchar * location)
{
 ...
 // demux = gst_element_factory_make_or_warn ("flupsdemux", "demux");
  ...
  audio_bin = gst_bin_new ("a_decoder_bin");
  a_decoder = gst_element_factory_make_or_warn ("ffdec_mp3", "a_dec");
  a_queue = gst_element_factory_make_or_warn ("queue", "a_queue");
  audiosink = gst_element_factory_make_or_warn (ASINK, "a_sink");
  gst_bin_add (GST_BIN (audio_bin), a_decoder);
  gst_bin_add (GST_BIN (audio_bin), a_queue);
  ...
  setup_dynamic_link (demux, "audio_00", gst_element_get_static_pad (audio_bin,
          "sink"), NULL);

  video_bin = gst_bin_new ("v_decoder_bin");
  v_decoder = gst_element_factory_make_or_warn ("ffdec_h264", "v_dec");
  v_queue = gst_element_factory_make_or_warn ("queue", "v_queue");
  ...
  setup_dynamic_link (demux, "video_00", gst_element_get_static_pad (video_bin,
          "sink"), NULL);
...
{
  GstElement *videosink = gst_element_factory_make_or_warn (VSINK, "v_sink");
...
do_seek (GtkWidget * widget)
{
  ...
  GST_DEBUG("value=%f, real=%d",gtk_range_get_value (GTK_RANGE (widget)),real);
  ...
  if (res) {
    if (flush_seek) {
      GST_DEBUG("gst_element_get_state(... SEEK_TIMEOUT)");
      gst_element_get_state (GST_ELEMENT (pipeline), NULL, NULL, SEEK_TIMEOUT);
    } else {
      GST_DEBUG("set_update_scale(TRUE)");
...
static void
go_end_cb (GtkButton * button, gpointer data)
{
  GstStateChangeReturn ret;
  g_static_mutex_lock (&state_mutex);
...
failed:
  {
    g_static_mutex_unlock (&state_mutex);
    g_print ("GO END: PAUSE failed\n");
    gtk_statusbar_push (GTK_STATUSBAR (statusbar), status_id, "Pause failed while go end");
  }
}

static void
dump_dot_cb (GtkButton * button, gpointer data)
{
//	char b[200];
	//g_setenv("GST_DEBUG_DUMP_DOT_DIR",(const gchar *)getcwd(b,200),TRUE);
	GST_DEBUG("GST_DEBUG_DUMP_DOT_DIR=%s",g_getenv("GST_DEBUG_DUMP_DOT_DIR"));
	GST_DEBUG("pipeline = 0x%p, data=0x%p",pipeline,data);

	GST_DEBUG_BIN_TO_DOT_FILE(GST_BIN(pipeline), GST_DEBUG_GRAPH_SHOW_ALL, "player");
}
...
clear_streams (GstElement * pipeline)
{
  ...
  for (i = 0; i < n_video; i++)
  {
	  gtk_list_store_clear(video_combo_items);
	 // gtk_combo_box_remove_text (GTK_COMBO_BOX (video_combo), 0);
  }
  for (i = 0; i < n_audio; i++)
  {
	  gtk_list_store_clear(audio_combo_items);
    //gtk_combo_box_remove_text (GTK_COMBO_BOX (audio_combo), 0);
  }
  for (i = 0; i < n_text; i++)
  {
	  gtk_list_store_clear(text_combo_items);
  ...
  if (pipeline_type == 16 && need_streams) {
    ...
    for (i = 0; i < n_video; i++) {
     ...
     // gtk_combo_box_append_text (GTK_COMBO_BOX (video_combo), name);
	  gtk_list_store_append(video_combo_items, &ite);
	  gtk_list_store_set(video_combo_items, &ite, 0, name, -1);
      g_free (name);
    }
    ...
    for (i = 0; i < n_audio; i++) {
	  ...
	  gtk_list_store_append(audio_combo_items, &ite);
	  gtk_list_store_set(audio_combo_items, &ite, 0, name, -1);
    ...
    for (i = 0; i < n_text; i++) {
	  ...
	  gtk_list_store_append(text_combo_items, &ite);
	  gtk_list_store_set(text_combo_items, &ite, 0, name, -1);
      g_free (name);
    }
...
init_visualization_features (void)
{
  ...
  for (walk = list; walk; walk = g_list_next (walk)) {
	...
	gtk_list_store_append(vis_combo_items, &ite);
	gtk_list_store_set(vis_combo_items, &ite, 0, name, -1);
  }
...
msg_state_changed (GstBus * bus, GstMessage * message, GstPipeline * pipeline)
{
  ...
  if (s && GST_MESSAGE_SRC (message) == GST_OBJECT_CAST (pipeline)) {
    ...
    GST_DEBUG("states: old %d, new %d, pending %d",old,new,pending);
    ...
    if (new == GST_STATE_PLAYING) {
      ...
      GST_DEBUG("set_update_scale (FALSE);");
      set_update_scale (FALSE);
    }
  }
}
...
bus_sync_handler (GstBus * bus, GstMessage * message, GstPipeline * data)
{
  if ((GST_MESSAGE_TYPE (message) == GST_MESSAGE_ELEMENT) &&
    ...
    g_print("prepared xwindow-id, return\n");
  }
  return GST_BUS_PASS;
}
//#endif
...
static gboolean
handle_draw_cb (GtkWidget *widget,
				  cairo_t   *cr,
				  gpointer   data)
{
  if (state < GST_STATE_PAUSED) {
...
//       gtk_draw_rectangle (window, style->black_gc, TRUE, 0, 0,
...
#ifdef _WIN32
	embed_xid = (gulong)GDK_WINDOW_HWND (window);
#else
...
main (int argc, char **argv)
{
  ...
  GtkWidget *play_button, *pause_button, *stop_button, *shot_button, *go_end_button, *dot_button;
  ...
  g_signal_connect (video_window, "draw",
  ...
  go_end_button = gtk_button_new_from_stock (GTK_STOCK_MEDIA_NEXT);
  dot_button = gtk_button_new_with_label("Dot");
  ...
  /* step expander */
  {
    ...
    format_combo = gtk_combo_box_new_with_model(GTK_TREE_MODEL(store));
	gtk_cell_layout_pack_start (GTK_CELL_LAYOUT (format_combo),
		cell, TRUE);
	gtk_cell_layout_set_attributes (GTK_CELL_LAYOUT (format_combo),
		cell, "text", 0, NULL);
	gtk_combo_box_set_wrap_width (GTK_COMBO_BOX (format_combo),
		3);
...
//  gtk_range_set_update_policy (GTK_RANGE (hscale), GTK_UPDATE_CONTINUOUS);
  ...
  if (pipeline_type == 16) {
	  ...
	  GtkCellRenderer* video_combo_cell, *audio_combo_cell, *text_combo_cell, * vis_combo_cell;
	  /* the playbin2 panel controls for the video/audio/subtitle tracks */
    panel = gtk_hbox_new (FALSE, 0);
	

	video_combo_cell = gtk_cell_renderer_text_new();
	audio_combo_cell = gtk_cell_renderer_text_new();
	text_combo_cell = gtk_cell_renderer_text_new();
	vis_combo_cell = gtk_cell_renderer_text_new();


	video_combo_items = gtk_list_store_new(1,G_TYPE_STRING);
	audio_combo_items = gtk_list_store_new(1, G_TYPE_STRING);
	text_combo_items = gtk_list_store_new(1, G_TYPE_STRING);
	vis_combo_items = gtk_list_store_new(1, G_TYPE_STRING);

    video_combo = gtk_combo_box_new_with_model(GTK_TREE_MODEL(video_combo_items));
    audio_combo = gtk_combo_box_new_with_model (GTK_TREE_MODEL(audio_combo_items));
    text_combo = gtk_combo_box_new_with_model (GTK_TREE_MODEL(text_combo_items));

	gtk_cell_layout_pack_start (GTK_CELL_LAYOUT (video_combo),
		video_combo_cell, TRUE);
	gtk_cell_layout_set_attributes (GTK_CELL_LAYOUT (video_combo),
		video_combo_cell, "text", 0, NULL);
	gtk_combo_box_set_wrap_width (GTK_COMBO_BOX (video_combo),
		3);

	gtk_cell_layout_pack_start (GTK_CELL_LAYOUT (audio_combo),
		audio_combo_cell, TRUE);
	gtk_cell_layout_set_attributes (GTK_CELL_LAYOUT (audio_combo),
		audio_combo_cell, "text", 0, NULL);
	gtk_combo_box_set_wrap_width (GTK_COMBO_BOX (audio_combo),
		3);

	gtk_cell_layout_pack_start (GTK_CELL_LAYOUT (text_combo),
		text_combo_cell, TRUE);
	gtk_cell_layout_set_attributes (GTK_CELL_LAYOUT (text_combo),
		text_combo_cell, "text", 0, NULL);
	gtk_combo_box_set_wrap_width (GTK_COMBO_BOX (text_combo),
		3);
    ...
    g_signal_connect (G_OBJECT (shot_button), "clicked", G_CALLBACK (shot_cb),
        pipeline);
    vis_combo = gtk_combo_box_new_with_model (GTK_TREE_MODEL(vis_combo_items));


	gtk_cell_layout_pack_start (GTK_CELL_LAYOUT (vis_combo),
		vis_combo_cell, TRUE);
	gtk_cell_layout_set_attributes (GTK_CELL_LAYOUT (vis_combo),
		vis_combo_cell, "text", 0, NULL);
	gtk_combo_box_set_wrap_width (GTK_COMBO_BOX (vis_combo),
		3);
  ...
  gtk_box_pack_start (GTK_BOX (hbox), go_end_button, FALSE, FALSE, 2);
  gtk_box_pack_start (GTK_BOX (hbox), dot_button, FALSE, FALSE, 2);
  ...
  g_signal_connect (G_OBJECT (stop_button), "clicked", G_CALLBACK (stop_cb),
      pipeline);
  g_signal_connect (G_OBJECT (go_end_button), "clicked", G_CALLBACK (go_end_cb),
      pipeline);
  g_signal_connect (G_OBJECT (dot_button), "clicked", G_CALLBACK (dump_dot_cb),
      pipeline);
