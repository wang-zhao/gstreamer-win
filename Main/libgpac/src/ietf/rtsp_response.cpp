...
GF_RTSPTransport *gf_rtsp_transport_clone(GF_RTSPTransport *original)
{
	...
	if (original->destination) tr->destination = _strdup(original->destination);
	if (original->source) tr->source = _strdup(original->source);
	if (original->Profile) tr->Profile = _strdup(original->Profile);
	return tr;
}
...
void gf_rtsp_set_response_value(GF_RTSPResponse *rsp, char *Header, char *Value)
{
	...
	if (!_stricmp(Header, "Accept")) rsp->Accept = _strdup(Value);
	else if (!_stricmp(Header, "Accept-Encoding")) rsp->Accept_Encoding = _strdup(Value);
	else if (!_stricmp(Header, "Accept-Language")) rsp->Accept_Language = _strdup(Value);
	else if (!_stricmp(Header, "Allow")) rsp->Allow = _strdup(Value);
	else if (!_stricmp(Header, "Authorization")) rsp->Authorization = _strdup(Value);
	else if (!_stricmp(Header, "Bandwidth")) sscanf(Value, "%d", &rsp->Bandwidth);
	else if (!_stricmp(Header, "Blocksize")) sscanf(Value, "%d", &rsp->Blocksize);
	else if (!_stricmp(Header, "Cache-Control")) rsp->Cache_Control = _strdup(Value);
	else if (!_stricmp(Header, "Conference")) rsp->Conference = _strdup(Value);
	else if (!_stricmp(Header, "Connection")) rsp->Connection = _strdup(Value);
	else if (!_stricmp(Header, "Content-Base")) rsp->Content_Base = _strdup(Value);	
	else if (!_stricmp(Header, "Content-Encoding")) rsp->Content_Encoding = _strdup(Value);	
	else if (!_stricmp(Header, "Content-Length")) sscanf(Value, "%d", &rsp->Content_Length);
	else if (!_stricmp(Header, "Content-Language")) rsp->Content_Language = _strdup(Value);	
	else if (!_stricmp(Header, "Content-Location")) rsp->Content_Location = _strdup(Value);	
	else if (!_stricmp(Header, "Content-Type")) rsp->Content_Type = _strdup(Value);	
	else if (!_stricmp(Header, "CSeq")) sscanf(Value, "%d", &rsp->CSeq);
	else if (!_stricmp(Header, "Date")) rsp->Date = _strdup(Value);	
	else if (!_stricmp(Header, "Expires")) rsp->Expires = _strdup(Value);	
	else if (!_stricmp(Header, "From")) rsp->From = _strdup(Value);	
	else if (!_stricmp(Header, "Host")) rsp->Host = _strdup(Value);	
	else if (!_stricmp(Header, "If-Match")) rsp->If_Match = _strdup(Value);	
	else if (!_stricmp(Header, "If-Modified-Since")) rsp->If_Modified_Since = _strdup(Value);	
	else if (!_stricmp(Header, "Last-Modified")) rsp->Last_Modified = _strdup(Value);	
	else if (!_stricmp(Header, "Location")) rsp->Location = _strdup(Value);	
	else if (!_stricmp(Header, "Proxy-Authenticate")) rsp->Proxy_Authenticate = _strdup(Value);	
	else if (!_stricmp(Header, "Proxy-Require")) rsp->Proxy_Require = _strdup(Value);	
	else if (!_stricmp(Header, "Public")) rsp->Public = _strdup(Value);	
	else if (!_stricmp(Header, "Referer")) rsp->Referer = _strdup(Value);	
	else if (!_stricmp(Header, "Require")) rsp->Require = _strdup(Value);	
	else if (!_stricmp(Header, "Retry-After")) rsp->Retry_After = _strdup(Value);	
	else if (!_stricmp(Header, "Scale")) sscanf(Value, "%lf", &rsp->Scale);
	else if (!_stricmp(Header, "Server")) rsp->Server = _strdup(Value);	
	else if (!_stricmp(Header, "Speed")) sscanf(Value, "%lf", &rsp->Speed);
	else if (!_stricmp(Header, "Timestamp")) rsp->Timestamp = _strdup(Value);	
	else if (!_stricmp(Header, "Unsupported")) rsp->Unsupported = _strdup(Value);	
	else if (!_stricmp(Header, "User-Agent")) rsp->User_Agent = _strdup(Value);	
	else if (!_stricmp(Header, "Vary")) rsp->Vary = _strdup(Value);	
	else if (!_stricmp(Header, "Via")) rsp->Vary = _strdup(Value);	
	else if (!_stricmp(Header, "WWW_Authenticate")) rsp->Vary = _strdup(Value);	
	else if (!_stricmp(Header, "Transport")) {
		LinePos = 0;
		while (1) {
	...
	//Session
	else if (!_stricmp(Header, "Session")) {
		LinePos = gf_token_get(Value, 0, ";\r\n", LineBuffer, 400);
		rsp->Session = _strdup(LineBuffer);
		//get timeout if any
		if (Value[LinePos] == ';') {
			LinePos += 1;
			LinePos = gf_token_get(Value, LinePos, ";\r\n", LineBuffer, 400);
	...
	//Range
	else if (!_stricmp(Header, "Range")) rsp->Range = gf_rtsp_range_parse(Value);
	//RTP-Info
	else if (!_stricmp(Header, "RTP-Info")) {
		LinePos = 0;
		while (1) {
			LinePos = gf_token_get(Value, LinePos, ",\r\n", LineBuffer, 400);
			if (LinePos <= 0) return;
			...
			while (1) {	
				...
				if (!_stricmp(param_name, "url")) info->url = _strdup(param_val);
				else if (!_stricmp(param_name, "seq")) sscanf(param_val, "%d", &info->seq);
				else if (!_stricmp(param_name, "rtptime")) {
					sscanf(param_val, "%i", &s_val);
					info->rtp_time = (s_val>0) ? s_val : 0;
				}
				else if (!_stricmp(param_name, "ssrc")) {
					sscanf(param_val, "%i", &s_val);
	...
	else if (!_strnicmp(Header, "x-", 2)) {
		x_Att = (GF_X_Attribute*)malloc(sizeof(GF_X_Attribute));
		x_Att->Name = _strdup(Header+2);
		x_Att->Value = NULL;
		if (Value && strlen(Value)) x_Att->Value = _strdup(Value);
...
GF_Err RTSP_ParseResponseHeader(GF_RTSPSession *sess, GF_RTSPResponse *rsp, u32 BodyStart)
{
	...
	//string info
	Pos = gf_token_get(LineBuffer, Pos, "\t\r\n", ValBuf, 400);
	if (Pos > 0) rsp->ResponseInfo = _strdup(ValBuf);
...
exit:
	if (rsp->Connection && !_stricmp(rsp->Connection, "Close"))
		force_reset = 1;
