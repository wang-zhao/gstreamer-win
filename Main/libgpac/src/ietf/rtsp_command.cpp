...
*/
	...
	if (!strcmp(com->method, GF_RTSP_OPTIONS)) {
		...
		if (sCtrl) {
			...
			else if (!_strnicmp(sCtrl, "rtsp", 4)) {
				sprintf(buffer, "%s %s %s\r\n", com->method, sCtrl, GF_RTSP_VERSION);
			}
...
void gf_rtsp_set_command_value(GF_RTSPCommand *com, char *Header, char *Value)
{
	...
	if (!_stricmp(Header, "Accept")) com->Accept = _strdup(Value);
	else if (!_stricmp(Header, "Accept-Encoding")) com->Accept_Encoding = _strdup(Value);
	else if (!_stricmp(Header, "Accept-Language")) com->Accept_Language = _strdup(Value);
	else if (!_stricmp(Header, "Authorization")) com->Authorization = _strdup(Value);
	else if (!_stricmp(Header, "Bandwidth")) sscanf(Value, "%d", &com->Bandwidth);
	else if (!_stricmp(Header, "Blocksize")) sscanf(Value, "%d", &com->Blocksize);
	else if (!_stricmp(Header, "Cache-Control")) com->Cache_Control = _strdup(Value);
	else if (!_stricmp(Header, "Conference")) com->Conference = _strdup(Value);
	else if (!_stricmp(Header, "Connection")) com->Connection = _strdup(Value);
	else if (!_stricmp(Header, "Content-Length")) sscanf(Value, "%d", &com->Content_Length);
	else if (!_stricmp(Header, "CSeq")) sscanf(Value, "%d", &com->CSeq);
	else if (!_stricmp(Header, "From")) com->From = _strdup(Value);
	else if (!_stricmp(Header, "Proxy_Authorization")) com->Proxy_Authorization = _strdup(Value);
	else if (!_stricmp(Header, "Proxy_Require")) com->Proxy_Require = _strdup(Value);
	else if (!_stricmp(Header, "Range")) com->Range = gf_rtsp_range_parse(Value);
	else if (!_stricmp(Header, "Referer")) com->Referer = _strdup(Value);
	else if (!_stricmp(Header, "Scale")) sscanf(Value, "%lf", &com->Scale);
	else if (!_stricmp(Header, "Session")) com->Session = _strdup(Value);
	else if (!_stricmp(Header, "Speed")) sscanf(Value, "%lf", &com->Speed);
	else if (!_stricmp(Header, "User_Agent")) com->User_Agent = _strdup(Value);
	//Transports
...
GF_Err RTSP_ParseCommandHeader(GF_RTSPSession *sess, GF_RTSPCommand *com, u32 BodyStart)
{
	...
	com->method = _strdup((const char *) ValBuf);

	//URL
	Pos = gf_token_get(LineBuffer, Pos, " \t\r\n", ValBuf, 1024);
	if (Pos <= 0) return GF_OK;
	com->service_name = _strdup(ValBuf);
...
GF_Err gf_rtsp_get_command(GF_RTSPSession *sess, GF_RTSPCommand *com)
{
	...
	if (sess->last_session_id && com->Session && !strcmp(com->Session, sess->last_session_id) 
		&& com->Connection && !_stricmp(com->Connection, "Close")) {
