...
found:
	schema[i] = 0;
	if (_stricmp(schema, "rtsp") && _stricmp(schema, "rtspu")) return GF_URL_ERROR; 
...
*/
	...
	if (!_stricmp(schema, "rtsp")) *useTCP = 1;
...
GF_RTSPSession *gf_rtsp_session_new(char *sURL, u16 DefaultPort)
{
	...
	sess->Server = _strdup(server);
	sess->Service = _strdup(service);
...
void gf_rtsp_set_mobile_ip(GF_RTSPSession *sess, char *MobileIP)
{
	if (sess->MobileIP) free(sess->MobileIP);
	sess->MobileIP = NULL;
	if (MobileIP) sess->MobileIP = _strdup(MobileIP);
}
