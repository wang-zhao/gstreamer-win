...
GF_Err gf_rtp_setup_transport(GF_RTPChannel *ch, GF_RTSPTransport *trans_info, char *remote_address)
{
	...
	if (trans_info->destination) 
		ch->net_info.destination = _strdup(trans_info->destination);

	if (trans_info->Profile) 
		ch->net_info.Profile = _strdup(trans_info->Profile);

	if (!ch->net_info.IsUnicast && trans_info->destination) {
		ch->net_info.source = _strdup(trans_info->destination);
		...
		ch->net_info.source = _strdup(trans_info->source);
	} else {
		ch->net_info.source = _strdup(remote_address);
	}
...
GF_Err gf_rtp_initialize(GF_RTPChannel *ch, u32 UDPBufferSize, Bool IsSource, u32 PathMTU, u32 ReorederingSize, u32 MaxReorderDelay, char *local_ip)
{
	...
	if (ch->net_info.Profile && 
		( !_stricmp(ch->net_info.Profile, GF_RTSP_PROFILE_RTP_AVP) 
		|| !_stricmp(ch->net_info.Profile, "RTP/AVP/UDP")
		|| !_stricmp(ch->net_info.Profile, "RTP/SAVP")
		)
		) {
