...
GF_RTSPTransport *gf_rtsp_transport_parse(char *buffer)
{
	...
	if (_strnicmp(buffer, "RTP/AVP", 7) && _strnicmp(buffer, "RTP/SAVP", 8)) return NULL;
	...
	while (1) {	
		...
		if (IsFirst) {
			tmp->Profile = _strdup(param_name);
			IsFirst = 0;
			continue;
		}

		if (!_stricmp(param_name, "destination")) {
			if (tmp->destination) free(tmp->destination);
			tmp->destination = _strdup(param_val);
		}
		else if (!_stricmp(param_name, "source")) {
			if (tmp->source) free(tmp->source);
			tmp->source = _strdup(param_val);
		}
		else if (!_stricmp(param_name, "unicast")) tmp->IsUnicast = 1;
		else if (!_stricmp(param_name, "RECORD")) tmp->IsRecord = 1;
		else if (!_stricmp(param_name, "append")) tmp->Append = 1;
		else if (!_stricmp(param_name, "interleaved")) {
			u32 rID, rcID;
			tmp->IsInterleaved = 1;
		...
		else if (!_stricmp(param_name, "layers")) sscanf(param_val, "%d", &tmp->MulticastLayers);
		else if (!_stricmp(param_name, "ttl")) sscanf(param_val, "%c	", &tmp->TTL);
		else if (!_stricmp(param_name, "port")) sscanf(param_val, "%hd-%hd", &tmp->port_first, &tmp->port_last);
		else if (!_stricmp(param_name, "server_port")) sscanf(param_val, "%hd-%hd", &tmp->port_first, &tmp->port_last);
		else if (!_stricmp(param_name, "client_port")) sscanf(param_val, "%hd-%hd", &tmp->client_port_first, &tmp->client_port_last);
		else if (!_stricmp(param_name, "ssrc")) sscanf(param_val, "%X", &tmp->SSRC);
	}
	return tmp;
}
