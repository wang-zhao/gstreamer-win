...
static u32 gf_rtp_get_payload_type(GF_RTPMap *map, GF_SDPMedia *media)
{
	...
	if (!_stricmp(map->payload_name, "MP4V-ES") ) return GF_RTP_PAYT_MPEG4;
	else if (!_stricmp(map->payload_name, "mpeg4-generic")) return GF_RTP_PAYT_MPEG4;
	else if (!_stricmp(map->payload_name, "enc-mpeg4-generic")) return GF_RTP_PAYT_MPEG4;
	/*optibase mm400 card hack*/
	else if (!_stricmp(map->payload_name, "enc-generic-mp4") ) {
		free(map->payload_name);
		map->payload_name = _strdup("enc-mpeg4-generic");
		return GF_RTP_PAYT_MPEG4;
	}

	/*LATM: only without multiplexing (not tested but should be straight AUs)*/
	else if (!_stricmp(map->payload_name, "MP4A-LATM")) {
		GF_SDP_FMTP *fmtp;
		i=0;
		while ((fmtp = (GF_SDP_FMTP *) gf_list_enum(media->FMTP, &i))) {
			...
			while ((att = (GF_X_Attribute *)gf_list_enum(fmtp->Attributes, &j))) {
				if (!_stricmp(att->Name, "cpresent") && atoi(att->Value)) return 0;
			}
		}
		return GF_RTP_PAYT_LATM;
	}
	else if (!_stricmp(map->payload_name, "MPA")) return GF_RTP_PAYT_MPEG12_AUDIO;
	else if (!_stricmp(map->payload_name, "MPV")) return GF_RTP_PAYT_MPEG12_VIDEO;
	else if (!_stricmp(map->payload_name, "H263-1998") || !_stricmp(map->payload_name, "H263-2000")) return GF_RTP_PAYT_H263;
	else if (!_stricmp(map->payload_name, "AMR")) return GF_RTP_PAYT_AMR;
	else if (!_stricmp(map->payload_name, "AMR-WB")) return GF_RTP_PAYT_AMR_WB;
	else if (!_stricmp(map->payload_name, "3gpp-tt")) return GF_RTP_PAYT_3GPP_TEXT;
	else if (!_stricmp(map->payload_name, "H264")) return GF_RTP_PAYT_H264_AVC;
	else if (!_stricmp(map->payload_name, "richmedia+xml")) return GF_RTP_PAYT_3GPP_DIMS;
	else if (!_stricmp(map->payload_name, "ac3")) return GF_RTP_PAYT_AC3;
	else return 0;
}
...
static GF_Err payt_set_param(GF_RTPDepacketizer *rtp, char *param_name, char *param_val)
{
	...
	if (!_stricmp(param_name, "Profile-level-id")) {
	...
	else if (!_stricmp(param_name, "config")) {
	...
	else if (!_stricmp(param_name, "ConstantSize")) {
		rtp->sl_map.ConstantSize = atoi(param_val);
	}
	/*constant size (size of all AUs) */
	else if (!_stricmp(param_name, "ConstantDuration")) {
		rtp->sl_map.ConstantDuration = atoi(param_val);
	}
	/*object type indication (not needed when IOD is here)*/
	else if (!_stricmp(param_name, "ObjectType")) {
		rtp->sl_map.ObjectTypeIndication = atoi(param_val);
	}
	else if (!_stricmp(param_name, "StreamType")) 
		rtp->sl_map.StreamType = atoi(param_val);
	else if (!_stricmp(param_name, "mode")) {
		strcpy(rtp->sl_map.mode, param_val);
		/*in case no IOD and no streamType/OTI in the file*/
		if (!_stricmp(param_val, "AAC-hbr") || !_stricmp(param_val, "AAC-lbr") || !_stricmp(param_val, "CELP-vbr") || !_stricmp(param_val, "CELP-cbr")) {
			rtp->sl_map.StreamType = GF_STREAM_AUDIO;
			rtp->sl_map.ObjectTypeIndication = 0x40;
		}
		/*in case no IOD and no streamType/OTI in the file*/
		else if (!_stricmp(param_val, "avc-video") ) {
			rtp->sl_map.StreamType = GF_STREAM_VISUAL;
			rtp->sl_map.ObjectTypeIndication = 0x21;
		}
	}

	else if (!_stricmp(param_name, "DTSDeltaLength")) rtp->sl_map.DTSDeltaLength = atoi(param_val);
	else if (!_stricmp(param_name, "CTSDeltaLength")) rtp->sl_map.CTSDeltaLength = atoi(param_val);
	else if (!_stricmp(param_name, "SizeLength")) rtp->sl_map.SizeLength = atoi(param_val);
	else if (!_stricmp(param_name, "IndexLength")) rtp->sl_map.IndexLength = atoi(param_val);
	else if (!_stricmp(param_name, "IndexDeltaLength")) rtp->sl_map.IndexDeltaLength = atoi(param_val);
	else if (!_stricmp(param_name, "RandomAccessIndication")) rtp->sl_map.RandomAccessIndication = atoi(param_val);
	else if (!_stricmp(param_name, "StreamStateIndication")) rtp->sl_map.StreamStateIndication = atoi(param_val);
	else if (!_stricmp(param_name, "AuxiliaryDataSizeLength")) rtp->sl_map.AuxiliaryDataSizeLength = atoi(param_val);

	/*H264/AVC config - we only handle mode 0 and 1*/
	else if (!_stricmp(param_name, "packetization-mode")) 
		rtp->h264_pck_mode = 1;
	/*AMR config*/
	else if (!_stricmp(param_name, "octet-align")) 
		rtp->flags |= GF_RTP_AMR_ALIGN;
	/*ISMACryp config*/
	else if (!_stricmp(param_name, "ISMACrypCryptoSuite")) {
		if (!_stricmp(param_val, "AES_CTR_128")) 
			rtp->isma_scheme = GF_4CC('i','A','E','C');
		else 
			rtp->isma_scheme = 0;
	}
	else if (!_stricmp(param_name, "ISMACrypSelectiveEncryption")) {
		if (!_stricmp(param_val, "1") || !_stricmp(param_val, "true"))
			rtp->flags |= GF_RTP_ISMA_SEL_ENC;
		else
			rtp->flags &= ~GF_RTP_ISMA_SEL_ENC;
	}
	else if (!_stricmp(param_name, "ISMACrypIVLength")) 
		rtp->sl_map.IV_length = atoi(param_val);
	else if (!_stricmp(param_name, "ISMACrypDeltaIVLength")) 
		rtp->sl_map.IV_delta_length = atoi(param_val);
	else if (!_stricmp(param_name, "ISMACrypKeyIndicatorLength")) 
		rtp->sl_map.KI_length = atoi(param_val);
	else if (!_stricmp(param_name, "ISMACrypKeyIndicatorPerAU")) {
		if (!_stricmp(param_val, "1") || !_stricmp(param_val, "true"))
			rtp->flags |= GF_RTP_ISMA_HAS_KEY_IDX;
		else 
			rtp->flags &= ~GF_RTP_ISMA_HAS_KEY_IDX;
	}
	else if (!_stricmp(param_name, "ISMACrypKey")) 
		rtp->key = _strdup(param_val);
...
static GF_Err gf_rtp_payt_setup(GF_RTPDepacketizer *rtp, GF_RTPMap *map, GF_SDPMedia *media)
{
	...
	if (!_stricmp(map->payload_name, "enc-mpeg4-generic")) rtp->flags |= GF_RTP_HAS_ISMACRYP;
	...
	case GF_RTP_PAYT_MPEG4:
		...
		/*RFC3016 flags*/
		if (!_stricmp(map->payload_name, "MP4V-ES")) {
			rtp->sl_map.StreamType = GF_STREAM_VISUAL;
	...
	case GF_RTP_PAYT_H263:
		{
			...
			while ((att = (GF_X_Attribute *)gf_list_enum(media->Attributes, &j))) {
				if (_stricmp(att->Name, "cliprect")) continue;
	...
	case GF_RTP_PAYT_3GPP_TEXT:
	{
		...
		while ((fmtp = (GF_SDP_FMTP*)gf_list_enum(media->FMTP, &i))) {
			...
			while ((att = (GF_X_Attribute *)gf_list_enum(fmtp->Attributes, &j))) {
				...
				if (!_stricmp(att->Name, "width")) tcfg.text_width = atoi(att->Value);
				else if (!_stricmp(att->Name, "height")) tcfg.text_height = atoi(att->Value);
				else if (!_stricmp(att->Name, "tx")) tcfg.horiz_offset = atoi(att->Value);
				else if (!_stricmp(att->Name, "ty")) tcfg.vert_offset = atoi(att->Value);
				else if (!_stricmp(att->Name, "layer")) tcfg.layer = atoi(att->Value);
				else if (!_stricmp(att->Name, "max-w")) tcfg.video_width = atoi(att->Value);
				else if (!_stricmp(att->Name, "max-h")) tcfg.video_height = atoi(att->Value);
				else if (!_stricmp(att->Name, "tx3g")) tx3g = att->Value;
			}
		}
	...
	case GF_RTP_PAYT_H264_AVC:
	{
		...
		while ((fmtp = (GF_SDP_FMTP*)gf_list_enum(media->FMTP, &i))) {
			...
			while ((att = (GF_X_Attribute *)gf_list_enum(fmtp->Attributes, &j))) {
				char *nal_ptr, *sep;
				if (_stricmp(att->Name, "sprop-parameter-sets")) continue;
