		...
		if (ref) {
			...
			case TAG_SVG_audio:
				...
				if (gf_node_get_attribute_by_tag(ref, TAG_XLINK_ATT_href, 0, 0, &info)==GF_OK) {
					return get_sync_reference(is, (XMLRI*)info.far_ptr, o_type, orig_ref ? orig_ref : ref, post_pone);
				}
				return NULL;
			case TAG_SVG_video:
...
GF_MediaObject *gf_mo_register(GF_Node *node, MFURL *url, Bool lock_timelines)
{
	...
	case TAG_SVG_audio: 
		...
		if (gf_node_get_attribute_by_tag(node, TAG_SVG_ATT_syncReference, 0, 0, &info)==GF_OK) {
			syncRef = get_sync_reference(is, (XMLRI*)info.far_ptr, GF_MEDIA_OBJECT_UNDEF, node, &post_pone);
...
Bool gf_mo_has_audio(GF_MediaObject *mo)
{
	...
	for (i=0; i<gf_list_count(is->ODlist); i++) {
		GF_ObjectManager *odm = (GF_ObjectManager*)gf_list_get(is->ODlist, i);
