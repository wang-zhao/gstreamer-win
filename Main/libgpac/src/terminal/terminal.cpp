...
static void gf_term_reload_cfg(GF_Terminal *term)
{
	...
	if (term->user->init_flags & GF_TERM_NO_VISUAL_THREAD){
		...
		if (sOpt) {
			if (!_stricmp(sOpt, "low")) prio = GF_THREAD_PRIORITY_LOWEST;
			else if (!_stricmp(sOpt, "normal")) prio = GF_THREAD_PRIORITY_NORMAL;
			else if (!_stricmp(sOpt, "high")) prio = GF_THREAD_PRIORITY_HIGHEST;
			else if (!_stricmp(sOpt, "real-time")) prio = GF_THREAD_PRIORITY_REALTIME;
		} else {
		...
		if (sOpt) {
			...
			if (!_stricmp(sOpt, "Single")) mode = GF_TERM_THREAD_SINGLE;
			else if (!_stricmp(sOpt, "Multi")) mode = GF_TERM_THREAD_MULTI;
...
GF_Terminal *gf_term_new(GF_User *user)
{
	...
	for (i=0; i< gf_list_count(tmp->extensions); i++) {
		GF_TermExt *ifce = (GF_TermExt*)gf_list_get(tmp->extensions, i);
...
GF_Err gf_term_del(GF_Terminal * term)
{
	...
	for (i=0; i< gf_list_count(term->extensions); i++) {
		GF_TermExt *ifce = (GF_TermExt*)gf_list_get(term->extensions, i);
...
void gf_term_handle_services(GF_Terminal *term)
{
	...
	if (!term->reload_state && term->unthreaded_extensions) {
		...
		for (i=0; i<count; i++) {
			GF_TermExt *ifce = (GF_TermExt*)gf_list_get(term->unthreaded_extensions, i);
...
void gf_term_service_media_event(GF_ObjectManager *odm, u32 event_type)
{
	...
	if (odm->mo) {
		...
		if (!count) return;
		if (!(gf_node_get_dom_event_filter((GF_Node*)gf_list_get(odm->mo->nodes, 0)) & GF_DOM_EVENT_MEDIA_ACCESS))
			return;
	} else {
		count = 0;
	}
	...
	if (!_strnicmp(mae.session_name, "rtsp:", 5) 
		|| !_strnicmp(mae.session_name, "sdp:", 4)
		|| !_strnicmp(mae.session_name, "rtp:", 4)
	) 
		transport = 1;
	else if (!_strnicmp(mae.session_name, "dvb:", 4)) 
		transport = 2;
...
void gf_term_navigate_to(GF_Terminal *term, const char *toURL)
{
	...
	if (!term->reload_url) term->reload_url = _strdup(toURL);
	term->reload_state = 1;
}
...
void gf_term_attach_service(GF_Terminal *term, GF_InputService *service_hdl)
{
	...
	odm->net_service->url = _strdup("Internal Service Handler");
...
GF_Err gf_term_scene_update(GF_Terminal *term, char *type, char *com)
{
	...
	if (type && (!_stricmp(type, "application/x-laser+xml") || !_stricmp(type, "laser"))) {
		load.type = GF_SM_LOAD_XSR;
		time = gf_inline_get_time(term->root_scene);
	}
	else if (type && (!_stricmp(type, "image/svg+xml") || !_stricmp(type, "svg")) ) {
		load.type = GF_SM_LOAD_XSR;
		time = gf_inline_get_time(term->root_scene);
	}
	else if (type && (!_stricmp(type, "model/x3d+xml") || !_stricmp(type, "x3d")) ) load.type = GF_SM_LOAD_X3D;
	else if (type && (!_stricmp(type, "model/x3d+vrml") || !_stricmp(type, "x3dv")) ) load.type = GF_SM_LOAD_X3DV;
	else if (type && (!_stricmp(type, "model/vrml") || !_stricmp(type, "vrml")) ) load.type = GF_SM_LOAD_VRML;
	else if (type && (!_stricmp(type, "application/x-xmt") || !_stricmp(type, "xmt")) ) load.type = GF_SM_LOAD_XMTA;
	else if (type && (!_stricmp(type, "application/x-bt") || !_stricmp(type, "bt")) ) load.type = GF_SM_LOAD_BT;
...
static void gf_term_sample_scenetime(GF_InlineScene *scene)
{
	...
	for (i=0; i<count; i++) {
		GF_ObjectManager *odm = (GF_ObjectManager*)gf_list_get(scene->ODlist, i);
