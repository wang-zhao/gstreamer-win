...
static GF_Err IS_AttachStream(GF_BaseDecoder *plug, GF_ESD *esd)
{
	...
	if (!_stricmp(devName, "KeySensor")) {
	...
	} else if (!_stricmp(devName, "StringSensor")) {
	...
	} else if (!_stricmp(devName, "Mouse")) {
		is->type = IS_Mouse;
...
#if GPAC_HTK_DEMO
	} else if (!_stricmp(devName, "HTKSensor")) {
		FILE *f;
		...
		for (i=0; i<nb_word; i++) {
			...
			for (j=0; j<nbPhone; j++) {
				...
				if (j) fprintf(f, " ");
				if (!_stricmp(szPh, "vc")) fprintf(f, "vcl");
	...
	while ((n = (X_StringSensor*)gf_list_enum(term->x3d_sensors, &i))) {
		...
		if (character=='\b') {
			if (n->deletionAllowed && st->text_len) {
				...
				szStr[len] = 0;
				n->enteredText.buffer = _strdup(szStr);
