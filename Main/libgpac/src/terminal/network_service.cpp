...
static void term_on_message(void *user_priv, GF_ClientService *service, GF_Err error, const char *message)
{
	...
	if (error==GF_IP_UDP_TIMEOUT) {
		...
		if (sOpt && !_stricmp(sOpt, "yes")) {
			...
			term->reload_state = 1;
			term->reload_url = _strdup(term->root_scene->root_od->net_service->url);
...
//	od->objectDescriptorID = GF_ESM_DYNAMIC_OD_ID;
	...
	for (i=0; i<gf_list_count(is->media_objects); i++) {
		char *frag, *ext;
		GF_ESD *esd;
		GF_MediaObject *mo = (GF_MediaObject*)gf_list_get(is->media_objects, i);
		if (!mo->odm) continue;
...
static char *get_mime_type(GF_Terminal *term, const char *url, GF_Err *ret_code)
{
	...
	(*ret_code) = GF_OK;
	if (_strnicmp(url, "http", 4)) return NULL;
	...
	if (mime_type) mime_type = _strdup(mime_type);
...
static GF_InputService *gf_term_can_handle_service(GF_Terminal *term, const char *url, const char *parent_url, Bool no_mime_check, char **out_url, GF_Err *ret_code)
{
	...
	*out_url = NULL;
	...
	if (mime_type && 
		(!_stricmp(mime_type, "text/plain") 
			|| !_stricmp(mime_type, "video/quicktime")
			|| !_stricmp(mime_type, "application/octet-stream")
		) 
	) {
		free(mime_type);
		mime_type = NULL;
	...
	if (!ifce && !mime_type && strstr(sURL, "://") && _strnicmp(sURL, "file://", 7)) ext = NULL;
...
void NM_DeleteService(GF_ClientService *ns)
{
	...
	if (ns->cache) gf_term_service_cache_close(ns, (sOpt && !_stricmp(sOpt, "yes")) ? 1 : 0);
...
GF_Err gf_term_service_cache_load(GF_ClientService *ns)
{
	...
	e = mcache->Open(mcache, ns, szName, (sOpt && !_stricmp(sOpt, "yes")) ? 1 : 0);
	if (e) {
