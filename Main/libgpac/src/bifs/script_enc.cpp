...
static GF_Err EncScriptFields(ScriptEnc *sc_enc)
{
	...
	for (i=0; i<nbFields; i++) {
		...
		gf_list_add(sc_enc->identifiers, _strdup(info.name));
...
Bool SFE_NextToken(ScriptEnc *sc_enc)
{
	...
	if (isalpha(sc_enc->cur_buf[i]) || (sc_enc->cur_buf[i]=='_')) {
		...
		if (!_stricmp(sc_enc->token, "TRUE") || !_stricmp(sc_enc->token, "FALSE") ) {
...
void SFE_PutIdentifier(ScriptEnc *sc_enc, char *id) 
{
	...
	gf_list_add(sc_enc->identifiers, _strdup(id));
...
u32 SFE_LoadExpression(ScriptEnc *sc_enc, u32 *expr_sep)
{
	...
	while ( (sc_enc->token_code != TOK_SEMICOLON) && (sc_enc->token_code != TOK_RIGHT_CURVE) ) {
		...
		case TOK_STRING:
		case TOK_BOOLEAN:
			gf_list_add(sc_enc->id_buf, _strdup(sc_enc->token));
			break;
		case TOK_FUNCTION:
			goto break_loop;
		}
...
GF_Err SFScript_Encode(GF_BifsEncoder *codec, SFScript *script_field, GF_BitStream *bs, GF_Node *n)
{
	...
	if (sc_enc.cur_buf) {
		if (!_strnicmp(sc_enc.cur_buf, "javascript:", 11) 
			|| !_strnicmp(sc_enc.cur_buf, "vrmlscript:", 11)
			|| !_strnicmp(sc_enc.cur_buf, "ECMAScript:", 11)
			) {
			sc_enc.cur_buf += 11;
		} else if (!_strnicmp(sc_enc.cur_buf, "mpeg4script:", 12) ) {
			sc_enc.cur_buf += 12;
		}
	}
...
{
    u32 v = 1;
	if (!_stricmp(str, "false") || !strcmp(str, "0")) v = 0;
