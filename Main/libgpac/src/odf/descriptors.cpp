...
u32 gf_odf_stream_type_by_name(const char *streamType)
{
	...
	if (!_stricmp(streamType, "ObjectDescriptor")) return GF_STREAM_OD;
	if (!_stricmp(streamType, "ClockReference")) return GF_STREAM_OCR;
	if (!_stricmp(streamType, "SceneDescription")) return GF_STREAM_SCENE;
	if (!_stricmp(streamType, "Visual")) return GF_STREAM_VISUAL;
	if (!_stricmp(streamType, "Audio")) return GF_STREAM_AUDIO;
	if (!_stricmp(streamType, "MPEG7")) return GF_STREAM_MPEG7;
	if (!_stricmp(streamType, "IPMP")) return GF_STREAM_IPMP;
	if (!_stricmp(streamType, "OCI")) return GF_STREAM_OCI;
	if (!_stricmp(streamType, "MPEGJ")) return GF_STREAM_MPEGJ;
	if (!_stricmp(streamType, "Interaction")) return GF_STREAM_INTERACT;
	if (!_stricmp(streamType, "Text")) return GF_STREAM_TEXT;
	return 0;
}
...
GF_Err gf_odf_get_ui_config(GF_DefaultDescriptor *dsi, GF_UIConfig *cfg)
{
	...
	if (!_stricmp(cfg->deviceName, "StringSensor") && gf_bs_available(bs)) {
...
GF_Err gf_odf_encode_ui_config(GF_UIConfig *cfg, GF_DefaultDescriptor **out_dsi)
{
	...
	*out_dsi = NULL;
	...
	if (!_stricmp(cfg->deviceName, "StringSensor")) {
		...
		for (i=0; i<nb_desc; i++) {
			...
			if (txdesc->font_count) {
				...
				for (j=0; j<txdesc->font_count; j++) {
					...
					txdesc->fonts[j].fontName = a->font_table->fonts[j].fontName ? _strdup(a->font_table->fonts[j].fontName) : NULL;
				}
			}
