...
GF_Err gf_odf_dump_ui_cfg(GF_UIConfig *uid, FILE *trace, u32 indent, Bool XMTDump)
{
	...
	if (!_stricmp(devName, "StringSensor") && uid->termChar) {
	...
	if (uid->ui_data_length) {
		if (!_stricmp(uid->deviceName, "HTKSensor")) {
			...
			for (i=0; i<nb_word; i++) {
				...
				for (j=0; j<nbPhone; j++) {
					...
					if (j) fprintf(trace, " ");
					if (!_stricmp(szPh, "vc")) fprintf(trace, "vcl");
...
GF_Err DumpRawUIConfig(GF_DefaultDescriptor *dsi, FILE *trace, u32 indent, Bool XMTDump, u32 oti)
{
	...
	if (!_stricmp(devName, "StringSensor") && gf_bs_available(bs)) {
