...
static void lsr_write_any_uri(GF_LASeRCodec *lsr, XMLRI *iri, const char *name)
{
	...
	if (is_iri) {
		if (!iri->string || _strnicmp(iri->string, "data:", 5)) {
	...
	while (att) {
	    ...
	    case TAG_SVG_ATT_requiredFeatures: 
		{
			...
			for (i=0; i<count; i++) {
				...
				if (!ext) continue;
				if (!_stricmp(ext, "Animation")) { vals[tot_count] = 0; tot_count++; }
				else if (!_stricmp(ext, "Audio")) { vals[tot_count] = 1; tot_count++; }
				else if (!_stricmp(ext, "ComposedVideo")) { vals[tot_count] = 2; tot_count++; }
				else if (!_stricmp(ext, "ConditionalProcessing")) { vals[tot_count] = 3; tot_count++; }
				else if (!_stricmp(ext, "ConditionalProcessingAttribute")) { vals[tot_count] = 4; tot_count++; }
				else if (!_stricmp(ext, "CoreAttribute")) { vals[tot_count] = 5; tot_count++; }
				else if (!_stricmp(ext, "Extensibility")) { vals[tot_count] = 6; tot_count++; }
				else if (!_stricmp(ext, "ExternalResourcesRequired")) { vals[tot_count] = 7; tot_count++; }
				else if (!_stricmp(ext, "Font")) { vals[tot_count] = 8; tot_count++; }
				else if (!_stricmp(ext, "Gradient")) { vals[tot_count] = 9; tot_count++; }
				else if (!_stricmp(ext, "GraphicsAttribute")) { vals[tot_count] = 10; tot_count++; }
				else if (!_stricmp(ext, "Handler")) { vals[tot_count] = 11; tot_count++; }
				else if (!_stricmp(ext, "Hyperlinking")) { vals[tot_count] = 12; tot_count++; }
				else if (!_stricmp(ext, "Image")) { vals[tot_count] = 13; tot_count++; }
				else if (!_stricmp(ext, "OpacityAttribute")) { vals[tot_count] = 14; tot_count++; }
				else if (!_stricmp(ext, "PaintAttribute")) { vals[tot_count] = 15; tot_count++; }
				else if (!_stricmp(ext, "Prefetch")) { vals[tot_count] = 16; tot_count++; }
				else if (!_stricmp(ext, "SVG")) { vals[tot_count] = 17; tot_count++; }
				else if (!_stricmp(ext, "SVG-animation")) { vals[tot_count] = 18; tot_count++; }
				else if (!_stricmp(ext, "SVG-dynamic")) { vals[tot_count] = 19; tot_count++; }
				else if (!_stricmp(ext, "SVG-static")) { vals[tot_count] = 20; tot_count++; }
				else if (!_stricmp(ext, "SVGDOM")) { vals[tot_count] = 21; tot_count++; }
				else if (!_stricmp(ext, "SVGDOM-animation")) { vals[tot_count] = 22; tot_count++; }
				else if (!_stricmp(ext, "SVGDOM-dynamic")) { vals[tot_count] = 23; tot_count++; }
				else if (!_stricmp(ext, "SVGDOM-static")) { vals[tot_count] = 24; tot_count++; }
				else if (!_stricmp(ext, "Script")) { vals[tot_count] = 25; tot_count++; }
				else if (!_stricmp(ext, "Shape")) { vals[tot_count] = 26; tot_count++; }
				else if (!_stricmp(ext, "SolidColor")) { vals[tot_count] = 27; tot_count++; }
				else if (!_stricmp(ext, "Structure")) { vals[tot_count] = 28; tot_count++; }
				else if (!_stricmp(ext, "Text")) { vals[tot_count] = 29; tot_count++; }
				else if (!_stricmp(ext, "TimedAnimation")) { vals[tot_count] = 30; tot_count++; }
				else if (!_stricmp(ext, "TransformedVideo")) { vals[tot_count] = 31; tot_count++; }
				else if (!_stricmp(ext, "Video")) { vals[tot_count] = 32; tot_count++; }
				else if (!_stricmp(ext, "XlinkAttribute")) { vals[tot_count] = 33; tot_count++; }
			}
...
*/
	...
	case SVG_Transform_Translate_datatype: return 9;
	case SVG_Transform_Scale_datatype: return 8;
	case SVG_Transform_Rotate_datatype: 
		...
		return 1;
	case SVG_Transform_SkewX_datatype: return 1;
	case SVG_Transform_SkewY_datatype: return 1;
...
static void lsr_write_an_anim_value(GF_LASeRCodec *lsr, void *val, u32 lsr_type, u32 svg_type, u32 transform_type, const char *name)
{
    ...
    case 5: lsr_write_paint(lsr, (SVG_Paint*)val, name); break;
    ...
    case 9: // point
		if (svg_type==SVG_Motion_datatype) {
			lsr_write_coordinate(lsr, ((GF_Matrix2D*)val)->m[2], 0, "valX");
			lsr_write_coordinate(lsr, ((GF_Matrix2D*)val)->m[5], 0, "valY");
		} else {
...
static void lsr_write_circle(GF_LASeRCodec *lsr, SVG_Element *elt)
{
	...
	lsr_write_coordinate_ptr(lsr, atts.cx, 1, "cx");
	lsr_write_coordinate_ptr(lsr, atts.cy, 1, "cy");
	lsr_write_coordinate_ptr(lsr, atts.r, 0, "r");
	lsr_write_any_attribute(lsr, elt, 1);
	lsr_write_group_content(lsr,  elt, 0);
}
static void lsr_write_conditional(GF_LASeRCodec *lsr, SVG_Element *elt)
{
	...
	lsr_write_command_list(lsr, up ? up->updates : NULL, elt, 0);
...
static void lsr_write_cursorManager(GF_LASeRCodec *lsr, SVG_Element *elt)
{
	...
	lsr_write_id(lsr, (GF_Node *) elt);
	lsr_write_rare(lsr, (GF_Node *) elt);
	lsr_write_coordinate_ptr(lsr, atts.x, 1, "x");
	lsr_write_coordinate_ptr(lsr, atts.x, 1, "y");
	lsr_write_href(lsr, atts.xlink_href);
	lsr_write_any_attribute(lsr, elt, 1);
	lsr_write_group_content(lsr,  elt, 0);
}
...
static void lsr_write_ellipse(GF_LASeRCodec *lsr, SVG_Element *elt)
{
	...
	lsr_write_coordinate_ptr(lsr, atts.cx, 1, "cx");
	lsr_write_coordinate_ptr(lsr, atts.cy, 1, "cy");
	lsr_write_coordinate_ptr(lsr, atts.rx, 0, "rx");
	lsr_write_coordinate_ptr(lsr, atts.ry, 0, "ry");
	lsr_write_any_attribute(lsr, elt, 1);
	lsr_write_group_content(lsr, elt, 0);
}
static void lsr_write_foreignObject(GF_LASeRCodec *lsr, SVG_Element *elt)
{
	...
	lsr_write_stroke(lsr, (SVG_Element*)elt, &atts);
	GF_LSR_WRITE_INT(lsr, (atts.externalResourcesRequired&&*atts.externalResourcesRequired) ? 1 : 0, 1, "externalResourcesRequired");
	lsr_write_coordinate_ptr(lsr, atts.height, 0, "height");
	lsr_write_coordinate_ptr(lsr, atts.width, 0, "width");
	lsr_write_coordinate_ptr(lsr, atts.x, 1, "x");
	lsr_write_coordinate_ptr(lsr, atts.y, 1, "y");
...
static void lsr_write_g(GF_LASeRCodec *lsr, SVG_Element *elt, Bool ommit_tag)
{
	...
	if (!ommit_tag 
		&& lsr_elt_has_same_base(lsr, &atts, lsr->prev_g, &same_fill, NULL, 0)
		&& same_fill
		) {
		/*samegType*/
...
static void lsr_write_image(GF_LASeRCodec *lsr, SVG_Element *elt)
{
	...
	lsr_write_rare(lsr, (GF_Node *) elt);
	GF_LSR_WRITE_INT(lsr, (atts.externalResourcesRequired && *atts.externalResourcesRequired) ? 1 : 0, 1, "externalResourcesRequired");
	lsr_write_coordinate_ptr(lsr, atts.height, 1, "height");
	...
	lsr_write_content_type(lsr, atts.xlink_type, "type");
	lsr_write_coordinate_ptr(lsr, atts.width, 1, "width");
	lsr_write_coordinate_ptr(lsr, atts.x, 1, "x");
	lsr_write_coordinate_ptr(lsr, atts.y, 1, "y");
	lsr_write_href(lsr, atts.xlink_href);
...
static void lsr_write_line(GF_LASeRCodec *lsr, SVG_Element *elt, Bool ommit_tag)
{
	...
	if (!ommit_tag && lsr_elt_has_same_base(lsr, &atts, lsr->prev_line, &same_fill, NULL, 0)
		&& same_fill
		) {
		/*samelineType*/
	...
	lsr_write_coordinate_ptr(lsr, atts.x1, 1, "x1");
	lsr_write_coordinate_ptr(lsr, atts.x2, 0, "x2");
	lsr_write_coordinate_ptr(lsr, atts.y1, 1, "y1");
	lsr_write_coordinate_ptr(lsr, atts.y2, 0, "y2");
	if (!is_same) {
		lsr_write_any_attribute(lsr, elt, 1);
		lsr->prev_line = elt;
	}
...
static void lsr_write_linearGradient(GF_LASeRCodec *lsr, SVG_Element *elt)
{
	...
	lsr_write_coordinate_ptr(lsr, atts.x1, 1, "x1");
	lsr_write_coordinate_ptr(lsr, atts.x2, 1, "x2");
	lsr_write_coordinate_ptr(lsr, atts.y1, 1, "y1");
	lsr_write_coordinate_ptr(lsr, atts.y2, 1, "y2");
	lsr_write_any_attribute(lsr, elt, 1);
	lsr_write_group_content(lsr, elt, 0);
}
...
static void lsr_write_path(GF_LASeRCodec *lsr, SVG_Element *elt, Bool ommit_tag)
{
	...
	if (!ommit_tag && lsr_elt_has_same_base(lsr, &atts, lsr->prev_path, &same_fill, NULL, 0) ) {
		is_same = 1;
		if (!same_fill) {
			/*samepathfillType*/
...
static void lsr_write_polygon(GF_LASeRCodec *lsr, SVG_Element *elt, Bool is_polyline, Bool ommit_tag)
{
	...
	if (!ommit_tag && lsr_elt_has_same_base(lsr, &atts, lsr->prev_polygon, &same_fill, &same_stroke, 1) 
		) {
...
static void lsr_write_radialGradient(GF_LASeRCodec *lsr, SVG_Element *elt)
{
	...
	lsr_write_coordinate_ptr(lsr, atts.cy, 1, "cy");
	lsr_write_gradient_units(lsr, atts.gradientUnits);
	lsr_write_coordinate_ptr(lsr, atts.r, 1, "r");
	lsr_write_any_attribute(lsr, elt, 1);
	lsr_write_group_content(lsr, elt, 0);
}
static void lsr_write_rect(GF_LASeRCodec *lsr, SVG_Element *elt, Bool ommit_tag)
{
	...
	if (!ommit_tag && lsr_elt_has_same_base(lsr, &atts, lsr->prev_rect, &same_fill, NULL, 0) 
		) {
		if (!same_fill) {
			same_type = 2;
			/*samerectfillType*/
		...
		lsr_write_stroke(lsr,  elt, &atts);
		lsr_write_coordinate_ptr(lsr, atts.height, 0, "height");
		lsr_write_coordinate_ptr(lsr, atts.rx, 1, "rx");
		lsr_write_coordinate_ptr(lsr, atts.ry, 1, "ry");
		lsr_write_coordinate_ptr(lsr, atts.width, 0, "width");
		lsr_write_coordinate_ptr(lsr, atts.x, 1, "x");
		lsr_write_coordinate_ptr(lsr, atts.y, 1, "y");
		lsr_write_any_attribute(lsr, elt, 1);
...
static void lsr_write_script(GF_LASeRCodec *lsr, SVG_Element *elt)
{
	...
	lsr_write_rare(lsr, (GF_Node *) elt);
	GF_LSR_WRITE_INT(lsr, (atts.externalResourcesRequired && *atts.externalResourcesRequired) ? 1 : 0, 1, "externalResourcesRequired");
	lsr_write_script_type(lsr, atts.xlink_type);
	lsr_write_href(lsr, atts.xlink_href);
	lsr_write_any_attribute(lsr, elt, 1);
	lsr_write_group_content(lsr, elt, 0);
}
...
static void lsr_write_svg(GF_LASeRCodec *lsr, SVG_Element *elt)
{
	...
	if (atts.syncBehaviorDefault) {
		...
		default: GF_LSR_WRITE_INT(lsr, 2, 2, "syncBehavior"); break;
		}
	}
...
static void lsr_write_use(GF_LASeRCodec *lsr, SVG_Element *elt, Bool ommit_tag)
{
	...
	if (!ommit_tag && lsr_elt_has_same_base(lsr, &atts, lsr->prev_use, NULL, NULL, 0) 
		/*TODO check overflow*/
	) {
		is_same = 1;
		/*sameuseType*/
...
static void lsr_write_update_content_model(GF_LASeRCodec *lsr, SVG_Element *parent, void *node)
{
	...
	if (tag==TAG_LSR_conditional) {
		...
		switch(tag) {
		case TAG_SVG_a: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_a, 6, "ch6"); lsr_write_a(lsr, (SVG_Element*)node); break;
		case TAG_SVG_animate: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_animate, 6, "ch6"); lsr_write_animate(lsr, (SVG_Element*)node, parent); break;
		case TAG_SVG_animateColor: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_animateColor, 6, "ch6"); lsr_write_animate(lsr, (SVG_Element*)node, parent); break;
		case TAG_SVG_animateMotion: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_animateMotion, 6, "ch6"); lsr_write_animateMotion(lsr, (SVG_Element*)node, parent); break;
		case TAG_SVG_animateTransform: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_animateTransform, 6, "ch6"); lsr_write_animateTransform(lsr, (SVG_Element*)node, parent); break;
		case TAG_SVG_audio: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_audio, 6, "ch6"); lsr_write_audio(lsr, (SVG_Element*)node); break;
		case TAG_SVG_circle: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_circle, 6, "ch6"); lsr_write_circle(lsr, (SVG_Element*)node); break;
		case TAG_SVG_defs: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_defs, 6, "ch6"); lsr_write_defs(lsr, (SVG_Element*)node); break;
		case TAG_SVG_desc: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_desc, 6, "ch6"); lsr_write_data(lsr, (SVG_Element*)node); break;
		case TAG_SVG_ellipse: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_ellipse, 6, "ch6"); lsr_write_ellipse(lsr, (SVG_Element*)node); break;
		case TAG_SVG_foreignObject: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_foreignObject, 6, "ch6"); lsr_write_foreignObject(lsr, (SVG_Element*)node); break;
		case TAG_SVG_g: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_g, 6, "ch6"); lsr_write_g(lsr, (SVG_Element*)node, 1); break;
		case TAG_SVG_image: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_image, 6, "ch6"); lsr_write_image(lsr, (SVG_Element*)node); break;
		case TAG_SVG_line: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_line, 6, "ch6"); lsr_write_line(lsr, (SVG_Element*)node, 1); break;
		case TAG_SVG_linearGradient: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_linearGradient, 6, "ch6"); lsr_write_linearGradient(lsr, (SVG_Element*)node); break;
		case TAG_SVG_metadata: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_metadata, 6, "ch6"); lsr_write_data(lsr, (SVG_Element*)node); break;
		case TAG_SVG_mpath: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_mpath, 6, "ch6"); lsr_write_mpath(lsr, (SVG_Element*)node); break;
		case TAG_SVG_path: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_path, 6, "ch6"); lsr_write_path(lsr, (SVG_Element*)node, 1); break;
		case TAG_SVG_polygon: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_polygon, 6, "ch6"); lsr_write_polygon(lsr, (SVG_Element*)node, 0, 1); break;
		case TAG_SVG_polyline: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_polyline, 6, "ch6"); lsr_write_polygon(lsr, (SVG_Element*)node, 1, 1); break;
		case TAG_SVG_radialGradient: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_radialGradient, 6, "ch6"); lsr_write_radialGradient(lsr, (SVG_Element*)node); break;
		case TAG_SVG_rect: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_rect, 6, "ch6"); lsr_write_rect(lsr, (SVG_Element*)node, 1); break;
		case TAG_SVG_script: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_script, 6, "ch6"); lsr_write_script(lsr, (SVG_Element*)node); break;
		case TAG_SVG_set: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_set, 6, "ch6"); lsr_write_set(lsr, (SVG_Element*)node, parent); break;
		case TAG_SVG_stop: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_stop, 6, "ch6"); lsr_write_stop(lsr, (SVG_Element*)node); break;
		case TAG_SVG_svg: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_svg, 6, "ch6"); lsr_write_svg(lsr, (SVG_Element*)node); break;
		case TAG_SVG_switch: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_switch, 6, "ch6"); lsr_write_switch(lsr, (SVG_Element*)node); break;
		case TAG_SVG_text: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_text, 6, "ch6"); lsr_write_text(lsr, (SVG_Element*)node, 1); break;
		case TAG_SVG_title: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_title, 6, "ch6"); lsr_write_data(lsr, (SVG_Element*)node); break;
		case TAG_SVG_tspan: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_tspan, 6, "ch6"); lsr_write_tspan(lsr, (SVG_Element*)node); break;
		case TAG_SVG_use: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_use, 6, "ch6"); lsr_write_use(lsr, (SVG_Element*)node, 1); break;
		case TAG_SVG_video: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_video, 6, "ch6"); lsr_write_video(lsr, (SVG_Element*)node); break;
		case TAG_SVG_listener: GF_LSR_WRITE_INT(lsr, LSR_UPDATE_CONTENT_MODEL_listener, 6, "ch6"); lsr_write_listener(lsr, (SVG_Element*)node); break;
		}
	}
}
...
static void lsr_write_update_value(GF_LASeRCodec *lsr, SVG_Element *elt, u32 fieldType, u32 att_tag, u32 transformType, void *val, Bool is_indexed)
{
	...
	if (is_indexed) {
		...
		case SVG_Points_datatype:/*points*/
		{
			...
			lsr_write_coordinate(lsr, pt->x, 0, "x");
			lsr_write_coordinate(lsr, pt->y, 0, "y");
		}
			break;
		...
		case SMIL_KeyTimes_datatype:/*keyTime*/
		{
			...
			if ((f==0) || (f==FIX_ONE)) {
				...
				GF_LSR_WRITE_INT(lsr, 0, 1, "hasShort"); 
				lsr_write_fixed_clamp(lsr, f, "timevalue");
			}
		}
			break;
		...
		case XMLRI_datatype:
			if ((att_tag==TAG_XLINK_ATT_href) || (att_tag==TAG_SVG_ATT_syncReference)) {
				...
				lsr_write_vluimsbf5(lsr, gf_node_get_id((GF_Node*) ((XMLRI*)val)->target) - 1, "ID");
			} else {
		...
		case SVG_Focus_datatype:
			if ( (((SVG_Focus*)val)->type == SVG_FOCUS_AUTO) || !((SVG_Focus*)val)->target.target) {
				...
				GF_LSR_WRITE_INT(lsr, 0, 1, "isDefault");
				GF_LSR_WRITE_INT(lsr, 0, 1, "isEscape");
				lsr_write_vluimsbf5(lsr, gf_node_get_id((GF_Node*) ((SVG_Focus*)val)->target.target) - 1, "ID");
			}
			break;
		...
		case SVG_LanguageID_datatype:
			...
			break;
		case SVG_Motion_datatype:
			lsr_write_coordinate(lsr, ((GF_Matrix2D *)val)->m[2], 0, "pointValueX");
			lsr_write_coordinate(lsr, ((GF_Matrix2D *)val)->m[5], 0, "pointValueY");
			break;
		case SVG_Points_datatype:
...
static void lsr_check_font_index(GF_LASeRCodec *lsr, SVG_FontFamily *font)
{
	...
	if (font && (font->type == SVG_FONTFAMILY_VALUE) && font->value) {
		...
		if (!found) gf_list_add(lsr->font_table, _strdup(font->value));
	}
}
...
static void lsr_check_font_and_color(GF_LASeRCodec *lsr, SVG_Element *elt)
{
	...
	while (att) {
		...
		case SVG_Paint_datatype:
			lsr_check_col_index(lsr, NULL, (SVG_Paint*)att->data);
			break;
		case SVG_FontFamily_datatype:
			lsr_check_font_index(lsr, (SVG_FontFamily*)att->data);
			break;
		case SMIL_AttributeName_datatype:
		{
			...
			if (an->name) {
				...
				else if (!strcmp(an->name, "solid-olor")) check_anim_col = 2;
	...
	if (check_anim_font || check_anim_col) {
		...
		while (att) {
			...
			case SMIL_AnimateValue_datatype:
				if (check_anim_font) lsr_check_font_index(lsr, (SVG_FontFamily*)((SMIL_AnimateValue*)att->data)->value);
				else if (check_anim_col == 1) lsr_check_col_index(lsr, NULL, (SVG_Paint*)((SMIL_AnimateValue*)att->data)->value);
				else if (check_anim_col == 2) lsr_check_col_index(lsr, (SVG_Color*)((SMIL_AnimateValue*)att->data)->value, NULL);
				break;
...
check_children:
	...
	while (l) {
		if (l->node->sgprivate->tag==TAG_DOMUpdates) {
			...
			for (i=0; i<count; i++) {
				u32 j, c2;
				GF_Command *com = (GF_Command*)gf_list_get(up->updates, i);
