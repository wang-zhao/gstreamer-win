...
GF_Err gf_isom_set_meta_type(GF_ISOFile *file, Bool root_meta, u32 track_num, u32 metaType)
{
	...
	meta->handler->nameUTF8 = _strdup(szName);
	return GF_OK;
}
...
GF_Err gf_isom_add_meta_item(GF_ISOFile *file, Bool root_meta, u32 track_num, Bool self_reference, char *resource_path, const char *item_name, const char *mime_type, const char *content_encoding, const char *URL, const char *URN)
{
	...
	/*get relative name*/
	if (item_name) {
		infe->item_name = _strdup(item_name);
	} else if (resource_path) {
		if (strrchr(resource_path, GF_PATH_SEPARATOR)) {
			infe->item_name = _strdup(strrchr(resource_path, GF_PATH_SEPARATOR) + 1);
		} else {
			infe->item_name = _strdup(resource_path);
		}
	}

	if (mime_type) {
		infe->content_type = _strdup(mime_type);
	} else {
		infe->content_type = _strdup("application/octet-stream");
	}
	if (content_encoding) infe->content_encoding = _strdup(content_encoding);
	...
	if (self_reference) {
		...
		if (!infe->item_name) infe->item_name = _strdup("");
		return GF_OK;
	}
	...
	/*store full path for info*/
		...
		infe->full_path = _strdup(resource_path);
	}
	return GF_OK;
}
