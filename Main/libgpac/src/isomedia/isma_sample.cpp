...
GF_Err gf_isom_change_ismacryp_protection(GF_ISOFile *the_file, u32 trackNumber, u32 StreamDescriptionIndex, char *scheme_uri, char *kms_uri)
{
	...
	if (scheme_uri) {
		...
		sea->protection_info->scheme_type->URI = _strdup(scheme_uri);
	}
	if (kms_uri) {
		free(sea->protection_info->info->ikms->URI);
		sea->protection_info->info->ikms->URI = _strdup(kms_uri);
	}
	return GF_OK;
}
						   ...
						   Bool selective_encryption, u32 KI_length, u32 IV_length)
{
	...
	if (contentID) sea->protection_info->info->okms->hdr->ContentID = _strdup(contentID);
	if (kms_URI) sea->protection_info->info->okms->hdr->RightsIssuerURL = _strdup(kms_URI);
	if (textual_headers) {
		sea->protection_info->info->okms->hdr->TextualHeaders = (char*)malloc(sizeof(char)*textual_headers_len);
