...
GF_Err ctts_Read(GF_Box *s, GF_BitStream *bs)
{
	...
	ptr->nb_entries = gf_bs_read_u32(bs);
	ptr->alloc_size = ptr->nb_entries;
	ptr->entries = (GF_DttsEntry*)malloc(sizeof(GF_DttsEntry)*ptr->alloc_size);
...
GF_Err stsc_Read(GF_Box *s, GF_BitStream *bs)
{
	...
	ptr->nb_entries = gf_bs_read_u32(bs);
	ptr->alloc_size = ptr->nb_entries;
	ptr->entries = (GF_StscEntry*)malloc(sizeof(GF_StscEntry)*ptr->alloc_size);
	if (!ptr->entries) return GF_OUT_OF_MEM;
...
GF_Err stts_Read(GF_Box *s, GF_BitStream *bs)
{
	...
	ptr->nb_entries = gf_bs_read_u32(bs);
	ptr->alloc_size = ptr->nb_entries;
	ptr->entries = (GF_SttsEntry*)malloc(sizeof(GF_SttsEntry)*ptr->alloc_size);
	if (!ptr->entries) return GF_OUT_OF_MEM;
	for (i=0; i<ptr->nb_entries; i++) {
		ptr->entries[i].sampleCount = gf_bs_read_u32(bs);
...
GF_Err metx_Read(GF_Box *s, GF_BitStream *bs)
{
	...
	if (i) ptr->content_encoding = _strdup(str);
	...
	if (i) ptr->mime_type_or_namespace = _strdup(str);
	...
	if (ptr->type==GF_ISOM_BOX_TYPE_METX) {
		...
		if (i) ptr->xml_schema_loc = _strdup(str);
	}
	ptr->size = size;
