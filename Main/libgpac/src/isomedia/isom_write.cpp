...
GF_Err gf_isom_set_root_od_url(GF_ISOFile *movie, char *url_string)
{
	...
	case GF_ODF_ISOM_OD_TAG:
		...
		((GF_IsomObjectDescriptor *)movie->moov->iods->descriptor)->URLString = url_string ? _strdup(url_string) : NULL;
		break;
	case GF_ODF_ISOM_IOD_TAG:
		...
		((GF_IsomInitialObjectDescriptor *)movie->moov->iods->descriptor)->URLString = url_string ? _strdup(url_string) : NULL;
		break;
	default:
		return GF_ISOM_INVALID_FILE;
	}
	...
	if (duration) {
		...
		if (ent->sampleCount == 1) {
			...
			if (stts->nb_entries==stts->alloc_size) {
				stts->alloc_size++;
				stts->entries = (GF_SttsEntry*)realloc(stts->entries, sizeof(GF_SttsEntry)*stts->alloc_size);
...
GF_Err gf_isom_set_final_name(GF_ISOFile *movie, char *filename)
{
	...
	if (filename) {
		...
		movie->finalName = _strdup(filename);
...
GF_Err gf_isom_add_chapter(GF_ISOFile *movie, u32 trackNumber, u64 timestamp, char *name)
{
	...
	ce->name = name ? _strdup(name) : NULL;
...
GF_Err gf_isom_set_track_name(GF_ISOFile *the_file, u32 trackNumber, char *name)
{
	...
	trak->name = NULL;
	if (name) trak->name = _strdup(name);
	return GF_OK;
}
...
GF_Err gf_isom_set_handler_name(GF_ISOFile *the_file, u32 trackNumber, const char *nameUTF8)
{
	...
	if (!_strnicmp(nameUTF8, "file://", 7)) {
		...
		szLine[j] = 0;
		trak->Media->handler->nameUTF8 = _strdup(szLine);
	}
	return GF_OK;
}
...
GF_Err gf_isom_set_track_switch_parameter(GF_ISOFile *movie, u32 trackNumber, u32 trackRefGroup, Bool is_switch_group, u32 *switchGroupID, u32 *criteriaList, u32 criteriaListCount)
{
	...
	if (*switchGroupID) {
		...
		tsel->attributeList = (u32*)malloc(sizeof(u32)*criteriaListCount);
...
GF_Err gf_isom_timed_meta_data_config_new(GF_ISOFile *movie, u32 trackNumber, Bool is_xml, char *mime_or_namespace, char *content_encoding, char *schema_loc, char *URLname, char *URNname, u32 *outDescriptionIndex)
{
	...
	metad->mime_type_or_namespace = _strdup(mime_or_namespace);
	if (content_encoding) metad->content_encoding = _strdup(content_encoding);
	if (schema_loc) metad->xml_schema_loc = _strdup(schema_loc);
