...
/*
    ...
    */
#ifdef WIN32
    extern "C" s32 gettimeofday(struct timeval *tp, void *tz);
#endif
...
GF_Err gf_sk_get_local_ip(GF_Socket *sock, char *buffer)
{
	...
	if (sock->flags & GF_SOCK_HAS_PEER) {
		...
		if (getsockname(sock->socket, (struct sockaddr*) &name, (int*)&len)) return GF_IP_NETWORK_FAILURE;
...
GF_Err gf_sk_set_block_mode(GF_Socket *sock, u32 NonBlockingOn)
{
	...
	if (sock->socket) {
		res = ioctlsocket(sock->socket, FIONBIO, (u_long*)&val);
...
u32 gf_sk_is_multicast_address(char *multi_IPAdd)
{
	...
	if (sep && !_strnicmp(multi_IPAdd, "ff", 2)) return 1;
...
GF_Err gf_sk_receive(GF_Socket *sock, char *buffer, u32 length, u32 startFrom, u32 *BytesRead)
{
	...
	*BytesRead = 0;
	...
	if (sock->flags & GF_SOCK_HAS_PEER)
		res = recvfrom(sock->socket, (char *) buffer + startFrom, length - startFrom, 0, (struct sockaddr *)&sock->dest_addr, (int*)&sock->dest_addr_len);
	else
...
#endif
	sk = accept(sock->socket, (struct sockaddr *) &sock->dest_addr,(int*) &client_address_size);
...
GF_Err gf_sk_get_local_info(GF_Socket *sock, u16 *Port, u32 *Familly)
{
	...
	if (Port) {
		...
		*Port = (u32) ntohs(the_add.sin6_port);
		...
		if (getsockname(sock->socket, (struct sockaddr *) &the_add, (int*)&size) == SOCKET_ERROR) return GF_IP_NETWORK_FAILURE;
