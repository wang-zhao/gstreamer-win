...
#ifndef GPAC_DISABLE_LOG
	if (name) {
		tmp->log_name = _strdup(name);
	} else {
		char szN[20];
		sprintf(szN, "0x%08x", (u32) tmp);
		tmp->log_name = _strdup(szN);
	}
	log_add_thread(tmp);
#endif
	return tmp;
...
#ifdef WIN32
	::InitializeCriticalSection(&tmp->hMutex);
	if (!(&tmp->hMutex)) {
#else
...
{
#ifdef WIN32
	DeleteCriticalSection(&mx->hMutex);
#else
...
Bool gf_mx_try_lock(GF_Mutex *mx)
{
	...
	switch (TryEnterCriticalSection(&mx->hMutex)) {
	case 0:
