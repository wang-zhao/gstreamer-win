...
{
	if (!_strnicmp(url, "file://", 7)) return 1;
	if (!_strnicmp(url, "file:///", 8)) return 1;
...
{
	if (!_strnicmp(url, "http://", 7)) return 1;
#ifdef GPAC_HAS_SSL
	if (!_strnicmp(url, "https://", 8)) return 1;
#endif
	return 0;
}
...
void gf_dm_configure_cache(GF_DownloadSession *sess)
{
	...
	sess->cache_name = (char*)malloc(sizeof(char)*len);
	sess->cache_name[0] = 0;
	...
	if (opt && !_stricmp(opt, "yes")) sess->cache_start_size = 0;
}
...
void gf_dm_sess_del(GF_DownloadSession *sess)
{
	...
	if (sess->cache_name) {
		...
		if (opt && !_stricmp(opt, "yes")) gf_delete_file(sess->cache_name);
...
static GF_Err gf_dm_setup_from_url(GF_DownloadSession *sess, char *url)
{
	...
	tmp = strchr(url, '/');
	sess->remote_path = _strdup(tmp ? tmp : "/");
	if (tmp) {
		tmp[0] = 0;
		tmp_url = _strdup(url);
		tmp[0] = '/';
	...
	tmp = strrchr(tmp_url, '@');
	if (tmp) {
		sess->server_name = _strdup(tmp+1);
		tmp[0] = 0;
		tmp = strchr(tmp_url, ':');
		if (sess->user) free(sess->user);
		sess->user = NULL;
		...
		if (tmp) {
			sess->passwd = _strdup(tmp+1);
			tmp[0] = 0;
		}
		sess->user = _strdup(tmp_url);
	} else {
		sess->server_name = _strdup(tmp_url);
	}
	free(tmp_url);
...
GF_DownloadManager *gf_dm_new(GF_Config *cfg)
{
	...
	if (opt) {
		if (opt[strlen(opt)-1] != GF_PATH_SEPARATOR) {
			...
			dm->cache_directory = _strdup(opt);
		}
	}
#ifdef GPAC_HAS_SSL
	ssl_init(dm, 0);
...
{
	char *res;
	if (_strnicmp(line, header_name, strlen(header_name))) return NULL;
...
void http_do_requests(GF_DownloadSession *sess)
{
	...
	if (sess->status==GF_NETIO_CONNECTED) {
		...
		if (send_profile || par.data) {
			...
			char *tmp_buf = (char*)malloc(sizeof(char)*(len+par.size));
	...
	if (sess->status == GF_NETIO_WAIT_FOR_REPLY) {
		...
		if (!strncmp("ICY", comp, 4)) {
			...
			sess->mime_type = _strdup("audio/mpeg");
		...
		while (1) {
			...
			if (!_stricmp(hdr, "Content-Length") ) ContentLength = (u32) atoi(hdr_val);
			else if (!_stricmp(hdr, "Content-Type")) {			
				if (sess->mime_type) free(sess->mime_type);
				sess->mime_type = _strdup(hdr_val);
				while (1) {
			...
			else if (!_stricmp(hdr, "Content-Range")) {			
				range = 1;
			...
			else if (!_stricmp(hdr, "Accept-Ranges")) {
				if (strstr(hdr_val, "none")) no_range = 1;
			}
			else if (!_stricmp(hdr, "Location")) 
				new_location = _strdup(hdr_val);
			else if (!_stricmp(hdr, "icy-metaint")) 
				sess->icy_metaint = atoi(hdr_val);
			else if (!_stricmp(hdr, "ice") || !_stricmp(hdr, "icy") ) 
				is_ice = 1;
			else if (!_stricmp(hdr, "X-UserProfileID") ) 
				gf_cfg_set_key(sess->dm->cfg, "Downloader", "UserProfileID", hdr_val);
/*			else if (!_stricmp(hdr, "Connection") ) 
...
//		if (!is_ice && !ContentLength) e = GF_REMOTE_SERVICE_ERROR;
		...
		if (is_ice) {
			sess->flags |= GF_NETIO_SESSION_NOT_CACHED;
			if (sess->mime_type && !_stricmp(sess->mime_type, "video/nsv")) {
				free(sess->mime_type);
				sess->mime_type = _strdup("audio/aac");
			}
		}
