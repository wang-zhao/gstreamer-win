...
static void svg_traverse_switch(GF_Node *node, void *rs, Bool is_destroy)
{
	...
	SVGPropertiesPointers backup_props;
	u32 backup_flags;
	s32 *selected_idx = (s32*)gf_node_get_private(node);
	u32 styling_size = sizeof(SVGPropertiesPointers);
	SVGAllAttributes all_atts;
	GF_TraverseState *tr_state = (GF_TraverseState *) rs;
...
static void svg_a_handle_event(GF_Node *handler, GF_DOM_Event *event, GF_Node *observer)
{
	...
	if (is_timing_target((GF_Node*)all_atts.xlink_href->target)) {
		gf_smil_timing_insert_clock((GF_Node*)all_atts.xlink_href->target, 0, gf_node_get_scene_time((GF_Node *)handler) );
	} 
