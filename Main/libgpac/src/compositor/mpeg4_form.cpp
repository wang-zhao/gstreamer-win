...
static void form_apply(FormStack *st, const char *constraint, u32 *group_idx, u32 count)
{
	...
	/*SHin*/
	if (!_strnicmp(constraint, "SHin", 4)) {
		shin_apply(st, group_idx, count);
		return;
	}
	/*SH*/
	if (!_strnicmp(constraint, "SH", 2)) {
	...
	/*SVin*/
	if (!_strnicmp(constraint, "SVin", 4)) {
		svin_apply(st, group_idx, count);
		return;
	}
	/*SV*/
	if (!_strnicmp(constraint, "SV", 2)) {
	...
	/*AH*/
	if (!_strnicmp(constraint, "AH", 2)) {
		ah_apply(st, group_idx, count);
		return;
	}
	/*AV*/
	if (!_strnicmp(constraint, "AV", 2)) {
