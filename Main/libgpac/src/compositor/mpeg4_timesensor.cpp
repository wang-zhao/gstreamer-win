...
void timesensor_update_time(GF_TimeNode *st)
{
	...
	TimeSensorStack *stack = (TimeSensorStack *)gf_node_get_private((GF_Node*)st->udta);

	if (! TS->enabled) {
		if (TS->isActive) {
			TS->cycleTime = gf_node_get_scene_time((GF_Node*)st->udta);
	...
	currentTime = gf_node_get_scene_time((GF_Node*)st->udta);
	if (!TS->isActive) {
	...
	if (TS->isActive) {
		TS->time = currentTime;
		gf_node_event_out((GF_Node*)st->udta, 8);//"time"
		...
		*/
		...
		if ((TS->stopTime > stack->start_time) && (currentTime >= TS->stopTime) ) {
			...
			gf_node_event_out((GF_Node*)st->udta, 6);//"fraction_changed"
			timesensor_deactivate(stack, TS);
			return;
		}
		if (! TS->loop) {
	...
	if (!TS->isActive) {
		...
		TS->isActive = 1;
		gf_node_event_out((GF_Node*)st->udta, 7); //"isActive"
