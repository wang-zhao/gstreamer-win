...
void drawable_check_focus_highlight(GF_Node *node, GF_TraverseState *tr_state, GF_Rect *orig_bounds)
{
	...
	prev_node = (GF_Node*)gf_node_get_private(hlight->node);
	if (prev_node != node) {
	...
	if (props->fill->type==SVG_PAINT_URI) {
		...
		if (props->fill->iri.type == XMLRI_ELEMENTID) {
			...
			case TAG_SVG_solidColor:
			{
				...
				gf_node_traverse((GF_Node*)props->fill->iri.target, tr_state);
			
				if (gf_node_dirty_get((GF_Node*)props->fill->iri.target))
					ret = 1;
	...
	if (props->stroke->type==SVG_PAINT_URI) {
		...
		if (props->stroke->iri.type == XMLRI_ELEMENTID) {
			...
			case TAG_SVG_solidColor:
			{
				...
				gf_node_traverse((GF_Node*)props->stroke->iri.target, tr_state);

				if (gf_node_dirty_get((GF_Node*)props->stroke->iri.target))
					ret = 1;
