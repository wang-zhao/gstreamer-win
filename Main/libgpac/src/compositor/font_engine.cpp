...
GF_FontManager *gf_font_manager_new(GF_User *user)
{
	...
	font_mgr->id_buffer = (u32*)malloc(sizeof(u32)*font_mgr->id_buffer_size);
...
void gf_font_predestroy(GF_Font *font)
{
	if (font->spans) {
		while (gf_list_count(font->spans)) {
			GF_TextSpan *ts = (GF_TextSpan*)gf_list_get(font->spans, 0);
...
GF_Font *gf_font_manager_set_font_ex(GF_FontManager *fm, char **alt_fonts, u32 nb_fonts, u32 styles, Bool check_only)
{
	...
	for (i=0; i<nb_fonts; i++) {
		...
		while (font) {
			if ((check_only || !font->not_loaded) && font->name && !_stricmp(font->name, alt_fonts[i])) {
				s32 fw;
				s32 w;
				u32 diff;
		...
		if (fm->reader) {
			...
			if (!e) {
				...
				fm->reader->get_font_info((_font_reader*)fm->reader, (char**)&the_font->name, (s32*)&the_font->em_size, &the_font->ascent, &the_font->descent, &the_font->underline, &the_font->line_spacing, &the_font->max_advance_h, &the_font->max_advance_v);
				the_font->styles = styles;
				if (!the_font->name) the_font->name = _strdup(alt_fonts[i]);
...
GF_TextSpan *gf_font_manager_create_span(GF_FontManager *fm, GF_Font *font, char *text, Fixed font_size, Bool needs_x_offset, Bool needs_y_offset, Bool needs_rotate, const char *xml_lang, Bool fliped_text, u32 styles, GF_Node *user)
{
	...
	if (e==GF_BUFFER_TOO_SMALL) {
		...
		fm->id_buffer = (u32*)realloc(fm->id_buffer, sizeof(u32) * len);
...
//	span->lang = xml_lang;
	...
	span->nb_glyphs = len;
	span->glyphs = (GF_Glyph**)malloc(sizeof(void *)*len);
	if (needs_x_offset) {
		span->dx = (Fixed*)malloc(sizeof(Fixed)*len);
		memset(span->dx, 0, sizeof(Fixed)*len);
	}
	if (needs_y_offset) {
		span->dy = (Fixed*)malloc(sizeof(Fixed)*len);
		memset(span->dy, 0, sizeof(Fixed)*len);
	}
	if (needs_rotate) {
		span->rot = (Fixed*)malloc(sizeof(Fixed)*len);
...
static Bool span_setup_texture(GF_Compositor *compositor, GF_TextSpan *span, Bool for_3d, GF_TraverseState *tr_state)
{
	...
	raster->stencil_set_texture(
		stencil, 
		span->ext->txh->data, 
		span->ext->txh->width, 
		span->ext->txh->height, 
		span->ext->txh->stride,
		(GF_PixelFormat)(span->ext->txh->pixelformat), 
		(GF_PixelFormat)(span->ext->txh->pixelformat), 
		1
	);
...
*/				
		...
		if (ctx) {
			...
			if (!compositor->sel_buffer_alloc || compositor->sel_buffer_len == compositor->sel_buffer_alloc) {
				...
				compositor->sel_buffer = (u16*)realloc(compositor->sel_buffer, sizeof(u16)*compositor->sel_buffer_alloc);
			}
