...
static void flush_text_node_edit(GF_Compositor *compositor, Bool final_flush)
{
	...
	if (compositor->sel_buffer_len) {
		const u16 *lptr;
		txt = (u8*)malloc(sizeof(char)*2*compositor->sel_buffer_len);
		lptr = compositor->sel_buffer;
		len = gf_utf8_wcstombs((char*)txt, 2*compositor->sel_buffer_len, &lptr);
		txt[len] = 0;
		*compositor->edited_text = _strdup((const char*)txt);
		free(txt);
	}
...
GF_Err gf_sc_paste_text(GF_Compositor *compositor, const char *text)
{
	...
	conv_buf = (u16*)malloc(sizeof(u16)*len);
			...
			if (!cmd_type) res = &((GF_DOMText *)child->node)->textContent;
				...
				if (append) {
					...
					if (!compositor->caret_pos) {
						...
						cur->textContent = (char*)malloc(sizeof(char)*(len+1));
						srcp = compositor->sel_buffer;
						len = gf_utf8_wcstombs(cur->textContent, len, (const u16**)&srcp);
						...
						if (compositor->caret_pos+1<compositor->sel_buffer_len) {
							...
							ntext->textContent = (char*)malloc(sizeof(char)*(len+1));
							srcp = compositor->sel_buffer + compositor->caret_pos + 1;
							len = gf_utf8_wcstombs(ntext->textContent, len, (const u16**)&srcp);
							ntext->textContent[len] = 0;
						} else {
							ntext->textContent = _strdup("");
						}
					...
					if (delete_cr && child->next) {
						...
						if (child->next && (gf_node_get_tag(child->next->node)==TAG_DOMText) ) {
							...
							if (n2->textContent) {
								n1->textContent = (char*)realloc(n1->textContent, sizeof(char)*(strlen(n1->textContent)+strlen(n2->textContent)+1));
...
*/
		...
		if (!res && !cmd_type) {
			GF_DOMText *t = gf_dom_add_text_node(compositor->focus_node, _strdup(""));
			res = &t->textContent;
		}
	...
	if (*res) {
		...
		compositor->sel_buffer = (u16*)malloc(sizeof(u16)*2);
