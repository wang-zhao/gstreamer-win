...
GF_Err gf_sc_set_viewpoint(GF_Compositor *compositor, u32 viewpoint_idx, const char *viewpoint_name)
{
	...
	for (i=0; i<count;i++) {
		...
		if (name && !_stricmp(name, viewpoint_name)) {
...
static void TraverseNavigationInfo(GF_Node *node, void *rs, Bool is_destroy)
{
	...
	for (i=0; i<ni->type.count; i++) {
		if (ni->type.vals[i] && !_stricmp(ni->type.vals[i], "NONE")) {
	...
	for (i=0; i<ni->type.count; i++) {
		if (ni->type.vals[i] && !_stricmp(ni->type.vals[i], "ANY")) tr_state->camera->navigation_flags |= NAV_ANY;
		if (!tr_state->camera->navigate_mode) {
			if (ni->type.vals[i] && !_stricmp(ni->type.vals[i], "NONE")) tr_state->camera->navigate_mode = GF_NAVIGATE_NONE;
			else if (ni->type.vals[i] && !_stricmp(ni->type.vals[i], "WALK")) tr_state->camera->navigate_mode = GF_NAVIGATE_WALK;
			else if (ni->type.vals[i] && !_stricmp(ni->type.vals[i], "EXAMINE")) tr_state->camera->navigate_mode = GF_NAVIGATE_EXAMINE;
			else if (ni->type.vals[i] && !_stricmp(ni->type.vals[i], "FLY")) tr_state->camera->navigate_mode = GF_NAVIGATE_FLY;
			else if (ni->type.vals[i] && !_stricmp(ni->type.vals[i], "QTVR")) tr_state->camera->navigate_mode = GF_NAVIGATE_VR;
		}
	}
