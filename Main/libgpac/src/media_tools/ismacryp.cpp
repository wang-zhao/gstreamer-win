...
void isma_ea_node_start(void *sax_cbck, const char *node_name, const char *name_space, const GF_XMLAttribute *attributes, u32 nb_attributes)
{
	...
	if (!strcmp(node_name, "ISMACrypTrack") || !strcmp(node_name, "OMATrack")) {
		...
		for (i=0; i<nb_attributes; i++) {
			...
			if (!_stricmp(att->name, "trackID") || !_stricmp(att->name, "ID")) {
				if (!strcmp(att->value, "*")) info->has_common_key = 1;
				else tkc->trackID = atoi(att->value);
			}
			else if (!_stricmp(att->name, "key")) {
				char *sKey = att->value;
				if (!_strnicmp(sKey, "0x", 2)) sKey += 2;
				if (strlen(sKey) == 32) {
					u32 j;
					for (j=0; j<32; j+=2) {
						u32 v;
			...
			else if (!_stricmp(att->name, "salt")) {
				u32 len, j;
				char *sKey = att->value;
				if (!_strnicmp(sKey, "0x", 2)) sKey += 2;
				len = strlen(sKey);
			...
			else if (!_stricmp(att->name, "kms_URI")) strcpy(tkc->KMS_URI, att->value);
			else if (!_stricmp(att->name, "rightsIssuerURL")) strcpy(tkc->KMS_URI, att->value);
			else if (!_stricmp(att->name, "scheme_URI")) strcpy(tkc->Scheme_URI, att->value);
			else if (!_stricmp(att->name, "selectiveType")) {
				if (!_stricmp(att->value, "Rap")) tkc->sel_enc_type = GF_ISMACRYP_SELENC_RAP;
				else if (!_stricmp(att->value, "Non-Rap")) tkc->sel_enc_type = GF_ISMACRYP_SELENC_NON_RAP;
				else if (!_stricmp(att->value, "Rand")) tkc->sel_enc_type = GF_ISMACRYP_SELENC_RAND;
				else if (!_strnicmp(att->value, "Rand", 4)) {
				...
				else if (!_strnicmp(att->value, "Preview", 7)) {
					tkc->sel_enc_type = GF_ISMACRYP_SELENC_PREVIEW;
				}
			}
			else if (!_stricmp(att->name, "Preview")) {
				tkc->sel_enc_type = GF_ISMACRYP_SELENC_PREVIEW;
				sscanf(att->value, "%d", &tkc->sel_enc_range);
			}
			else if (!_stricmp(att->name, "ipmpType")) {
				if (!_stricmp(att->value, "None")) tkc->ipmp_type = 0;
				else if (!_stricmp(att->value, "IPMP")) tkc->sel_enc_type = 1;
				else if (!_stricmp(att->value, "IPMPX")) tkc->sel_enc_type = 2;
			}
			else if (!_stricmp(att->name, "ipmpDescriptorID")) tkc->ipmp_desc_id = atoi(att->value);
			else if (!_stricmp(att->name, "encryptionMethod")) {
			...
			else if (!_stricmp(att->name, "contentID")) strcpy(tkc->Scheme_URI, att->value);
			else if (!_stricmp(att->name, "rightsIssuerURL")) strcpy(tkc->KMS_URI, att->value);
			else if (!_stricmp(att->name, "transactionID")) {
				if (strlen(att->value)<=16) strcpy(tkc->TransactionID, att->value);
			}
			else if (!_stricmp(att->name, "textualHeaders")) {
			}
		}
	}
}
...
Bool gf_ismacryp_mpeg4ip_get_info(char *kms_uri, char *key, char *salt)
{
	...
	while (kms && !feof(kms)) {
		...
		if (_stricmp(szPath, kms_uri)) continue;
		for (i=0; i<24; i++) {
...
GF_Err gf_ismacryp_decrypt_track(GF_ISOFile *mp4, GF_TrackCryptInfo *tci, void (*progress)(void *cbk, u32 done, u32 total), void *cbk)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (ismasamp->flags & GF_ISOM_ISMA_IS_ENCRYPTED) {
			/*restore IV*/
			if (!prev_sample_encrypted) resync_IV(mc, ismasamp->IV, (char*)&tci->salt[0]);
...
GF_Err gf_ismacryp_decrypt_file(GF_ISOFile *mp4, const char *drm_file)
{
	...
	for (i=0; i<nb_tracks; i++) {
		...
		/*MPEG4IP*/
		else if (!_stricmp(KMS_URI, "AudioKey") || !_stricmp(KMS_URI, "VideoKey")) {
			if (!gf_ismacryp_mpeg4ip_get_info((char *) KMS_URI, (char*)&tci.key[0], (char*)&tci.salt[0])) {
				GF_LOG(GF_LOG_ERROR, GF_LOG_AUTHOR, ("[ISMA E&A] Couldn't load MPEG4IP ISMACryp keys for TrackID %d\n", trackID));
				continue;
			}
		} else if (!drm_file) {
			FILE *test = NULL;
			if (!_stricmp(scheme_URI, "urn:gpac:isma:encryption_scheme")) test = fopen(KMS_URI, "rt");
			...
			fclose(test);
			if (gf_ismacryp_gpac_get_info(tci.trackID, (char *) KMS_URI, (char*)&tci.key[0], (char*)&tci.salt[0]) != GF_OK) {
...
GF_Err gf_ismacryp_encrypt_track(GF_ISOFile *mp4, GF_TrackCryptInfo *tci, void (*progress)(void *cbk, u32 done, u32 total), void *cbk)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (isamp->flags & GF_ISOM_ISMA_IS_ENCRYPTED) {
			/*resync IV*/
			if (!prev_sample_encryped) resync_IV(mc, BSO, (char*)&tci->salt[0]);
		...
		if (is_isom) {
			...
			if (box_type==GF_4CC('m','d','r','i')) {
				...
				while (bsize<box_size) {
					...
					gf_bs_read_data(bs, (char*)&block[0], to_read);
