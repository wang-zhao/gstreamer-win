...
#include <internal/avilib.h>
#include <internal/ogg.h>
#include <internal/vobsub.h>
#include <xml.h>
#include <mpegts.h>
#include <constants.h>
...
GF_Err gf_import_avi_video(GF_MediaImporter *import)
{
	...
	if (!_stricmp(comp, "DIVX") || !_stricmp(comp, "DX50")	/*DivX*/
		|| !_stricmp(comp, "XVID") /*XviD*/
		|| !_stricmp(comp, "3iv2") /*3ivX*/
		|| !_stricmp(comp, "fvfw") /*ffmpeg*/
		|| !_stricmp(comp, "NDIG") /*nero*/
		|| !_stricmp(comp, "MP4V") /*!! not tested*/
		|| !_stricmp(comp, "M4CC") /*Divio - not tested*/
		|| !_stricmp(comp, "PVMM") /*PacketVideo - not tested*/
		|| !_stricmp(comp, "SEDG") /*Samsung - not tested*/
		|| !_stricmp(comp, "RMP4") /*Sigma - not tested*/
		|| !_stricmp(comp, "MP43") /*not tested*/
		|| !_stricmp(comp, "FMP4") /*not tested*/
		) {
		e = GF_OK;
	}
	else if (!_stricmp(comp, "DIV3") || !_stricmp(comp, "DIV4")) {
		...
		e = GF_NOT_SUPPORTED;
		goto exit;
	} else if (!_stricmp(comp, "H264") || !_stricmp(comp, "X264")) {
...
static void nhml_node_start(void *sax_cbck, const char *node_name, const char *name_space, const GF_XMLAttribute *attributes, u32 nb_attributes)
{
	...
	for (i=0; i<nb_attributes; i++) {
		...
		if (_stricmp(att->name, "DEF") && _stricmp(att->name, "id")) continue;
		node_id = _strdup(att->value);
		break;
	}
	if (!node_id) {
		node_id = _strdup("__nhml__none");
...
GF_Err gf_import_sample_from_xml(GF_MediaImporter *import, GF_ISOSample *samp, char *xml_file, char *xmlFrom, char *xmlTo, u32 *max_size)
{
	...
	tmp = strchr(xmlFrom, '.');
	*tmp = 0;
	if (_stricmp(xmlFrom, "doc")) breaker.from_id = _strdup(xmlFrom);
	...
	*tmp = '.';
	...
	tmp = strchr(xmlTo, '.');
	*tmp = 0;
	if (_stricmp(xmlTo, "doc")) breaker.to_id = _strdup(xmlTo);
...
static GF_Err compress_sample_data(GF_ISOSample *samp, u32 *max_size, char **dict, u32 offset)
{
	...
	if (*max_size < stream.total_out) {
		*max_size = samp->dataLength*ZLIB_COMPRESS_SAFE;
		samp->data = (char*)realloc(samp->data, *max_size * sizeof(char));
	} 
...
GF_Err gf_import_nhml_dims(GF_MediaImporter *import, Bool dims_doc)
{
	...
	if (_stricmp(root->name, szRootName)) { 
	...
	while ((att = (GF_XMLAttribute *)gf_list_enum(root->attributes, &i))) {
		if (!_stricmp(att->name, "streamType")) streamType = atoi(att->value);
		else if (!_stricmp(att->name, "mediaType") && (strlen(att->value)==4)) {
			mtype = GF_4CC(att->value[0], att->value[1], att->value[2], att->value[3]);
		}
		else if (!_stricmp(att->name, "mediaSubType") && (strlen(att->value)==4)) {
			sdesc.codec_tag = GF_4CC(att->value[0], att->value[1], att->value[2], att->value[3]);
		}
		else if (!_stricmp(att->name, "objectTypeIndication")) oti = atoi(att->value);
		else if (!_stricmp(att->name, "timeScale")) timescale = atoi(att->value);
		else if (!_stricmp(att->name, "width")) sdesc.width = atoi(att->value);
		else if (!_stricmp(att->name, "height")) sdesc.height = atoi(att->value);
		else if (!_stricmp(att->name, "parNum")) par_num = atoi(att->value);
		else if (!_stricmp(att->name, "parDen")) par_den = atoi(att->value);
		else if (!_stricmp(att->name, "sampleRate")) sdesc.samplerate = atoi(att->value);
		else if (!_stricmp(att->name, "numChannels")) sdesc.nb_channels = atoi(att->value);
		else if (!_stricmp(att->name, "baseMediaFile")) strcpy(szMedia, att->value);
		else if (!_stricmp(att->name, "specificInfoFile")) strcpy(szInfo, att->value);
		else if (!_stricmp(att->name, "trackID")) tkID = atoi(att->value);
		else if (!_stricmp(att->name, "inRootOD")) inRootOD = (!_stricmp(att->value, "yes") );
		else if (!_stricmp(att->name, "DTS_increment")) dts_inc = atoi(att->value);
		else if (!_stricmp(att->name, "gzipSamples")) do_compress = (!_stricmp(att->value, "yes")) ? 1 : 0;
		else if (!_stricmp(att->name, "gzipDictionary")) {
			u32 d_size;
			if (_stricmp(att->value, "self")) {
		...
		/*unknow desc related*/
		else if (!_stricmp(att->name, "compressorName")) strcpy(sdesc.compressor_name, att->value);
		else if (!_stricmp(att->name, "codecVersion")) sdesc.version = atoi(att->value);
		else if (!_stricmp(att->name, "codecRevision")) sdesc.revision = atoi(att->value);
		else if (!_stricmp(att->name, "codecVendor") && (strlen(att->value)==4)) {
			sdesc.vendor_code = GF_4CC(att->value[0], att->value[1], att->value[2], att->value[3]);
		}
		else if (!_stricmp(att->name, "temporalQuality")) sdesc.temporal_quality = atoi(att->value);
		else if (!_stricmp(att->name, "spatialQuality")) sdesc.spacial_quality = atoi(att->value);
		else if (!_stricmp(att->name, "horizontalResolution")) sdesc.h_res = atoi(att->value);
		else if (!_stricmp(att->name, "verticalResolution")) sdesc.v_res = atoi(att->value);
		else if (!_stricmp(att->name, "bitDepth")) sdesc.depth = atoi(att->value);
		else if (!_stricmp(att->name, "bitsPerSample")) sdesc.bits_per_sample = atoi(att->value);

		/*DIMS stuff*/
		else if (!_stricmp(att->name, "profile")) dims.profile = atoi(att->value);
		else if (!_stricmp(att->name, "level")) dims.level = atoi(att->value);
		else if (!_stricmp(att->name, "pathComponents")) dims.pathComponents = atoi(att->value);
		else if (!_stricmp(att->name, "useFullRequestHost") && !_stricmp(att->value, "yes")) dims.fullRequestHost = 1;
		else if (!_stricmp(att->name, "stream_type") && !_stricmp(att->value, "secondary")) dims.streamType = 0;
		else if (!_stricmp(att->name, "contains_redundant")) {
			if (!_stricmp(att->value, "main")) dims.containsRedundant = 1;
			else if (!_stricmp(att->value, "redundant")) dims.containsRedundant = 2;
			else if (!_stricmp(att->value, "main+redundant")) dims.containsRedundant = 3;
		}
		else if (!_stricmp(att->name, "text_encoding")) dims.textEncoding = att->value;
		else if (!_stricmp(att->name, "content_encoding")) {
			if (!strcmp(att->value, "deflate")) {
				dims.contentEncoding = att->value;
				do_compress = 1;
			}
		}
		else if (!_stricmp(att->name, "content_script_types")) dims.content_script_types = att->value;
	...
	while ((node = (GF_XMLNode *) gf_list_enum(root->content, &i))) {
		...
		if (_stricmp(node->name, szSampleName) ) continue;
		...
		while ( (att = (GF_XMLAttribute *)gf_list_enum(node->attributes, &j))) {
			if (!_stricmp(att->name, "DTS") || !_stricmp(att->name, "time")) {
				u32 h, m, s, ms;
			...
			else if (!_stricmp(att->name, "CTSOffset")) samp->CTS_Offset = atoi(att->value);
			else if (!_stricmp(att->name, "isRAP") && !samp->IsRAP) {
				samp->IsRAP = (!_stricmp(att->value, "yes")) ? 1 : 0;
			}
			else if (!_stricmp(att->name, "isSyncShadow")) samp->IsRAP = !_stricmp(att->value, "yes") ? 2 : 0;
			else if (!_stricmp(att->name, "mediaOffset")) offset = (s64) atof(att->value) ;
			else if (!_stricmp(att->name, "dataLength")) samp->dataLength = atoi(att->value);
			else if (!_stricmp(att->name, "mediaFile")) strcpy(szMediaTemp, att->value);
			else if (!_stricmp(att->name, "xmlFrom")) strcpy(szXmlFrom, att->value);
			else if (!_stricmp(att->name, "xmlTo")) strcpy(szXmlTo, att->value);
			/*DIMS flags*/
			else if (!_stricmp(att->name, "is-Scene") && !_stricmp(att->value, "yes"))
				dims_flags |= GF_DIMS_UNIT_S;
			else if (!_stricmp(att->name, "is-RAP") && !_stricmp(att->value, "yes")) {
				dims_flags |= GF_DIMS_UNIT_M;
				samp->IsRAP = 1;
			}
			else if (!_stricmp(att->name, "is-redundant") && !_stricmp(att->value, "yes"))
				dims_flags |= GF_DIMS_UNIT_I;
			else if (!_stricmp(att->name, "redundant-exit") && !_stricmp(att->value, "yes")) 
				dims_flags |= GF_DIMS_UNIT_D;
			else if (!_stricmp(att->name, "priority") && !_stricmp(att->value, "high")) 
				dims_flags |= GF_DIMS_UNIT_P;
			else if (!_stricmp(att->name, "compress") && !_stricmp(att->value, "yes")) 
...
GF_Err gf_import_amr_evrc_smv(GF_MediaImporter *import)
{
	...
	if (!_strnicmp(magic, "#!AMR\n", 6)) {
	...
	else if (!_strnicmp(magic, "#!AMR-WB_MC1.0\n", 15)) {
		fclose(mdia);
		...
		if (ext && !_stricmp(ext, ".amr")) { mtype = GF_ISOM_SUBTYPE_3GP_AMR; update_gpp_cfg = 1; ext = "AMR"; msg = "Importing AMR";}
		else if (ext && !_stricmp(ext, ".evc")) { mtype = GF_ISOM_SUBTYPE_3GP_EVRC; oti = 0xA0; ext = "EVRC"; msg = "Importing EVRC";}
		else if (ext && !_stricmp(ext, ".smv")) { mtype = GF_ISOM_SUBTYPE_3GP_SMV; oti = 0xA1; ext = "SMV"; msg = "Importing SMV";}
		else {
			fclose(mdia);
...
GF_Err gf_import_qcp(GF_MediaImporter *import)
{
	...
	while (gf_bs_available(bs)) {
		...
		} else if (!_strnicmp(magic, "labl", 4)) {
		} else if (!_strnicmp(magic, "offs", 4)) {
		} else if (!_strnicmp(magic, "cnfg", 4)) {
		} else if (!_strnicmp(magic, "text", 4)) {
		}
		gf_bs_skip_bytes(bs, chunk_size);
		if (has_pad) gf_bs_read_u8(bs);
	}
...
u32 H263_NextStartCode(GF_BitStream *bs)
{
	...
	while (!end) {
		...
		if (bpos == (u32) load_size) {
			...
			gf_bs_read_data(bs, (char*)&h263_cache[0], (u32) load_size);
		}
...
//	case GF_M2TS_EVT_SDT_REPEAT:
	...
	case GF_M2TS_EVT_SDT_FOUND:
		...
		for (i=0; i<import->nb_progs; i++) {
			...
			strcpy(import->pg_info[i].name, (const char*)sdt->service);
...
GF_Err gf_media_import(GF_MediaImporter *importer)
{
	...
	/*AVI audio/video*/
	if (!_strnicmp(ext, ".avi", 4) || !_stricmp(fmt, "AVI") ) {
		e = gf_import_avi_video(importer);
		if (e) return e;
		return gf_import_avi_audio(importer);
	}
	/*OGG audio/video*/
	if (!_strnicmp(ext, ".ogg", 4) || !_stricmp(fmt, "OGG")) {
		e = gf_import_ogg_video(importer);
		if (e) return e;
		return gf_import_ogg_audio(importer);
	}
	/*MPEG PS*/
	if (!_strnicmp(ext, ".mpg", 4) || !_strnicmp(ext, ".mpeg", 5) 
		|| !_strnicmp(ext, ".vob", 4) || !_strnicmp(ext, ".vcd", 4) || !_strnicmp(ext, ".svcd", 5)
		|| !_stricmp(fmt, "MPEG1") || !_stricmp(fmt, "MPEG-PS")  || !_stricmp(fmt, "MPEG2-PS") 
		) {
	...
	/*MPEG-2 TS*/
	if (!_strnicmp(ext, ".ts", 3) || !_strnicmp(ext, ".m2t", 4) 
		|| !_stricmp(fmt, "MPEGTS") || !_stricmp(fmt, "MPEG-TS") 
		|| !_stricmp(fmt, "MPGTS") || !_stricmp(fmt, "MPG-TS") 
		|| !_stricmp(fmt, "MPEG2TS")  || !_stricmp(fmt, "MPEG2-TS") 
		|| !_stricmp(fmt, "MPG2TS")  || !_stricmp(fmt, "MPG2-TS") 
		) {
		return gf_import_mpeg_ts(importer);
	}

	/*MPEG1/2 Audio*/
	if (!_strnicmp(ext, ".mp2", 4) || !_strnicmp(ext, ".mp3", 4) || !_strnicmp(ext, ".m1a", 4) || !_strnicmp(ext, ".m2a", 4) || !_stricmp(fmt, "MP3") || !_stricmp(fmt, "MPEG-AUDIO") ) 
		return gf_import_mp3(importer);
	/*NHNT*/
	if (!_strnicmp(ext, ".media", 5) || !_strnicmp(ext, ".info", 5) || !_strnicmp(ext, ".nhnt", 5) || !_stricmp(fmt, "NHNT") )
		return gf_import_nhnt(importer);
	/*NHML*/
	if (!_strnicmp(ext, ".nhml", 5) || !_stricmp(fmt, "NHML") )
		return gf_import_nhml_dims(importer, 0);
	/*jpg & png & jp2*/
	if (!_strnicmp(ext, ".jpg", 4) || !_strnicmp(ext, ".jpeg", 5) || !_strnicmp(ext, ".jp2", 4) || !_strnicmp(ext, ".png", 4) || !_stricmp(fmt, "JPEG") || !_stricmp(fmt, "PNG") || !_stricmp(fmt, "JP2") ) 
		return gf_import_still_image(importer);
	/*MPEG-2/4 AAC*/
	if (!_strnicmp(ext, ".aac", 4) || !_stricmp(fmt, "AAC") || !_stricmp(fmt, "MPEG4-AUDIO") ) 
		return gf_import_aac_adts(importer);
	/*AMR & 3GPP2 speec codecs*/
	if (!_strnicmp(ext, ".amr", 4) || !_strnicmp(ext, ".awb", 4) || !_strnicmp(ext, ".smv", 4) || !_strnicmp(ext, ".evc", 4)
		|| !_stricmp(fmt, "AMR") || !_stricmp(fmt, "EVRC") || !_stricmp(fmt, "SMV") ) 
		return gf_import_amr_evrc_smv(importer);
	/*QCelp & other in QCP file format*/
	if (!_strnicmp(ext, ".qcp", 4) || !_stricmp(fmt, "QCELP") ) 
		return gf_import_qcp(importer);
	/*MPEG-4 video*/
	if (!_strnicmp(ext, ".cmp", 4) || !_strnicmp(ext, ".m4v", 4) || !_stricmp(fmt, "CMP") || !_stricmp(fmt, "MPEG4-Video") ) 
		return gf_import_cmp(importer, 0);
	/*MPEG-1/2 video*/
	if (!_strnicmp(ext, ".m2v", 4) || !_strnicmp(ext, ".m1v", 4) || !_stricmp(fmt, "MPEG2-Video") || !_stricmp(fmt, "MPEG1-Video") ) 
		return gf_import_cmp(importer, 2);
	/*H2632 video*/
	if (!_strnicmp(ext, ".263", 4) || !_strnicmp(ext, ".h263", 5) || !_stricmp(fmt, "H263") ) 
		return gf_import_h263(importer);
	/*H264/AVC video*/
	if (!_strnicmp(ext, ".h264", 5) || !_strnicmp(ext, ".264", 4) || !_strnicmp(ext, ".x264", 5)
		|| !_strnicmp(ext, ".h26L", 5) || !_strnicmp(ext, ".26l", 4)
		|| !_stricmp(fmt, "AVC") || !_stricmp(fmt, "H264") ) 
		return gf_import_h264(importer);
	/*MPEG-4 SAF multiplex*/
	if (!_strnicmp(ext, ".saf", 4) || !_strnicmp(ext, ".lsr", 4) || !_stricmp(fmt, "SAF") ) 
		return gf_import_saf(importer);
	/*text subtitles*/
	if (!_strnicmp(ext, ".srt", 4) || !_strnicmp(ext, ".sub", 4) || !_strnicmp(ext, ".ttxt", 5) 
		|| !_stricmp(fmt, "SRT") || !_stricmp(fmt, "SUB") || !_stricmp(fmt, "TEXT") ) 
		return gf_import_timed_text(importer);
	/*VobSub*/
    if (!_strnicmp(ext, ".idx", 4) || !_stricmp(fmt, "VOBSUB")) 
	...
	/*AC3*/
	if (!_strnicmp(ext, ".ac3", 4) || !_stricmp(fmt, "AC3") )
		return gf_import_ac3(importer);
	/*DIMS*/
	if (!_strnicmp(ext, ".dml", 4) || !_stricmp(fmt, "DIMS") )
		return gf_import_nhml_dims(importer, 1);

	/*try XML things*/
	xml_type = gf_xml_get_root_type(importer->in_name, &e);
	if (xml_type) {
		if (!_stricmp(xml_type, "TextStream") || !_stricmp(xml_type, "text3GTrack") ) {
			free(xml_type);
			return gf_import_timed_text(importer);
		}
		else if (!_stricmp(xml_type, "NHNTStream")) {
			free(xml_type);
			return gf_import_nhml_dims(importer, 0);
		}
		else if (!_stricmp(xml_type, "DIMSStream") ) {
			free(xml_type);
			return gf_import_nhml_dims(importer, 1);
		}
		free(xml_type);
