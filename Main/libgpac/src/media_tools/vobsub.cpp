...
char *vobsub_lang_id(char *name)
{
	...
	for (i = 0; i < count; i++) {
		if (!_stricmp(lang_table[i].lang, name)) {
...
GF_Err vobsub_read_idx(FILE *file, vobsub_file *vobsub, s32 *version)
{
	...
	for (line = 0; !error && fgets(strbuf, sizeof(strbuf), file); line++)
	{
		...
		*pos  = '\0';
		...
		else if (_stricmp(entry, "palette") == 0)
		{
			s32 c;
			u8  palette[16][4];
		...
		else if (id >= 0 && _stricmp(entry, "delay") == 0)
		{
			s32  hh, mm, ss, ms;
			char c;
			s32  sign = (str[0] == '-') ? -1 : 1;
		...
		else if (id >= 0 && _stricmp(entry, "timestamp") == 0)
		{
			vobsub_pos *vspos;
			s32         sign;
			char        c;
