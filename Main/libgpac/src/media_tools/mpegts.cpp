	...
	while (sc_pos<data_len) {
		...
		if (!start_code_found) {
			...
			if (!pes->vid_w && (nal_type==GF_AVC_NALU_SEQ_PARAM)) {
				AVCState avc;
				s32 idx;
				GF_BitStream *bs = gf_bs_new((char*)data+5, sc_pos-5, GF_BITSTREAM_READ);
...
//					u32 k = 0;
		...
		if (pes->aud_sr) {
			...
			if (size>pes->buf_len) {
				pes->buf_len = size;
				pes->buf = (unsigned char*)realloc(pes->buf, sizeof(char)*pes->buf_len);
			}
...
static void gf_m2ts_section_complete(GF_M2TS_Demuxer *ts, GF_M2TS_SectionFilter *sec, GF_M2TS_SECTION_ES *ses)
{
	if (! sec->process_section && !sec->had_error) {
		if (ts->on_event) {
			...
			pck.data = (char*)malloc(sizeof(unsigned char)*pck.data_len);
		...
		status = 0;
		/*parse header*/
		data = (unsigned char*)sec->section;
...
static void gf_m2ts_process_pes(GF_M2TS_Demuxer *ts, GF_M2TS_PES *pes, GF_M2TS_Header *hdr, unsigned char *data, u32 data_size, GF_M2TS_AdaptationField *paf)
{
	...
	if (hdr->payload_start) {
		...
		/*reassemble pes*/
		if (pes->data) pes->data = (unsigned char*)realloc(pes->data, pes->data_len+data_size);
		else pes->data = (unsigned char*)malloc(data_size);
