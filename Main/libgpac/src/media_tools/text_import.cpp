...
#include <internal/isomedia_dev.h>
...
static GF_Err gf_text_guess_format(char *filename, u32 *fmt)
{
	...
	*fmt = GF_TEXT_IMPORT_NONE;
	...
	else if (!_strnicmp(szLine, "<?xml ", 6)) {
		char *ext = strrchr(filename, '.');
		if (!_strnicmp(ext, ".ttxt", 5)) *fmt = GF_TEXT_IMPORT_TTXT;
		ext = strstr(szLine, "?>");
		if (ext) ext += 2;
	...
	if (cfg) {
		...
		for (i=0; i<count; i++) {
			...
			if (!sd->font_count) {
				...
				sd->font_count = 1;
				sd->fonts[0].fontID = 1;
				sd->fonts[0].fontName = _strdup("Serif");
			}
		...
		sd->font_count = 1;
		sd->fonts[0].fontID = 1;
		sd->fonts[0].fontName = _strdup(import->fontName ? import->fontName : "Serif");
...
void ttxt_parse_text_box(GF_MediaImporter *import, GF_XMLNode *n, GF_BoxRecord *box)
{
	...
	while ( (att=(GF_XMLAttribute *)gf_list_enum(n->attributes, &i))) {
		if (!_stricmp(att->name, "top")) box->top = atoi(att->value);
		else if (!_stricmp(att->name, "bottom")) box->bottom = atoi(att->value);
		else if (!_stricmp(att->name, "left")) box->left = atoi(att->value);
		else if (!_stricmp(att->name, "right")) box->right = atoi(att->value);
	}
}
...
void ttxt_parse_text_style(GF_MediaImporter *import, GF_XMLNode *n, GF_StyleRecord *style)
{
	...
	while ( (att=(GF_XMLAttribute *)gf_list_enum(n->attributes, &i))) {
		if (!_stricmp(att->name, "fromChar")) style->startCharOffset = atoi(att->value);
		else if (!_stricmp(att->name, "toChar")) style->endCharOffset = atoi(att->value);
		else if (!_stricmp(att->name, "fontID")) style->fontID = atoi(att->value);
		else if (!_stricmp(att->name, "fontSize")) style->font_size = atoi(att->value);
		else if (!_stricmp(att->name, "color")) style->text_color = ttxt_get_color(import, att->value);
		else if (!_stricmp(att->name, "styles")) {
	...
	while ( (node = (GF_XMLNode*)gf_list_enum(root->content, &i))) {
				...
				if (!strcmp(sdesc->name, "TextSampleDescription")) {
					...
					while ( (att=(GF_XMLAttribute *)gf_list_enum(sdesc->attributes, &k))) {
						if (!strcmp(att->name, "horizontalJustification")) {
							if (!_stricmp(att->value, "center")) td.horiz_justif = 1;
							else if (!_stricmp(att->value, "right")) td.horiz_justif = (s8) -1;
							else if (!_stricmp(att->value, "left")) td.horiz_justif = 0;
						}
						else if (!strcmp(att->name, "verticalJustification")) {
							if (!_stricmp(att->value, "center")) td.vert_justif = 1;
							else if (!_stricmp(att->value, "bottom")) td.vert_justif = (s8) -1;
							else if (!_stricmp(att->value, "top")) td.vert_justif = 0;
						}
						else if (!strcmp(att->name, "backColor")) td.back_color = ttxt_get_color(import, att->value);
						else if (!strcmp(att->name, "verticalText") && !_stricmp(att->value, "yes") ) td.displayFlags |= GF_TXT_VERTICAL;
						else if (!strcmp(att->name, "fillTextRegion") && !_stricmp(att->value, "yes") ) td.displayFlags |= GF_TXT_FILL_REGION;
						else if (!strcmp(att->name, "continuousKaraoke") && !_stricmp(att->value, "yes") ) td.displayFlags |= GF_TXT_KARAOKE;
						else if (!strcmp(att->name, "scroll")) {
							if (!_stricmp(att->value, "inout")) td.displayFlags |= GF_TXT_SCROLL_IN | GF_TXT_SCROLL_OUT;
							else if (!_stricmp(att->value, "in")) td.displayFlags |= GF_TXT_SCROLL_IN;
							else if (!_stricmp(att->value, "out")) td.displayFlags |= GF_TXT_SCROLL_OUT;
						}
						else if (!strcmp(att->name, "scrollMode")) {
							u32 scroll_mode = GF_TXT_SCROLL_CREDITS;
							if (!_stricmp(att->value, "Credits")) scroll_mode = GF_TXT_SCROLL_CREDITS;
							else if (!_stricmp(att->value, "Marquee")) scroll_mode = GF_TXT_SCROLL_MARQUEE;
							else if (!_stricmp(att->value, "Right")) scroll_mode = GF_TXT_SCROLL_RIGHT;
							else if (!_stricmp(att->value, "Down")) scroll_mode = GF_TXT_SCROLL_DOWN;
						...
						if (!strcmp(ext->name, "TextBox")) ttxt_parse_text_box(import, ext, &td.default_pos);
							...
							while ( (ftable=(GF_XMLNode*)gf_list_enum(ext->content, &z))) {
								...
								while ( (att=(GF_XMLAttribute *)gf_list_enum(ftable->attributes, &m))) {
									if (!_stricmp(att->name, "fontID")) td.fonts[td.font_count-1].fontID = atoi(att->value);
									else if (!_stricmp(att->name, "fontName")) td.fonts[td.font_count-1].fontName = _strdup(att->value);
								}
							}
						}
					}
					...
					if (!td.fonts) {
						...
						td.fonts[0].fontID = 1;
						td.fonts[0].fontName = _strdup("Serif");
					}
		...
		/*sample text*/
			...
			while ( (ext=(GF_XMLNode*)gf_list_enum(node->content, &j))) {
				...
				if (!_stricmp(ext->name, "Style")) {
				...
				else if (!_stricmp(ext->name, "Blinking")) {
					u16 start, end;
					start = end = 0;
					k=0;
					while ( (att=(GF_XMLAttribute *)gf_list_enum(ext->attributes, &k))) {
				...
				else if (!_stricmp(ext->name, "HyperLink")) {
					u16 start, end;
					char *url, *url_tt;
					start = end = 0;
					url = url_tt = NULL;
					...
					while ( (att=(GF_XMLAttribute *)gf_list_enum(ext->attributes, &k))) {
						...
						else if (!strcmp(att->name, "URL")) url = _strdup(att->value);
						else if (!strcmp(att->name, "URLToolTip")) url_tt = _strdup(att->value);
					}
					gf_isom_text_add_hyperlink(samp, url, url_tt, start, end);
					if (url) free(url);
					if (url_tt) free(url_tt);
				}
				else if (!_stricmp(ext->name, "Karaoke")) {
					u32 startTime;
					GF_XMLNode *krok;
					startTime = 0;
					k=0;
...
void tx3g_parse_text_box(GF_MediaImporter *import, GF_XMLNode *n, GF_BoxRecord *box)
{
	...
	memset(box, 0, sizeof(GF_BoxRecord));
	while ((att=(GF_XMLAttribute *)gf_list_enum(n->attributes, &i))) {
		if (!_stricmp(att->name, "x")) box->left = atoi(att->value);
		else if (!_stricmp(att->name, "y")) box->top = atoi(att->value);
		else if (!_stricmp(att->name, "height")) box->bottom = atoi(att->value);
		else if (!_stricmp(att->name, "width")) box->right = atoi(att->value);
	}
}
...
static GF_Err gf_text_import_texml(GF_MediaImporter *import)
{
	...
	while ( (node=(GF_XMLNode*)gf_list_enum(root->content, &i))) {
		...
		while ((att=(GF_XMLAttribute *)gf_list_enum(node->attributes, &j))) {
			...
			else if (!strcmp(att->name, "keyframe")) isRAP = !_stricmp(att->value, "true");
		}
		nb_styles = 0;
		nb_marks = 0;
		...
		while ((desc=(GF_XMLNode*)gf_list_enum(node->content, &j))) {
			...
			if (!strcmp(desc->name, "description")) {
						...
						while ((ftable=(GF_XMLNode*)gf_list_enum(sub->content, &m))) {
							...
							if (!strcmp(ftable->name, "font")) {
								...
								while ((att=(GF_XMLAttribute *)gf_list_enum(ftable->attributes, &n))) {
									if (!_stricmp(att->name, "id")) td.fonts[td.font_count-1].fontID = atoi(att->value);
									else if (!_stricmp(att->name, "name")) td.fonts[td.font_count-1].fontName = _strdup(att->value);
								}
							}
						}
					}
				...
				while ((sub=(GF_XMLNode*)gf_list_enum(desc->content, &k))) {
					...
					else if (!_stricmp(sub->name, "highlight")) {
						m=0;
						while ((att=(GF_XMLAttribute *)gf_list_enum(sub->attributes, &m))) {
					...
					else if (!_stricmp(sub->name, "blink")) {
						m=0;
						while ((att=(GF_XMLAttribute *)gf_list_enum(sub->attributes, &m))) {
							if (!strcmp(att->name, "startMarker")) GET_MARKER_POS(start, 0)
							else if (!strcmp(att->name, "endMarker")) GET_MARKER_POS(end, 1)
					...
					else if (!_stricmp(sub->name, "link")) {
						char *url, *url_tt;
						url = url_tt = NULL;
						m=0;
						while ((att=(GF_XMLAttribute *)gf_list_enum(sub->attributes, &m))) {
							...
							else if (!strcmp(att->name, "URL") || !strcmp(att->name, "href")) url = _strdup(att->value);
							else if (!strcmp(att->name, "URLToolTip") || !strcmp(att->name, "altString")) url_tt = _strdup(att->value);
						}
						gf_isom_text_add_hyperlink(samp, url, url_tt, start, end);
						if (url) free(url);
						if (url_tt) free(url_tt);
					}
					else if (!_stricmp(sub->name, "karaoke")) {
						u32 time = 0;
						GF_XMLNode *krok;
						m=0;
						while ((att=(GF_XMLAttribute *)gf_list_enum(sub->attributes, &m))) {
