...
static void swf_init_decompress(SWFReader *read)
{
	...
	dst_size -= 8;
	uncompress((Bytef*)dst+8, (uLongf *)&dst_size, (const Bytef*)src, size);
	dst_size += 8;
	free(src);
	read->mem = dst;
	gf_bs_del(read->bs);
...
static char *swf_get_string(SWFReader *read)
{
	...
	if (read->size>1024) {
		name = (char*)malloc(sizeof(char)*read->size);
	} else {
		name = szName;
	}
	while (1) {
	...
	if (read->size>1024) {
		return (char*)realloc(name, sizeof(char)*(strlen(name)+1));
	} else {
		return _strdup(szName);
	}
}
...
static GF_Err swf_def_font(SWFReader *read, u32 revision)
{
	...
	if (revision==0) {
		...
		swf_read_int(read, 8);
		count = swf_read_int(read, 8);
		ft->fontName = (char*)malloc(sizeof(u8)*count+1);
		ft->fontName[count] = 0;
		...
		if (ft->nbGlyphs) {
			...
			ft->glyph_codes = (u16*)malloc(sizeof(u16) * ft->nbGlyphs);
			for (i=0; i<ft->nbGlyphs; i++) {
		...
		if (ft->has_layout) {
			...
			if (ft->nbGlyphs) {
				ft->glyph_adv = (s16*)malloc(sizeof(s16) * ft->nbGlyphs);
...
static GF_Err swf_def_font_info(SWFReader *read)
{
	...
	count = swf_read_int(read, 8);
	ft->fontName = (char*)malloc(sizeof(char) * (count+1));
	ft->fontName[count] = 0;
	for (i=0; i<count; i++) ft->fontName[i] = swf_read_int(read, 8);
	swf_read_int(read, 2);
	ft->is_unicode = swf_read_int(read, 1);
...
static GF_Err swf_def_text(SWFReader *read, u32 revision)
{
	...
	while (1) {
		...
		if (!flag) {
			...
			gr->orig_y = offY;
			gr->nbGlyphs = count;
			gr->indexes = (u32*)malloc(sizeof(u32) * gr->nbGlyphs);
			gr->dx = (Fixed*)malloc(sizeof(Fixed) * gr->nbGlyphs);
...
static GF_Err swf_def_edit_text(SWFReader *read)
{
	...
	free(var_name);
	if (txt.init_value) free(txt.init_value);
...
static GF_Err swf_def_sound(SWFReader *read)
{
	...
	case 2:
	{
		...
		if (read->localPath) {
			snd->szFileName = (char*)malloc(sizeof(char)*GF_MAX_PATH);
			strcpy(snd->szFileName, read->localPath);
			strcat(snd->szFileName, szName);
		} else {
			snd->szFileName = _strdup(szName);
		}
...
static GF_Err swf_soundstream_hdr(SWFReader *read)
{
	...
	case 2:
		...
		read->sound_stream->szFileName = _strdup(szName);
...
static GF_Err swf_def_hdr_jpeg(SWFReader *read)
{
	...
	if (read->size) {
		read->jpeg_hdr = (u8*)malloc(sizeof(char)*read->size);
...
static GF_Err swf_def_bits_jpeg(SWFReader *read, u32 version)
{
	...
	if (version==3) {
		...
		osize = w*h*4;
		raw = (char*)malloc(sizeof(char)*osize);
		...
		if (size<AlphaPlaneSize) buf = (u8*)realloc(buf, sizeof(u8)*AlphaPlaneSize);
		swf_read_data(read, (char*)buf, AlphaPlaneSize);

		osize = w*h;
		dst = (char*)malloc(sizeof(char)*osize);
		...
		osize = w*h*4;
		buf = (u8*)realloc(buf, sizeof(char)*osize);
		gf_img_png_enc(raw, w, h, GF_PIXEL_RGBA, (char*)buf, &osize);
...
static GF_Err swf_process_tag(SWFReader *read)
{
	...
	case SWF_DEFINESHAPE2: return swf_parse_shape_def(read, NULL, 1);
	case SWF_DEFINESHAPE3: return swf_parse_shape_def(read, NULL, 2);
	...
	case SWF_DEFINEEDITTEXT: return swf_def_edit_text(read);
...
//	case SWF_DEFINEBUTTONSOUND:
	...
	case SWF_DEFINEBITSJPEG2: return swf_def_bits_jpeg(read, 2);
	case SWF_DEFINEBITSJPEG3: return swf_def_bits_jpeg(read, 3);
...
void gf_sm_load_done_swf(GF_SceneLoader *load)
{
	...
	if (read->jpeg_hdr) free(read->jpeg_hdr);
...
GF_Err gf_sm_load_init_swf(GF_SceneLoader *load)
{
	...
	if (load->localPath) read->localPath = _strdup(load->localPath);
	else {
		char *c;
		read->localPath = _strdup(load->fileName);
