...
static GF_Err gf_text_import_srt_bifs(GF_SceneManager *ctx, GF_ESD *src, GF_MuxInfo *mux)
{
	...
	while (1) {
		...
		if (!sOK || !strlen(szLine)) {
			...
			if (au) {
				...
				if (font && (italic || underlined || bold)) {
					...
					if (bold && italic && underlined) sfstr->buffer = _strdup("BOLDITALIC UNDERLINED");
					else if (italic && underlined) sfstr->buffer = _strdup("ITALIC UNDERLINED");
					else if (bold && underlined) sfstr->buffer = _strdup("BOLD UNDERLINED");
					else if (underlined) sfstr->buffer = _strdup("UNDERLINED");
					else if (bold && italic) sfstr->buffer = _strdup("BOLDITALIC");
					else if (bold) sfstr->buffer = _strdup("BOLD");
					else sfstr->buffer = _strdup("ITALIC");
		...
		default:
			...
			while (1) {
				if (!_strnicmp(ptr, "<i>", 3)) {
					italic = 1;
					ptr += 3;
				}
				else if (!_strnicmp(ptr, "<u>", 3)) {
					underlined = 1;
					ptr += 3;
				}
				else if (!_strnicmp(ptr, "<b>", 3)) {
					bold = 1;
					ptr += 3;
				}
				else
