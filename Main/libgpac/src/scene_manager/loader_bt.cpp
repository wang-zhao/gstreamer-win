...
next_line:
		...
		if ( (parser->line_buffer[parser->line_pos]=='#') 
			...
			if (!_strnicmp(parser->line_buffer+parser->line_pos, "#define ", 8)) {
				char *buf, *sep;
				parser->line_pos+=8;
				...
				if (sep && (sep[1]!='\n') ) {
					...
					sep[0] = 0;
					def->name = _strdup(buf);
					sep[0] = ' ';
					buf = sep+1;
					while (strchr(" \t", buf[0])) buf++;
					def->value = _strdup(buf);
					gf_list_add(parser->def_symbols, def);
				}
			}
			else if (!_strnicmp(parser->line_buffer+parser->line_pos, "#if 0", 5)) {
				parser->block_comment++;
			}
			else if (!_strnicmp(parser->line_buffer+parser->line_pos, "#endif", 6)) {
...
GF_Err gf_bt_parse_bool(GF_BTParser *parser, const char *name, SFBool *val)
{
	...
	if (!_stricmp(str, "true") || !strcmp(str, "1") ) {
		*val = 1;
	}
	else if (!_stricmp(str, "false") || !strcmp(str, "0") ) {
		*val = 0;
	} else {
...
Bool gf_bt_set_field_is(GF_BTParser *parser, GF_FieldInfo *info, GF_Node *n)
{
	...
	if (_strnicmp(&parser->line_buffer[parser->line_pos + i] , "IS", 2)) return 0;
...
GF_Route *gf_bt_parse_route(GF_BTParser *parser, Bool skip_def, Bool is_insert, GF_Command *com)
{
	...
	/*VRML loosy syntax*/
	if ((e != GF_OK) && parser->is_wrl && !_strnicmp(str, "set_", 4)) 
...
GF_Err gf_bt_parse_bifs_command(GF_BTParser *parser, char *name, GF_List *cmdList)
{
	s32 pos;
	GF_Route *r;
	GF_Node *n, *newnode, *opnode;
	GF_Command *com;
	GF_CommandField *inf;
	GF_FieldInfo info;
	char *str, field[1000], *sep;
	if (!name) {
		str = gf_bt_get_next(parser, 0);
	} else {
		str = name;
	...
	/*REPLACE commands*/
 	if (!strcmp(str, "REPLACE")) {
		...
		if (is_op) {
			...
			str = gf_bt_get_next(parser, 0);
			sep = strchr(str, '.');
			if (!sep) return gf_bt_report(parser, GF_BAD_PARAM, "\'.\' expected");
			sep[0] = 0;
			opnode = gf_bt_peek_node(parser, str);
	...
	if (!strcmp(str, "DELETE")) {
		...
		if (!strcmp(str, "ROUTE")) {
			...
			if (!com->RouteID) {
				com->unres_name = _strdup(str);
				com->unresolved = 1;
				gf_list_add(parser->unresolved_routes, com);
			}
			/*for bt<->xmt conversions*/
			com->def_name = _strdup(str);
			return gf_list_add(cmdList, com);
		}
		strcpy(field, str);
		n = gf_bt_peek_node(parser, str);
	...
	while (!parser->last_error) {
		...
		/*compatibility for old bt (mp4tool) in ProtoLibs*/
			...
			if (has_id) {
				...
				if (com) {
					com->RouteID = rID;
					com->def_name = _strdup(szDEFName);
					gf_sg_set_max_defined_route_id(parser->load->scene_graph, rID);
				} else if (r) {
					gf_sg_route_set_id(r, rID);
					gf_sg_route_set_name(r, szDEFName);
		...
		/*BIFS commands*/
			...
			if (force_new_com) {
				force_new_com = 0;
				parser->bifs_au = (GF_AUContext*)gf_list_last(parser->bifs_es->AUs);
