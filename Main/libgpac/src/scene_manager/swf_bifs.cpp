...
{
	u32 i, j;
	srec->path->idx = (u32*)malloc(sizeof(u32)*srec->path->nbPts);
	...
	for (i=0; i<srec->path->nbPts; i++) {
		...
		if (j==c->point.count) {
			c->point.count++;
			c->point.vals = (SFVec2f*)realloc(c->point.vals, sizeof(SFVec2f)*c->point.count);
	...
	while ((gr = (SWFGlyphRec*)gf_list_enum(text->text, &i))) {
		...
		if (use_text) {
			...
			if (ft->fontName) {
				...
				((SFString*)ptr)->buffer = _strdup(ft->fontName);
			}
			gf_sg_vrml_mf_reset(&f->justify, GF_SG_VRML_MFSTRING);
			gf_sg_vrml_mf_append(&f->justify, GF_SG_VRML_MFSTRING, &ptr);
			((SFString*)ptr)->buffer = _strdup("BEGIN");

			if (f->style.buffer) free(f->style.buffer);
			if (ft->is_italic && ft->is_bold) f->style.buffer = _strdup("BOLDITALIC");
			else if (ft->is_bold) f->style.buffer = _strdup("BOLD");
			else if (ft->is_italic) f->style.buffer = _strdup("ITALIC");
			else f->style.buffer = _strdup("PLAIN");
...
static void swf_ntext(void *sax_cbck, const char *content, Bool is_cdata)
{
	...
	len = strlen(content);
	if (!len) return;
	t->final = (char*)realloc(t->final, sizeof(char)*(t->len+len+1));
	t->final [t->len] = 0;
...
static GF_Err swf_bifs_define_edit_text(SWFReader *read, SWFEditText *text)
{
	...
	case 0:
		...
		break;
	case 1:
		((SFString*)ptr)->buffer = _strdup("END"); 
		break;
	case 3:
		((SFString*)ptr)->buffer = _strdup("JUSTIFY"); 
		break;
	default:
		((SFString*)ptr)->buffer = _strdup("MIDDLE"); 
		break;
	}
	...
	if (text->init_value) {
		...
		if (text->html) {
			...
			if (flat.final) {
				((SFString*)ptr)->buffer = _strdup(flat.final);
				free(flat.final);
			}
		} else {
			((SFString*)ptr)->buffer = _strdup(text->init_value);
		}
	}
	...
	if (layout) {		
		...
		switch (text->align) {
		case 0:
			layout->justify.vals[0] = _strdup("BEGIN"); 
			break;
		case 1:
			layout->justify.vals[0] = _strdup("END"); 
			break;
		case 3:
			layout->justify.vals[0] = _strdup("JUSTIFY"); 
			break;
		default:
			layout->justify.vals[0] = _strdup("MIDDLE"); 
			break;
		}
...
static void swf_bifs_end_of_clip(SWFReader *read)
{
	...
	au = (GF_AUContext*)gf_list_get(read->bifs_es->AUs, 0);
...
static GF_Err swf_bifs_setup_sound(SWFReader *read, SWFSound *snd, Bool soundstream_first_block)
{
	...
	mux->file_name = _strdup(snd->szFileName);
...
static GF_Err swf_bifs_setup_image(SWFReader *read, u32 ID, char *fileName)
{
	...
	mux->file_name = _strdup(fileName);
...
static void s2b_set_field(SWFReader *read, GF_List *dst, GF_Node *n, char *fieldName, s32 pos, u32 type, void *val, Bool insert)
{
	...
	for (i=0; i<count; i++) {
		com = (GF_Command*)gf_list_get(dst, i);
		...
		if (type==GF_SG_VRML_SFSTRING) {
			...
			((SFString*)f->field_ptr)->buffer = _strdup( (char *) val);
		} else {
...
static void s2b_control_sprite(SWFReader *read, GF_List *dst, u32 ID, Bool stop, Bool set_time, SFTime mediaStartTime, Bool rev_order)
{
	...
	for (i=0; i<gf_list_count(dst); i++) {
		GF_Command *com = (GF_Command*)gf_list_get(dst, i);
		if (com->node==obj) {
			GF_CommandField *f = (GF_CommandField*)gf_list_get(com->command_fields, 0);
		...
		if (character) {
			...
			if (br->up) {
				...
				if (sprite_ctrl) {
					...
					btnrec = (S2BBtnRec*)malloc(sizeof(S2BBtnRec));
...
static void swf_bifs_finalize(SWFReader *read)
{
	...
	while (gf_list_count(read->buttons)) {
		S2BBtnRec *btnrec = (S2BBtnRec*)gf_list_get(read->buttons, 0);
...
GF_Err swf_to_bifs_init(SWFReader *read)
{
	...
	gf_node_register(n, read->root);
	((M_WorldInfo *)n)->title.buffer = _strdup("GPAC SWF CONVERTION DISCLAIMER");
	...
	((M_WorldInfo *)n)->info.vals[0] = _strdup(szMsg);
	((M_WorldInfo *)n)->info.vals[1] = _strdup("Conversion done using GPAC version " GPAC_FULL_VERSION " - (C) 2000-2005 GPAC");
	((M_WorldInfo *)n)->info.vals[2] = _strdup("Macromedia SWF to MPEG-4 Conversion mapping released under GPL license");
	...
	if (read->flags & GF_SM_SWF_USE_IC2D) {
		...
		url->url = _strdup("urn:inet:gpac:builtin:IndexedCurve2D");
