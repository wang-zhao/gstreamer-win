...
static SVG_Element *svg_parse_element(GF_SVG_Parser *parser, const char *name, const char *name_space, const GF_XMLAttribute *attributes, u32 nb_attributes, SVG_NodeStack *parent, Bool *has_ns)
{
	...
	*has_ns = 0;
	...
	for (i=0; i<nb_attributes; i++) {
		...
		else if (!_stricmp(att->name, "id") || !_stricmp(att->name, "xml:id")) {
			if (!ID) ID = att->value;
		}
	}
	...
	for (i=0; i<nb_attributes; i++) {
		...
		if (!_stricmp(att_name, "style")) {
			gf_svg_parse_style((GF_Node *)elt, att->value);
			continue;
		} 
		if (anim) {
			if (!_stricmp(att_name, "to")) {
				anim->to = _strdup(att->value);
				continue;
			}
			if (!_stricmp(att_name, "from")) {
				anim->from = _strdup(att->value);
				continue;
			}
			if (!_stricmp(att_name, "by")) {
				anim->by = _strdup(att->value);
				continue;
			}
			if (!_stricmp(att_name, "values")) {
				anim->values = _strdup(att->value);
				continue;
			}
			if ((tag == TAG_SVG_animateTransform) && !_stricmp(att_name, "type")) {
				anim->type = _strdup(att->value);
				continue;
			}
		}

		if ((ns == GF_XMLNS_XLINK) && !_stricmp(att_name, "href") ) {
			if (gf_svg_is_animation_tag(tag)) {
				assert(anim);
				anim->target_id = _strdup(att->value);
				/*may be NULL*/
		...
		if ((tag == TAG_SVG_handler) && (ns == GF_XMLNS_XMLEV)) {
			if (!_stricmp(att_name, "event") ) {
				ev_event = att->value;
				continue;
			} 
			if (!_stricmp(att_name, "observer") ) {
		...
		if (ns == GF_XMLNS_LASER) { 
			...
			if (!_stricmp(att_name, "translation") ) {
				if (gf_node_get_attribute_by_tag((GF_Node *)elt, TAG_SVG_ATT_transform, 1, 1, &info)==GF_OK) {
					SVG_Point pt;
					SVG_Transform *mat = (SVG_Transform*)info.far_ptr;
					svg_parse_point(&pt, att->value);
		...
		if (!strncmp(att_name, "on", 2)) {
			...
			if (evtType != GF_EVENT_UNKNOWN) {
				...
				gf_dom_add_text_node((GF_Node *)handler, _strdup(att->value) );
				continue;
			} 
	...
	if (ev_event) {
		...
		if ( ((XMLRI *)info.far_ptr)->target) 
			gf_node_dom_listener_add((GF_Node*)((XMLRI *)info.far_ptr)->target, (GF_Node *) listener);
		else
	...
	case GF_SG_LSR_INSERT:
		...
		if (!strcmp(atAtt, "textContent")) {
			...
			if (atValue) {
				...
				*(SVG_String *)field->field_ptr = _strdup(atValue);
			}
			return GF_OK;
		}
	...
	case GF_SG_LSR_SEND_EVENT:
		...
		if (atString) {
			...
			if (key == GF_KEY_UNIDENTIFIED) {
				parser->command->send_event_string = _strdup(atString);
			} else {
				...
				*/
				{
				   ...
				   count = gf_list_count(parser->defered_animations);
				   for (i = 0; i < count; i++) {
					   anim = (SVG_DeferedAnimation*)gf_list_get(parser->defered_animations, i);
...
static void svg_text_content(void *sax_cbck, const char *text_content, Bool is_cdata)
{
	...
	if (!elt || tag == TAG_LSR_conditional) {
		...
		text->textContent = _strdup(text_content);
	...
	if (!skip_text) {
		text = gf_dom_add_text_node((GF_Node *)elt, _strdup(text_content));
