...
#include <internal/crypt_dev.h>


static Bool gf_crypt_assign_algo(GF_Crypt *td, const char *algorithm)
{
	if (!_stricmp(algorithm, "AES-128") || !_stricmp(algorithm, "Rijndael-128")) { gf_crypt_register_rijndael_128(td); return 1; }
#ifndef GPAC_CRYPT_ISMA_ONLY
	else if (!_stricmp(algorithm, "AES-192") || !_stricmp(algorithm, "Rijndael-192")) { gf_crypt_register_rijndael_192(td); return 1; }
	else if (!_stricmp(algorithm, "AES-256") || !_stricmp(algorithm, "Rijndael-256")) { gf_crypt_register_rijndael_256(td); return 1; }
	else if (!_stricmp(algorithm, "DES")) { gf_crypt_register_des(td); return 1; }
	else if (!_stricmp(algorithm, "3DES")) { gf_crypt_register_3des(td); return 1; }
#endif
	return 0;
}
static Bool gf_crypt_assign_mode(GF_Crypt *td, const char *mode)
{
	if (!_stricmp(mode, "CTR")) { gf_crypt_register_ctr(td); return 1; }
#ifndef GPAC_CRYPT_ISMA_ONLY
	else if (!_stricmp(mode, "CBC")) { gf_crypt_register_cbc(td); return 1; }
	else if (!_stricmp(mode, "CFB")) { gf_crypt_register_cfb(td); return 1; }
	else if (!_stricmp(mode, "ECB")) { gf_crypt_register_ecb(td); return 1; }
	else if (!_stricmp(mode, "nCFB")) { gf_crypt_register_ncfb(td); return 1; }
	else if (!_stricmp(mode, "nOFB")) { gf_crypt_register_nofb(td); return 1; }
	else if (!_stricmp(mode, "OFB")) { gf_crypt_register_ofb(td); return 1; }
	else if (!_stricmp(mode, "STREAM")) { gf_crypt_register_stream(td); return 1; }
#endif
	return 0;
}
