...
GF_Node *gf_xml_node_clone(GF_SceneGraph *inScene, GF_Node *orig, GF_Node *cloned_parent, char *inst_id, Bool deep)
{
	...
	if (orig->sgprivate->tag == TAG_DOMText) {
		...
		n_dst->textContent = _strdup(n_src->textContent);
	} else {
...
{
	char szFile[GF_MAX_PATH], buf[20],  *data, *ext;
	char* sep = 0;
	u32 data_size, idx;
	Bool existing;
	FILE *f;
...
#ifdef WIN32
		if (!sep) sep = (char*)strrchr(base_filename, '/');
#endif
		if (!sep) sep = (char *) base_filename;
		else sep += 1;
		strcat(szFile, sep);
	}
	sep = (char*)strrchr(szFile, '.');
	if (sep) sep[0] = 0;
	strcat(szFile, "_img_");
	...
	if (!strncmp(sep, ";base64,", 8)) {
		...
		data_size = gf_base64_decode((char*)sep, strlen(sep), data, data_size);
	}
		...
		if (!data) return GF_OUT_OF_MEM;
		sep += 8;
		data_size = gf_base16_decode((char*)sep, strlen(sep), data, data_size);
	}
