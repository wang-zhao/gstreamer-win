...
char *dom_node_flatten_text(GF_Node *n)
{
	...
	if ((n->sgprivate->tag==TAG_DOMText) && ((GF_DOMText*)n)->textContent) {
		if ( ((GF_DOMText*)n)->type == GF_DOM_TEXT_REGULAR) {
			res = _strdup(((GF_DOMText*)n)->textContent);
			len = strlen(res);
		}
	}
...
JSBool dom_event_add_listener_ex(JSContext *c, JSObject *obj, uintN argc, jsval *argv, jsval *rval, GF_Node *vrml_node)
{
	...
	if (callback) gf_dom_add_text_node((GF_Node *)handler, _strdup(callback));
...
void dom_node_set_textContent(GF_Node *n, char *text)
{
	...
	par->children = NULL;
	gf_dom_add_text_node(n, _strdup( text) );
	dom_node_changed(n, 1, NULL);
}
		...
		if (evtType != GF_EVENT_UNKNOWN) {
			...
			for (i=0;i<count; i++) {
				...
				if (text->sgprivate->tag==TAG_DOMText) {
					...
					text->textContent = _strdup(val);
				}
				goto exit;
			}
			/*nope, create a listener*/
			handler = gf_dom_listener_build(n, evtType, 0);
			gf_dom_add_text_node((GF_Node*)handler, _strdup(val) );
			goto exit;
		}
	}
...
static JSBool xml_element_set_id(JSContext *c, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
	...
	if (is_id) {
		...
		gf_node_set_id(n, gf_sg_get_max_node_id(n->sgprivate->scenegraph) + 1, _strdup(name) );
	} else if (node_id) {
		gf_node_remove_id(n);
	}
	return JS_TRUE;
...
static JSBool dom_text_setProperty(JSContext *c, JSObject *obj, jsval id, jsval *vp)
{
	...
	case 1: /*"data"*/
		...
		if (JSVAL_CHECK_STRING(*vp)) {
			...
			txt->textContent = str ? str : _strdup("" );
		}
...
static void xml_http_append_send_header(XMLHTTPContext *ctx, char *hdr, char *val)
{
	...
	if (ctx->headers) {
		...
		while (ctx->headers && ctx->headers[nb_hdr]) {
			if (_stricmp(ctx->headers[nb_hdr], hdr)) {
				nb_hdr+=2;
				continue;
			}
			/*ignore these ones*/
			if (!_stricmp(hdr, "Accept-Charset")
				|| !_stricmp(hdr, "Accept-Encoding")
				|| !_stricmp(hdr, "Content-Length")
				|| !_stricmp(hdr, "Expect")
				|| !_stricmp(hdr, "Date")
				|| !_stricmp(hdr, "Host")
				|| !_stricmp(hdr, "Keep-Alive")
				|| !_stricmp(hdr, "Referer")
				|| !_stricmp(hdr, "TE")
				|| !_stricmp(hdr, "Trailer")
				|| !_stricmp(hdr, "Transfer-Encoding")
				|| !_stricmp(hdr, "Upgrade") 
				) {
				return;
			}

			/*replace content for these ones*/
			if (!_stricmp(hdr, "Authorization")
				|| !_stricmp(hdr, "Content-Base")
				|| !_stricmp(hdr, "Content-Location")
				|| !_stricmp(hdr, "Content-MD5")
				|| !_stricmp(hdr, "Content-Range")
				|| !_stricmp(hdr, "Content-Type")
				|| !_stricmp(hdr, "Content-Version")
				|| !_stricmp(hdr, "Delta-Base")
				|| !_stricmp(hdr, "Depth")
				|| !_stricmp(hdr, "Destination")
				|| !_stricmp(hdr, "ETag")
				|| !_stricmp(hdr, "From")
				|| !_stricmp(hdr, "If-Modified-Since")
				|| !_stricmp(hdr, "If-Range")
				|| !_stricmp(hdr, "If-Unmodified-Since")
				|| !_stricmp(hdr, "Max-Forwards")
				|| !_stricmp(hdr, "MIME-Version")
				|| !_stricmp(hdr, "Overwrite")
				|| !_stricmp(hdr, "Proxy-Authorization")
				|| !_stricmp(hdr, "SOAPAction")
				|| !_stricmp(hdr, "Timeout") ) {
				free(ctx->headers[nb_hdr+1]);
				ctx->headers[nb_hdr+1] = _strdup(val);
				return;
			}
			/*append value*/
			else {
	...
	ctx->headers[nb_hdr] = _strdup(hdr);
	ctx->headers[nb_hdr+1] = _strdup(val ? val : "");
...
static void xml_http_append_recv_header(XMLHTTPContext *ctx, char *hdr, char *val)
{
	...
	ctx->recv_headers[nb_hdr] = _strdup(hdr);
	ctx->recv_headers[nb_hdr+1] = _strdup(val ? val : "");
...
static JSBool xml_http_open(JSContext *c, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
	...
	if (!ctx->url) ctx->url = _strdup(val);
...
static void xml_http_sax_start(void *sax_cbck, const char *node_name, const char *name_space, const GF_XMLAttribute *attributes, u32 nb_attributes)
{
	...
	for (i=0; i<nb_attributes; i++) {
		...
		if (!_stricmp(attributes[i].name, "xml:id")) {
			...
			att->name = _strdup(attributes[i].name);
			att->data = _strdup(attributes[i].value);
			if (prev) prev->next = (GF_DOMAttribute*)att;
			else node->attributes = (GF_DOMAttribute*)att;
			prev = att;
		}
...
static void xml_http_sax_text(void *sax_cbck, const char *content, Bool is_cdata)
{
	...
	if (par) {
		...
		txt = gf_dom_add_text_node((GF_Node *)par, _strdup(content) );
...
static void xml_http_on_data(void *usr_cbk, GF_NETIO_Parameter *parameter)
{
	...
	case GF_NETIO_PARSE_REPLY:
		...
		if (parameter->value) {
			ctx->statusText = _strdup(parameter->value);
		}
		ctx->readyState = 3;
		xml_http_state_change(ctx);
		return;
...
//			if (!JS_InstanceOf(c, JSVAL_TO_OBJECT(argv[0]), &documentClass, NULL) ) return JS_TRUE;
	...
	ctx->data = data ? _strdup(data) : NULL;
...
static JSBool dcci_setProperty(JSContext *c, JSObject *obj, jsval id, jsval *vp)
{
	...
	if (JSVAL_IS_INT(id)) {
		...
		case 0: 
			...
			if (!child)
				gf_dom_add_text_node((GF_Node*)n, _strdup(str) );
	...
	while (child) {
		if (child->node->sgprivate->tag == TAG_DOMText) {
			...
			if (!strcmp(txt->textContent, content) && ((txt->type==GF_DOM_TEXT_REGULAR) || is_cdata))
				return;
			if (txt->textContent) free(txt->textContent);
			txt->textContent = _strdup(content);
			txt->type = is_cdata ? GF_DOM_TEXT_CDATA : GF_DOM_TEXT_REGULAR;
			break;
		}
		child = child->next;
