...
GF_Proto *gf_sg_find_proto(GF_SceneGraph *sg, u32 ProtoID, char *name)
{
	...
	while ((proto = (GF_Proto *)gf_list_enum(sg->protos, &i))) {
		...
		if (name) {
			if (proto->Name && !_stricmp(name, proto->Name)) return proto;
...
{
	if (!_stricmp(fieldType, "SFBool")) return GF_SG_VRML_SFBOOL;
	else if (!_stricmp(fieldType, "SFFloat")) return GF_SG_VRML_SFFLOAT;
	else if (!_stricmp(fieldType, "SFDouble")) return GF_SG_VRML_SFDOUBLE;
	else if (!_stricmp(fieldType, "SFTime")) return GF_SG_VRML_SFTIME;
	else if (!_stricmp(fieldType, "SFInt32")) return GF_SG_VRML_SFINT32;
	else if (!_stricmp(fieldType, "SFString")) return GF_SG_VRML_SFSTRING;
	else if (!_stricmp(fieldType, "SFVec2f")) return GF_SG_VRML_SFVEC2F;
	else if (!_stricmp(fieldType, "SFVec3f")) return GF_SG_VRML_SFVEC3F;
	else if (!_stricmp(fieldType, "SFVec2d")) return GF_SG_VRML_SFVEC2D;
	else if (!_stricmp(fieldType, "SFVec3d")) return GF_SG_VRML_SFVEC3D;
	else if (!_stricmp(fieldType, "SFColor")) return GF_SG_VRML_SFCOLOR;
	else if (!_stricmp(fieldType, "SFColorRGBA")) return GF_SG_VRML_SFCOLORRGBA;
	else if (!_stricmp(fieldType, "SFRotation")) return GF_SG_VRML_SFROTATION;
	else if (!_stricmp(fieldType, "SFImage")) return GF_SG_VRML_SFIMAGE;
	else if (!_stricmp(fieldType, "SFNode")) return GF_SG_VRML_SFNODE;

	else if (!_stricmp(fieldType, "MFBool")) return GF_SG_VRML_MFBOOL;
	else if (!_stricmp(fieldType, "MFFloat")) return GF_SG_VRML_MFFLOAT;
	else if (!_stricmp(fieldType, "MFDouble")) return GF_SG_VRML_MFDOUBLE;
	else if (!_stricmp(fieldType, "MFTime")) return GF_SG_VRML_MFTIME;
	else if (!_stricmp(fieldType, "MFInt32")) return GF_SG_VRML_MFINT32;
	else if (!_stricmp(fieldType, "MFString")) return GF_SG_VRML_MFSTRING;
	else if (!_stricmp(fieldType, "MFVec2f")) return GF_SG_VRML_MFVEC2F;
	else if (!_stricmp(fieldType, "MFVec3f")) return GF_SG_VRML_MFVEC3F;
	else if (!_stricmp(fieldType, "MFVec2d")) return GF_SG_VRML_MFVEC2D;
	else if (!_stricmp(fieldType, "MFVec3d")) return GF_SG_VRML_MFVEC3D;
	else if (!_stricmp(fieldType, "MFColor")) return GF_SG_VRML_MFCOLOR;
	else if (!_stricmp(fieldType, "MFColorRGBA")) return GF_SG_VRML_MFCOLORRGBA;
	else if (!_stricmp(fieldType, "MFRotation")) return GF_SG_VRML_MFROTATION;
	else if (!_stricmp(fieldType, "MFImage")) return GF_SG_VRML_MFIMAGE;
	else if (!_stricmp(fieldType, "MFNode")) return GF_SG_VRML_MFNODE;
	...
	case GF_SG_VRML_SFSTRING:
		if (ori_field_type == GF_SG_VRML_SFURL) {
			...
			if (url->OD_ID>0) {
				...
				((SFString*)dest)->buffer = _strdup(tmp);
			} else {
				if ( ((SFString*)dest)->buffer) free(((SFString*)dest)->buffer);
				((SFString*)dest)->buffer = _strdup(url->url);
			}
		}
		/*for SFString to MFString cast*/
		else if (ori_field_type == GF_SG_VRML_SFSTRING) {
			if ( ((SFString*)dest)->buffer) free(((SFString*)dest)->buffer);
			((SFString*)dest)->buffer = _strdup(((SFString*)orig)->buffer);
		}
		return;
	case GF_SG_VRML_SFURL:
		...
		if ( ((SFString*)orig)->buffer) 
			url->url = _strdup(((SFString*)orig)->buffer);
		else 
			url->url = NULL;
		return;
	case GF_SG_VRML_MFSTRING:
...
void gf_sg_vrml_field_copy(void *dest, void *orig, u32 field_type)
{
	...
	case GF_SG_VRML_SFURL:
		...
		if (((SFURL *)orig)->url) 
			((SFURL *)dest)->url = _strdup(((SFURL *)orig)->url);
		else
			((SFURL *)dest)->url = NULL;
		break;
	case GF_SG_VRML_SFIMAGE:
	...
	case GF_SG_VRML_SFSCRIPT:
		...
		if ( ((SFScript*)orig)->script_text)
			((SFScript *)dest)->script_text = (u8*)_strdup( (char*) ((SFScript*)orig)->script_text );
		break;
	...
	case GF_SG_VRML_MFROTATION:
		...
		if (((GenMFField *)orig)->count != ((GenMFField *)dest)->count) {
			((GenMFField *)dest)->array = (char*)realloc(((GenMFField *)dest)->array, size);
