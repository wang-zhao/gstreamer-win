...
void gf_svg_node_del(GF_Node *node)
{
	...
	if (p->sgprivate->tag==TAG_SVG_listener) {
		...
		GF_DOMEventTarget *evt = (GF_DOMEventTarget*)node->sgprivate->UserPrivate;
	...
	/*remove this node from associated listeners*/
		...
		for (i=0; i<count; i++) {
			GF_Node *listener = (GF_Node*)gf_list_get(node->sgprivate->interact->dom_evt->evt_list, i);
