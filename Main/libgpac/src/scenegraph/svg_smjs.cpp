...
static JSBool dom_imp_has_feature(JSContext *c, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
	*rval = BOOLEAN_TO_JSVAL(JS_FALSE);
	if (argc) {
		...
		sep = fname[len];
		fname[len] = 0;
		if (!_stricmp(fname, "xml")) *rval = BOOLEAN_TO_JSVAL(JS_TRUE);
		else if (!_stricmp(fname, "core")) *rval = BOOLEAN_TO_JSVAL(JS_TRUE);
		else if (!_stricmp(fname, "traversal")) *rval = BOOLEAN_TO_JSVAL(JS_TRUE);
		else if (!_stricmp(fname, "uievents")) *rval = BOOLEAN_TO_JSVAL(JS_TRUE);
		else if (!_stricmp(fname, "mouseevents")) *rval = BOOLEAN_TO_JSVAL(JS_TRUE);
		else if (!_stricmp(fname, "mutationevents")) *rval = BOOLEAN_TO_JSVAL(JS_TRUE);
		else if (!_stricmp(fname, "events")) *rval = BOOLEAN_TO_JSVAL(JS_TRUE);
		...
		if (JSVAL_CHECK_STRING(*vp)) {
			...
			if (id) {
				...
				if (gf_node_get_attribute_by_tag(n, TAG_XML_ATT_id, 1, 0, &info)==GF_OK) {
					...
					*(DOM_String *)info.far_ptr = _strdup(id);
				}
...
void JSScript_LoadSVG(GF_Node *node)
{
	...
	if (gf_node_get_attribute_by_tag(node, TAG_XLINK_ATT_href, 0, 0, &href_info) == GF_OK) {
		...
		if (!url) url = _strdup(xmlri->string);
