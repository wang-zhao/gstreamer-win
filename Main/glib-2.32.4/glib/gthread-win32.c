...
g_thread_xp_DeleteSRWLock (gpointer mutex)
{
  ...
  if (lock && g_atomic_int_compare_and_exchange((GThreadSRWLock * volatile *) mutex, (gint)lock, 0))
    {
      if (lock->ever_shared)
...
g_thread_xp_DeleteConditionVariable (gpointer cond)
{
  ...
  if (cv && g_atomic_int_compare_and_exchange((GThreadXpCONDITION_VARIABLE * volatile *) cond, (gint)cv, 0))
  {
	  free (cv);
	  *(GThreadXpCONDITION_VARIABLE * volatile *) cond = NULL;
  }
