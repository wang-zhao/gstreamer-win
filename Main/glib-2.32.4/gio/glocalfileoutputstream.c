				  ...
				  GError        **error)
{
  ...
  while (1)
	...
	return -1;
      res = g_write (file->priv->fd, buffer, count);
      if (res == -1)
	{
          int errsv = errno;
					  ...
					  GError        **error)
{
   ...
   */
  if (g_fstati64 (file->priv->fd, &final_stat) == 0)
    file->priv->etag = _g_local_file_info_create_etag (&final_stat);

  res = g_close (file->priv->fd);
  if (res == -1)
    {
      int errsv = errno;
...
g_local_file_output_stream_tell (GFileOutputStream *stream)
{
  ...
  pos = g_lseek (file->priv->fd, 0, SEEK_CUR);
				 ...
				 GError            **error)
{
  ...
  pos = g_lseek (file->priv->fd, offset, seek_type_to_lseek (type));
		...
		GError **error)
{
  ...
  do
    {
      bytes_read = g_read (sfd, buffer, BUFSIZE);
      if (bytes_read == -1)
	{
          int errsv = errno;
      ...
      do
	{
	  bytes_written = g_write (dfd, write_buffer, bytes_to_write);
	  if (bytes_written == -1)
	    {
              int errsv = errno;
...
#ifdef G_OS_WIN32
  res = g_fstati64 (fd, &original_stat);
#else
...
#ifdef G_OS_WIN32
          tres = g_fstati64 (tmpfd, &tmp_statbuf);
#else
